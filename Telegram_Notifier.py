...
Purpose: Headless Streamlit rendering and Telegram transmission.
         ...
         - UPDATED: v38 - Pointed subprocess to dashboard_pro_v184.py
         - UPDATED: v37 - Pointed subprocess to dashboard_pro_v183.py
         - UPDATED: v36 - Pointed subprocess to dashboard_pro_v182.py
         - UPDATED: v35 - Pointed subprocess to dashboard_pro_v181.py
         - UPDATED: v34 - Pointed subprocess to dashboard_pro_v180.py
         - UPDATED: v33 - Integrated estate_env.py for dynamic OS-agnostic pathing.
=============================================================================
"""
...
from estate_env import TARGET_DIR, CONFIG_PATH, DB_PATH
...
def snap_and_send():
    ...
    try:
        ...
        # --- NEW: ROUTE SOCIAL MEDIA ASSETS ---
        promo_png_path = os.path.join(TARGET_DIR, "market_flow_promo.png")
        social_txt_path = os.path.join(TARGET_DIR, "market_flow_social.txt")
        
        if os.path.exists(promo_png_path) and os.path.exists(social_txt_path):
            print("[NOTIFIER] Found Market Flow social assets. Transmitting to Telegram...")
            with open(social_txt_path, "r", encoding="utf-8") as f:
                social_text = f.read()
                
            url_photo = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendPhoto"
            with open(promo_png_path, "rb") as photo_file:
                # Telegram captions are limited to 1024 chars. If text is short, send as caption.
                if len(social_text) <= 1000:
                    payload_photo = {"chat_id": CHAT_ID, "caption": social_text, "parse_mode": "Markdown"}
                    requests.post(url_photo, data=payload_photo, files={"photo": photo_file})
                else:
                    # Send photo first, then text as a separate message
                    requests.post(url_photo, data={"chat_id": CHAT_ID}, files={"photo": photo_file})
                    url_msg = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
                    requests.post(url_msg, data={"chat_id": CHAT_ID, "text": social_text, "parse_mode": "Markdown"})
                    
            print("[OK] Social media assets delivered to Telegram.")
            
            # Clean up assets so they don't get sent again tomorrow if the engine didn't run
            os.remove(promo_png_path)
            os.remove(social_txt_path)
