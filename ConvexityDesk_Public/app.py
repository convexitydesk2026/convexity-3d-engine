...
# 3. MAIN UI LAYOUT
col_grid, col_chart = st.columns([0.15, 0.85])

with col_grid:
    st.subheader("2. Edit Realized History")
    st.markdown("Type in a huge loss (e.g. -50000) to simulate a Black Swan.")
    # Editable Grid
    edited_df = st.data_editor(st.session_state['df_pnl'], use_container_width=True, height=700)
...
with col_chart:
    ...
    m1.metric("P&L", f"${metrics['pnl']:,.0f}")
    m2.metric("ROC", f"{metrics['roc']:.2f}%")
    m3.metric("IRR", f"{metrics['irr']:.2f}%")
    m4.metric("Sharpe", f"{metrics['sharpe']:.2f}")
    m5.metric("Max DD", f"${metrics['max_dd_dollars']:,.0f}")
    m6.metric("Calmar", f"{metrics['calmar']:.2f}")
    m7.metric("Alpha", f"{metrics['alpha']:.2f}%")
    m8.metric("Beta", f"{metrics['beta']:.2f}")
    m9.metric("Corr", f"{metrics['correlation']:.2f}")
