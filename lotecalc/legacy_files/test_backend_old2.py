...
VERSION: 1.2 (Updated for Permuta & 30/70 Cashflow)
DATE: August 03, 2026
...
def run_full_backend_test():
    ...
    print("\n3. Simulating User Inputs (Width: 30m, Depth: 40m, Price: $1,500,000, Permuta: $500,000)...")
    validation = validate_lot_inputs(width="30", depth="40", asking_price_str="1,500,000")
    
    if not validation["is_valid"]:
        print(f"❌ TEST FAILED: Validation Errors: {validation['errors']}")
        return
    
    width, depth, asking_price = validation["clean_data"]
    print("✅ Inputs Validated Successfully.")

    # STEP 4: Run the Math Engine
    print("\n4. Running Financial Math Engine (With 30/70 Cashflow & Permuta)...")
    engine = FeasibilityEngine(
        ...
        permuta_amount=500000  # NEW: Developer trades $500k in apartments for land
    )
