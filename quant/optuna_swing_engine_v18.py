# ==============================================================================
# File: C:/Users/donca/Desktop/Desktop HP Envy x360 al 22Abr24/Docs Manuel/IBKR_Options/quant/optuna_swing_engine_v18.py
# Version: 18.0
# Description:
#   Vectorized Swing Trading Optuna Engine (Time-Priority ORB Execution).
#   v18 implements true Opening Range Breakouts, ensuring 1-min Close > Resistance
#   with a Rising Vector (Green Candle). It sorts breakouts chronologically to 
#   maximize capital efficiency on a first-come, first-served basis.
# ==============================================================================
...
log_file = os.path.join(LOG_DIR, f"optuna_run_v18_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
class VectorizedSwingEngine:
    ...
    def objective(self, trial):
        ...
        for date in self.trading_dates:
            ...
            # Pre-scan Phase: Find all confirmed breakouts for the day
            for row in candidates:
                ...
                for i in range(last_orb_idx + 1, len(bars)):
                    ...
                    # The V18 Rules: Close must beat Resistance AND it must be a Green Candle
            ...
            for b_time, b_idx, sym, entry_px, initial_stop, risk_ps, row, bars in today_breakouts:
                if len(open_positions) >= p_max_positions or cash < 100: 
                    break # Time-Priority Block - Max Capacity Reached!
                ...
                for i in range(b_idx + 1, len(bars)):
                    ...
                    if not day1_target_hit:
                        if current_low <= active_stop:
                            cash += shares * (min(active_stop, bars[i, 1]) - 0.005)
                            day1_exit = True
                            break
