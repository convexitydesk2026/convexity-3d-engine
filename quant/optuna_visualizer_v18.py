# ==============================================================================
# File: C:/Users/donca/Desktop/Desktop HP Envy x360 al 22Abr24/Docs Manuel/IBKR_Options/quant/optuna_visualizer_v18.py
# Version: 18.0
# Description: Visualizer matching the V18 Chronological ORB Engine.
# ==============================================================================
...
from optuna_swing_engine_v18 import VectorizedSwingEngine, PROJECT_DIR
...
def visualize_champion_run():
    ...
    p_orb = int(best_params['orb_window'])
    p_target = float(best_params['atr_target'])
    p_r_mult = float(best_params['transition_r'])
    p_sma = str(best_params['trailing_sma'])
    p_max_positions = int(best_params['max_positions'])
    p_atr_stop_mult = float(best_params['atr_stop_multiplier'])
    p_risk_pct = float(best_params['risk_pct'])
    ...
    print("Simulating Champion Run (V18 ORB Logic)...")
    for date in engine.trading_dates:
        bod_nav = cash
        for sym, pos in open_positions.items():
            if (date, sym) in engine.daily_dict:
                pos['last_known_price'] = engine.daily_dict[(date, sym)]['d_close_prev']
        ...
        closed_syms = []
        for sym, pos in open_positions.items():
            if (date, sym) not in engine.daily_dict: 
                cash += pos['shares'] * (pos['last_known_price'] - 0.005)
                closed_syms.append(sym)
                trade_logs.append({
                    'symbol': sym, 'entry_time': pos['entry_time'], 'exit_time': f"{date.strftime('%Y-%m-%d')} (Delisted)",
                    'shares': pos['shares'], 'entry_price': pos['entry_price'], 'exit_price': pos['last_known_price'],
                    'pnl': pos['shares'] * (pos['last_known_price'] - pos['entry_price'] - 0.01)
                })
                continue
        ...
        is_bull_market = engine.spy_regime.get(date_str, True)
        if engine.use_regime_filter and not is_bull_market:
            continue 
            
        candidates = engine.candidates_by_date.get(date, [])
        today_breakouts = []
        
        # Pre-scan Phase (V18 ORB)
        for row in candidates:
            ...
            if last_orb_idx >= len(bars) - 1: continue 
            
            orb_high = np.max(bars[:last_orb_idx+1, 2])
            
            for i in range(last_orb_idx + 1, len(bars)):
                _open, _high, _low, _close = bars[i, 1:5]
                if _close > orb_high and _close > _open:
                    breakout_time = bars[i, 0]
                    risk_ps = row['atr_14_prev'] * p_atr_stop_mult
                    initial_stop = _close - risk_ps
                    if risk_ps > 0 and initial_stop > 0:
                        today_breakouts.append((breakout_time, i, sym, _close, initial_stop, risk_ps, row, bars))
                    break
                    
        # Chronological Execution
        today_breakouts.sort(key=lambda x: x[0])
        
        for b_time, b_idx, sym, entry_px, initial_stop, risk_ps, row, bars in today_breakouts:
            if len(open_positions) >= p_max_positions or cash < 100: 
                break 
            ...
            entry_time_str = f"{date.strftime('%Y-%m-%d')} {int(9 + (30+b_time)//60):02d}:{(30+b_time)%60:02d}:00"
            
            day1_exit = False
            day1_target_hit = False
            active_stop = initial_stop
            
            for i in range(b_idx + 1, len(bars)):
                current_high, current_low = bars[i, 2], bars[i, 3]
                if not day1_target_hit:
                    if current_low <= active_stop:
                        exit_price = min(active_stop, bars[i, 1])
    ...
    subtitle = f"ORB: {p_orb}m | Target: {p_target} | R-Mult: {p_r_mult} | SMA: {p_sma} | Max Pos: {p_max_positions} | ATR Stop: {p_atr_stop_mult}x | Risk: {p_risk_pct}"
    fig.update_layout(title=f"Champion Equity Curve (V18 ORB Engine)<br><sup>{subtitle}</sup>", template="plotly_dark", height=800)
    
    out_path = os.path.join(PROJECT_DIR, "champion_visualization_v18.html")
