# ==============================================================================
# File: C:/Users/donca/Desktop/Desktop HP Envy x360 al 22Abr24/Docs Manuel/IBKR_Options/quant/optuna_visualizer_v3.py
# Version: 3.0
# Description:
#   Visualizer for the 7.5x ATR Swing Engine. 
#   v3 Enhancement: Automatically retrieves the absolute Best Trial from the DB,
#   simulates the portfolio, generates the HTML equity curve, AND queries the 
#   database on-the-fly to find the EXACT minute of exit for Day 2+ holds.
#   It saves every single trade to a permanent Postgres table `champion_trade_log`
#   so you can verify the minute timestamps in TC2000.
# ==============================================================================
...
# Import from the v3 engine script
from optuna_swing_engine_v3 import VectorizedSwingEngine, PROJECT_DIR
...
def visualize_champion_run():
    ...
    # List to store detailed trade logs for the database
    trade_logs = []
    
    print("Simulating Champion Run and tracing exact exit timestamps...")
    ...
    for date in engine.trading_dates:
        ...
        for sym, pos in open_positions.items():
            ...
            # If stopped out on Day 2+, find the EXACT minute from DB
            if exit_price is not None:
                cash += pos['shares'] * (exit_price - 0.005)
                closed_syms.append(sym)
                
                # Query DB to find the exact minute of the breach
                query = f"""
            ...
            for sym, row in candidates.iterrows():
                ...
                if not day1_exit:
                    open_positions[sym] = {
                        ...
                        'initial_stop': lod, 'active_stop': lod, 'target_price': target, 
                        'state': 'trailing' if np.max(bars[idx+1:, 2]) >= target else 'waiting_target'
                    }

    print("Uploading Exact Trade Logs to Database Table: champion_trade_log")
    df_logs = pd.DataFrame(trade_logs)
    df_logs.to_sql('champion_trade_log', engine.engine, if_exists='replace', index=False)
    print("Logs saved successfully. Check PgAdmin4 to verify timestamps!")
