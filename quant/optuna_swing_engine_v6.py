# ==============================================================================
# File: C:/Users/donca/Desktop/Desktop HP Envy x360 al 22Abr24/Docs Manuel/IBKR_Options/quant/optuna_swing_engine_v6.py
# Version: 6.0
# Description:
#   Vectorized Swing Trading Optuna Engine (7.5x ATR Core).
#   v6 Fixes the MultiIndex Lexsort warning and guarantees scalar returns for 
#   prices to prevent Series ambiguity crashes. Removes the 100-day hard limit
#   so shorter smoke tests can score gradients correctly.
# ==============================================================================
...
class VectorizedSwingEngine:
    ...
    def load_data(self):
        ...
        # V6 Fix: Explicitly sort the MultiIndex so Pandas loc/at works perfectly
        ...
        df_top_cands = df_candidates.sort_values(['trading_date', 'ratio'], ascending=[True, False]).groupby('trading_date').head(40)
        
        logging.info(f"Memory Optimization: Reduced from {len(df_candidates)} to {len(df_top_cands)} hyper-targeted candidate days.")
        logging.info("Fetching Entry 1-Min Bars for top targets...")
        ...
        for date in self.trading_dates:
            ...
            for sym, pos in open_positions.items():
                if (date, sym) in self.daily_data.index:
                    # V6 Fix: Using .at to explicitly guarantee a single float is returned
            ...
            for sym, pos in open_positions.items():
                ...
                # V6 Fix: Using .at to explicitly extract scalars
