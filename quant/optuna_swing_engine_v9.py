# ==============================================================================
# File: C:/Users/donca/Desktop/Desktop HP Envy x360 al 22Abr24/Docs Manuel/IBKR_Options/quant/optuna_swing_engine_v9.py
# Version: 9.0
# Description:
#   Vectorized Swing Trading Optuna Engine (7.5x ATR Core).
#   v9 completely strips Pandas out of the Optuna objective loop. By converting 
#   the data to Pure Python Dictionaries beforehand, we eliminate all MultiIndex
#   "scalar access" crashes and drastically increase simulation speed.
# ==============================================================================
...
class VectorizedSwingEngine:
    ...
    def load_data(self):
        ...
        # --- V9 FIX: Convert Daily DataFrame to Pure Python Dictionary for Crash-Proof Loop Lookups ---
        ...
        # --- V9 FIX: Convert Candidates to Dict to remove Pandas iterating ---
        ...
        for date in self.trading_dates:
            ...
            for sym, pos in open_positions.items():
                if (date, sym) in self.daily_dict:
                    bod_nav += pos['shares'] * self.daily_dict[(date, sym)]['d_close_prev']
            daily_nav_curve.append(bod_nav)
            target_risk = bod_nav * self.risk_pct
            max_capital_per_trade = bod_nav / self.max_positions
            
            closed_syms = []
            for sym, pos in open_positions.items():
                if (date, sym) not in self.daily_dict: continue
            ...
            for row in candidates:
                if row['ratio'] < p_target: continue # Filter for dynamic target
                sym = row['symbol']
