# ==============================================================================
# File: C:/Users/donca/Desktop/Desktop HP Envy x360 al 22Abr24/Docs Manuel/IBKR_Options/quant/optuna_swing_engine_v8.py
# Version: 8.0
# Description:
#   Vectorized Swing Trading Optuna Engine (7.5x ATR Core).
#   v8 fixes the catastrophic scaling bugs:
#   - Implements strict Alphabetic-Only ticker filtering (kills warrants/units).
#   - Enforces $4.00 Minimum Price filter to stop penny stock commission drain.
#   - Adds a Maximum Capital Cap (1/15th NAV) per position.
#   - Pads LOD by minimum 0.5% to prevent infinite "Micro-Stop" share sizing.
#   - Corrects Cash deduction to explicitly include commissions.
#   - Expands Top 40 memory optimization to Top 100.
# ==============================================================================
...
class VectorizedSwingEngine:
    ...
    def load_data(self):
        logging.info("Step 1: Fetching Daily Data from Materialized View...")
        query_daily = f"""
        ...
        # V8: Pure Alphabetical Symbols Only (Removes .WS, .U, -A, etc.)
        ...
        # V8: Filter for Minimum Price
        df['price_pass'] = df['d_close_prev'] >= self.min_open_price
        
        self.daily_data = df.set_index(['trading_date', 'symbol']).sort_index()
        
        df_candidates = df[(df['ratio'] >= 5.0) & df['vol_cond'] & df['deadzone_pass'] & df['price_pass']].copy()
        df_candidates.reset_index(inplace=True)
        
        # V8: Expanded to Top 100 Candidates per day
        df_top_cands = df_candidates.sort_values(['trading_date', 'ratio'], ascending=[True, False]).groupby('trading_date').head(100)
        
        logging.info(f"Memory Optimization: Reduced from {len(df_candidates)} to {len(df_top_cands)} hyper-targeted candidate days.")
        ...
        self.trading_dates = sorted(self.daily_data.index.get_level_values(0).unique())
        
        with self.engine.connect() as conn:
            conn.execute(text("DROP TABLE IF EXISTS temp_optuna_candidates;"))
            conn.commit()
            
        logging.info("Data mapped perfectly. Engine ready for Optuna.")
        ...
        for date in self.trading_dates:
            ...
            for sym, pos in open_positions.items():
                if (date, sym) in self.daily_data.index:
                    bod_nav += pos['shares'] * self.daily_data.at[(date, sym), 'd_close_prev']
            daily_nav_curve.append(bod_nav)
            target_risk = bod_nav * self.risk_pct
            max_capital_per_trade = bod_nav / self.max_positions # V8: Max capital cap
            
            closed_syms = []
            for sym, pos in open_positions.items():
                if (date, sym) not in self.daily_data.index: continue
                
                d_O = self.daily_data.at[(date, sym), 'd_open']
                d_H = self.daily_data.at[(date, sym), 'd_high']
                d_L = self.daily_data.at[(date, sym), 'd_low']
                sma_val = self.daily_data.at[(date, sym), p_sma]
            ...
            if date in self.daily_data.index.get_level_values(0):
                ...
                candidates = day_data[(day_data['ratio'] >= p_target) & day_data['vol_cond'] & day_data['deadzone_pass'] & day_data['price_pass']].sort_values('ratio', ascending=False)
                ...
                for sym, row in candidates.iterrows():
                    ...
                    # V8: Max Capital Cap & Affordability Check
