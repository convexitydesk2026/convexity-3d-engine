...
st.markdown("This chart represents a real-world sample of the Volatility Risk Premium (VRP) strategy executed over a 3-month period. By systematically selling insurance (puts) on the S&P 500, we harvest the spread between *Implied Volatility* (market fear) and *Realized Volatility* (actual market movement).")

# Hardcoded Expectancy Chart (Mimicking the User's Screenshot)
col_e1, col_e2, col_e3, col_e4 = st.columns(4)
col_e1.markdown("<div class='info-box' style='text-align:center;'><span style='font-size:12px; color:#64748b; font-weight:bold;'>WIN RATE</span><br><span style='font-size:24px; font-weight:900; color:#16a34a;'>88.9%</span></div>", unsafe_allow_html=True)
col_e2.markdown("<div class='info-box' style='text-align:center;'><span style='font-size:12px; color:#64748b; font-weight:bold;'>AVERAGE WIN VS LOSS</span><br><span style='font-size:16px; font-weight:bold; color:#16a34a;'>+$1,480</span> <span style='color:#64748b;'>/</span> <span style='font-size:16px; font-weight:bold; color:#dc2626;'>-$910</span></div>", unsafe_allow_html=True)
col_e3.markdown("<div class='info-box' style='text-align:center;'><span style='font-size:12px; color:#64748b; font-weight:bold;'>PROFIT FACTOR</span><br><span style='font-size:24px; font-weight:900; color:#16a34a;'>13.02</span></div>", unsafe_allow_html=True)
col_e4.markdown("<div class='info-box' style='text-align:center; border-bottom: 3px solid #16a34a;'><span style='font-size:12px; color:#64748b; font-weight:bold;'>MATH EXPECTANCY ⓘ</span><br><span style='font-size:24px; font-weight:900; color:#16a34a;'>$1,214.36</span></div>", unsafe_allow_html=True)

# Dummy data to recreate the bubble chart
np.random.seed(42)
dates = pd.date_range(start="2026-04-01", end="2026-07-12", periods=15)
roc = np.random.uniform(4, 10, 13).tolist() + [-1.5, -5.0] # 13 wins, 2 losses
days_held = np.random.randint(10, 45, 15)
abs_pnl = np.abs(np.array(roc) * 200) # Proxy for bubble size

fig_exp = go.Figure(go.Scatter(
    x=dates, y=roc, mode='markers',
    marker=dict(
        size=abs_pnl, sizemode='area', sizeref=2.*max(abs_pnl)/(40.**2), sizemin=8,
        color=days_held, colorscale='RdYlBu', showscale=True, colorbar=dict(title="Holding Time (Days)"),
        line=dict(width=1.5, color='black')
    )
))
fig_exp.add_hline(y=0, line_color="black", line_width=2)
fig_exp.update_layout(title="Post-Mortem: Behavioral Bubble Chart (Size = Abs PnL | Color = Days in Trade)", yaxis_title="Return on Capital (ROC) %", height=400, plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=40, b=0))
