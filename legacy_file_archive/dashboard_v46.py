...
3. Launch the UI by running exactly:
   streamlit run dashboard_v46.py

CHANGELOG (v46):
         - PHASE 16 (Champion Analytics): Separated Tags from Notes, safely migrated database schema.
         - ADDED: Multi-select Expectancy Filter for edge-isolation in the Champion Calculator.
=============================================================================
"""
...
if not champ_hist_df.empty:   
    ...
    # Ensure tags column exists for display (in case of total emptiness)
    if 'Tags' not in display_hist.columns: display_hist['Tags'] = ''
    
    display_hist = display_hist[['Exit Date', 'Symbol', 'Silo', '1R Base ($)', 'Realized PnL ($)', 'R-Multiple', 'Tags', 'Notes']]
