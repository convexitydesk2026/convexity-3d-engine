r"""
=============================================================================
Script Name: Estate_Orchestrator_v22.py
Purpose: The Zero-Touch Automation Engine for the Family Office.
         v22 UPGRADES:
         - Seed & Append Database Architecture (Zero Gaps, Zero Bloat).
         - Eradicated Proxy Math: a1 and a5 pulled directly from MTMP/TRNT.
         - Automated Options Journaling: Velocity metrics calculated via Closed Lots.
         - Single Source of Truth enforcement via Section Code Parsing.
=============================================================================
"""
...
def parse_ibkr_flex(csv_text):
    ...
    return {sec: pd.DataFrame(rows) for sec, rows in data.items() if rows}
...
def process_ibkr_data(parsed_dfs):
    ...
    for sec in ['POST', 'CRTT']:
        if sec in parsed_dfs and not parsed_dfs[sec].empty:
            db_path = os.path.join(TARGET_DIR, f"Local_DB_{sec}.csv")
            parsed_dfs[sec].to_csv(db_path, index=False)
            local_db[sec] = parsed_dfs[sec]
        else:
            db_path = os.path.join(TARGET_DIR, f"Local_DB_{sec}.csv")
            local_db[sec] = pd.read_csv(db_path, dtype=str) if os.path.exists(db_path) else pd.DataFrame()
    ...
    dedup_subsets = {
        ...
        'TRNT': None, # Drop perfect duplicates for trades
    ...
    print("[OK] Local Databases Synced.")
...
if __name__ == "__main__":
    print("========================================================")
    print("     ESTATE ORCHESTRATOR v22 - INITIALIZING SYNC...      ")
    print("========================================================")
    create_backup()    
    update_ibkr_api_or_fallback()
    update_yfinance_data()
    print("\n3. Generating Dashboard HTML...")
    os.system('python Generate_Estate_Dashboard_v66.py')
    html_path = os.path.join(TARGET_DIR, "Family_Estate_Dashboard_v66.html")
