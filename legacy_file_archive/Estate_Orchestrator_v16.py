r"""
=============================================================================
Script Name: Estate_Orchestrator_v16.py
...
def process_ibkr_data(sections):
    ...
    # v16 FIX: Added .copy() to silence SettingWithCopyWarning
    if pos_df is not None and 'ReportDate' in pos_df.columns:
        pos_df = pos_df[pos_df['ReportDate'] == pos_df['ReportDate'].max()].copy()
    ...
    if cash_df is not None and 'ReportDate' in cash_df.columns:
        cash_df = cash_df[cash_df['ReportDate'] == cash_df['ReportDate'].max()].copy()
    elif cash_df is not None:
    ...
    # v16 CRITICAL FIX: Isolate final day to prevent cumulative inflation
    if pnl_df is not None and 'ReportDate' in pnl_df.columns:
        pnl_df = pnl_df[pnl_df['ReportDate'] == pnl_df['ReportDate'].max()].copy()
    elif pnl_df is not None:
        pnl_df = pnl_df.copy()
    ...
    # v16: Dump directly. "Last 365 Days" Flex Query natively handles accumulation.
    with open(os.path.join(TARGET_DIR, 'PnL_Attribution.json'), 'w') as f:
        json.dump(current_attr, f)
...
if __name__ == "__main__":
    ...
    os.system('python Generate_Estate_Dashboard_v64.py')
    
    html_path = os.path.join(TARGET_DIR, "Family_Estate_Dashboard_v64.html")
