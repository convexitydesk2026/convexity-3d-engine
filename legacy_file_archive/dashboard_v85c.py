...
3. Launch the UI by running exactly:
   streamlit run dashboard_v85c.py

CHANGELOG (v85c):
         - ENHANCEMENT: Added Capital Discipline Tracker (Historical Margin vs VIX overlay) to Section 6.
=============================================================================
"""
...
def process_metrics(df_acc, rf_rate):
    ...
    # v85 Absolute Truth Math: Start NAV is inherently 0
...
def load_and_process_data():
    ...
    try:
        ...
        # v85 FIX: Robust NAV Forward Fill (Solves the Phantom Volatility Jump)
    ...
    # Global tracking arrays (v85: Pure Absolute Math across all Silos. Kills the Whipsaw)
...
if not journal_raw_df.empty and not global_df.empty and not bench_df.empty:
    ...
    fig_disc.update_layout(
        ...
        yaxis=dict(title='Global Options Margin (%)', range=[0, 30], gridcolor='LightGray', zerolinecolor='black', titlefont=dict(color='purple'), tickfont=dict(color='purple')),
        yaxis2=dict(title='^VIX Level', overlaying='y', side='right', range=[0, max_vix], showgrid=False, titlefont=dict(color='black'), tickfont=dict(color='black')),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
    )
...
if not journal_raw_df.empty:
    ...
    with tab5:
        ...
        if not closed_df.empty:
            closed_df['Total P&L (USD)'] = pd.to_numeric(closed_df['Total P&L (USD)'], errors='coerce').fillna(0)
            wins = closed_df[closed_df['Total P&L (USD)'] > 0]
            losses = closed_df[closed_df['Total P&L (USD)'] <= 0]
