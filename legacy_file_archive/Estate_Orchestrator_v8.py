r"""
=============================================================================
Script Name: Estate_Orchestrator_v8.py
File Path: C:\Users\donca\Desktop\Desktop HP Envy x360 al 22Abr24\Docs Manuel\IBKR_Options\Estate_Orchestrator_v8.py
Purpose: The Zero-Touch Automation Engine for the Family Office.
         V8 UPGRADES:
         - Bulletproof "Dictionary Mapping" Parser: Completely immunizes the 
           script against "Jagged CSVs" (where IBKR UK accounts have 6 columns 
           but US accounts have 5). 
         - Added pd.to_numeric coercions to prevent string-to-float crashes.
...
def parse_ibkr_universal(csv_text):
    """ v8 CHANGE: Dictionary-based parser to perfectly handle jagged, multi-schema CSVs """
    lines = csv_text.strip().split('\n')
    nav_list, pos_list, cash_list = [], [],[]
    ...
    for line in lines:
        ...
        # 1. API FORMAT (Starts with section name)
        if parts[0] in['Change in NAV', 'Open Positions', 'Cash Report']:
            if len(parts) > 1 and parts[1] == 'Header':
        ...
        # Parse Data using active headers to create flexible dictionaries
    ...
    # Pandas magically handles dictionaries with differing keys by filling NaNs
    nav_df = pd.DataFrame(nav_list) if nav_list else None
    pos_df = pd.DataFrame(pos_list) if pos_list else None
    cash_df = pd.DataFrame(cash_list) if cash_list else None
...
def process_ibkr_data(sections):
    ...
    if nav_df is not None:
        ...
        # v8 CHANGE: Safe string conversion before replacing 'F'
        nav_df['AccountID'] = nav_df[acc_col].astype(str).str.replace('F', '', regex=False)
        
        # v8 CHANGE: Coerce to numeric safely to prevent empty string crashes
        nav_df['NAV'] = pd.to_numeric(nav_df[end_val_col], errors='coerce').fillna(0)
    ...
    # ---------------------------------------------------------
    # 2. Extract Live Allocations for Panel 2 Auto-Fill
    # ---------------------------------------------------------
    live_pos = {
    ...
    if pos_df is not None:
        ...
        for _, row in pos_df.iterrows():
            raw_acc = str(row.get(pos_acc_col, ''))
            if raw_acc not in acc_map: continue
            
            silo = acc_map[raw_acc]
            sym = str(row.get('Symbol', '')).upper()
            asset = str(row.get('AssetClass', ''))
            curr = str(row.get('CurrencyPrimary', row.get('Currency', 'USD')))
            ...
            # v8 CHANGE: Safe float extraction
            try: val = float(row.get(val_col, 0))
            except: val = 0.0
            
            fx_str = row.get(fx_col, '')
            try: fx = float(fx_str) if fx_str != '' else 1.0
            except: fx = 1.0
    ...
    if cash_df is not None:
        ...
        for _, row in cash_df.iterrows():
            raw_acc = str(row.get(cash_acc_col, ''))
            curr = str(row.get('CurrencyPrimary', row.get('Currency', '')))
            
            if raw_acc in acc_map and curr == 'BASE_SUMMARY':
                silo = acc_map[raw_acc]
                end_cash_col = 'EndingCash' if 'EndingCash' in row else 'Ending Cash'
                try:
                    cash_val = float(row.get(end_cash_col, 0))
                except:
                    cash_val = 0.0
...
if __name__ == "__main__":
    print("========================================================")
    print("     ESTATE ORCHESTRATOR v8 - INITIALIZING SYNC...      ")
    print("========================================================")
