r"""
=============================================================================
Script Name: Estate_Orchestrator_v9.py
File Path: C:\Users\donca\Desktop\Desktop HP Envy x360 al 22Abr24\Docs Manuel\IBKR_Options\Estate_Orchestrator_v9.py
Purpose: The Zero-Touch Automation Engine for the Family Office.
         V9 UPGRADES:
         - The "26 Trillion %" Ghost Buster: Groupby changed from `.sum()` 
           to `.agg({'NAV':'last', 'CashFlow':'sum'})` to correctly handle 
           multiple IBKR currency lines without doubling NAV.
         - The Margin Automator: Extracts Option Quantities from the open 
           positions list and mathematically deduces Opt Margin for Panel 2.
         - Bulletproof Header Parser: `get_col` prevents KeyErrors from IBKR 
           API column name shifts.
...
def get_col(df, possible_names):
    """ v9 CHANGE: Resilient column finder to prevent KeyErrors """
...
def process_ibkr_data(sections):
    # ---------------------------------------------------------
    # 1. Update Daily NAV (v9 FIX: .agg instead of .sum)
    # ---------------------------------------------------------
    ...
    if nav_df is not None:
        ...
        # v9 CHANGE: Used .agg('last', 'sum') to prevent duplicate line explosions!
    ...
    # ---------------------------------------------------------
    # 2. Extract Live Allocations & Opt Margin (v9 CHANGE)
    # ---------------------------------------------------------
    live_pos = {
        ...
        for _, row in pos_df.iterrows():
            ...
            if silo == 'D':
                ...
                if 'IB01' in sym: live_pos[silo]['IB01'] += usd_val
                    ...
                    # v9 CHANGE: Automate Opt Margin based on negative quantity
                    if qty < 0:
...
def update_yfinance_data():
    ...
    try:
        ...
        spy_open, spy_close = data['Open']['SPY'], data['Close']['SPY']
        ...
        for date, row in close_data.iterrows():
            if date.tz_localize(None) > max_date.tz_localize(None):
                ...
                if s10 > s20:
                    ...
                    tor = 5 if vix_price < 15 else (4 if vix_price <= 20 else 3)
                elif s10 < s20:
                    status, min_tor, max_tor = 'Red', 0, 1
                    tor = 0 if vix_price > 25 else 1
                else:
                    status, min_tor, max_tor, tor = 'Yellow', 1, 3, 2
...
if __name__ == "__main__":
    print("========================================================")
    print("     ESTATE ORCHESTRATOR v9 - INITIALIZING SYNC...      ")
    print("========================================================")
    ...
    os.system('python Generate_Estate_Dashboard_v52.py')
    
    html_path = os.path.join(TARGET_DIR, "Family_Estate_Dashboard_v52.html")
