r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v45.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 45 UPGRADES:
         - Added Jensen's Alpha and Pearson Correlation (Estate vs SPY).
         - Expanded the Master Aggregation UI header to 10 items (grid-cols-5).
         - Hardcoded the v44 Options Liability reconciliation patch into the JS.
         - All previous features perfectly preserved (Risk Radars, 13F Silo D, 
           Regime Ribbon, Monte Carlo auto-scaling).
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v45.html") # v45 CHANGE: Name updated
...
# v45 CHANGE: Initialize Alpha and Correlation strings
alpha_str = "N/A"
corr_str = "N/A"

print("\n========================================================")
print("             CIO DIAGNOSTICS & LOGGING (v45)            ") # v45 CHANGE
print("========================================================")
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        # v45 CHANGE: Calculate Alpha and Correlation mathematically
        if len(master_bench) > 1 and master_bench['SPY_Ret'].std() > 0:
            corr = master_bench['Estate_Ret'].corr(master_bench['SPY_Ret'])
            cov = master_bench['Estate_Ret'].cov(master_bench['SPY_Ret'])
            var_spy = master_bench['SPY_Ret'].var()
            beta = cov / var_spy if var_spy > 0 else 0
            
            rf = 0.045 # 4.5% IB01 Risk-Free Rate
            est_ann_ret = master_bench['Estate_Ret'].mean() * 252
            spy_ann_ret = master_bench['SPY_Ret'].mean() * 252
            
            # Jensen's Alpha Formula
            alpha = est_ann_ret - (rf + beta * (spy_ann_ret - rf))
...
<head>
    ...
    <title>Estate Master Dashboard v45</title> <!-- v45 CHANGE -->
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v45</p> <!-- v45 CHANGE -->
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <div class="xl:col-span-6 flex flex-col gap-6">
                ...
                <div class="bg-gray-100 p-4 rounded-xl shadow-md border border-gray-300 relative">
                    ...
                    <!-- v45 CHANGE: Expanded grid to md:grid-cols-5 (10 items total) to fit Alpha and Correlation seamlessly -->
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm mt-3 border-t border-gray-300 pt-3 font-mono text-center">
                        ...
                        <!-- v45 CHANGE: Added Alpha and Correlation HTML boxes -->
                        <div>SPY Alpha<br><span class="VAR_COLOR_ESTATE_ALPHA font-black text-lg">VAR_ESTATE_ALPHA</span></div>
                        <div>SPY Corr.<br><span class="text-indigo-700 font-black text-lg">VAR_ESTATE_CORR</span></div>
                    </div>
                </div>
    ...
    <script>
        ...
        function updateUI() {
            ...
            // v45 CHANGE: Hardcoded the v44 reconciliation fix for Silo A
            ...
            // v45 CHANGE: Hardcoded the v44 reconciliation fix for Silo C
...
# v45 CHANGE: Calculate Alpha and Correlation Strings, default to N/A if missing
alpha_str = "N/A"
corr_str = "N/A"
...
if len(global_df) > 1 and len(bench_df) > 1 and 'SPY_Ret' in master_bench.columns:
    if master_bench['SPY_Ret'].std() > 0:
        ...
        # 2. Jensen's Alpha (Assumes 4.5% Rf from IB01)
        cov = master_bench['Estate_Ret'].cov(master_bench['SPY_Ret'])
        var_spy = master_bench['SPY_Ret'].var()
        beta = cov / var_spy if var_spy > 0 else 0
        ...
        # Format HTML Output
        alpha_color = "text-green-700" if alpha > 0 else "text-red-600"
        alpha_str = f'<span class="{alpha_color} font-black text-lg">{alpha * 100:.2f}%</span>'
        
        # Correlation color: lower is better for diversification (Green < 0.5 < Indigo < 0.8 < Red)
        corr_color = "text-green-700" if corr < 0.5 else ("text-indigo-700" if corr < 0.8 else "text-red-600")
        corr_str = f'<span class="{corr_color} font-black text-lg">{corr:.2f}</span>'

# Inject into HTML
html_content = html_content.replace("VAR_ESTATE_ALPHA", alpha_str)
html_content = html_content.replace("VAR_ESTATE_CORR", corr_str)
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v45 created.")
