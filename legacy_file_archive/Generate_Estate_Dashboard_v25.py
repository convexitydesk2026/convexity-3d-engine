r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v25.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 25 UPGRADES: 
         - BUG FIX: Implemented safe_parse JSON pipeline to completely eliminate 
           JavaScript Syntax Errors caused by pandas NaN/Infinity outputs.
         - Removed cluttered text from Daily PnL histogram.
         - Overlaid SPY/QQQ Cumulative PnL lines on right-side secondary axis.
         - Added Verbose Command Prompt Diagnostics.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v25.html")
...
# SAFE JSON EXPORTER: Eradicates all NaN and Infinity bugs for Javascript
def to_safe_json(data):
    if isinstance(data, pd.Series):
        clean_data =[None if pd.isna(x) else x for x in data.tolist()]
    else:
        clean_data =[None if pd.isna(x) else x for x in data]
...
if os.path.exists(csv_file_path):
    try:
        ...
        if not pnl_df.empty:
            ...
            dates_pnl_js = to_safe_json(pnl_pivot.index.strftime('%Y-%m-%d'))
            silo_a_pnl = to_safe_json(pnl_pivot['U23144948']) if 'U23144948' in pnl_pivot else "[]"
            silo_b_pnl = to_safe_json(pnl_pivot['U23139264']) if 'U23139264' in pnl_pivot else "[]"
            silo_c_pnl = to_safe_json(pnl_pivot['U23154199']) if 'U23154199' in pnl_pivot else "[]"
            silo_d_pnl = to_safe_json(pnl_pivot['U25218481']) if 'U25218481' in pnl_pivot else "[]"
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        # Safely extract all arrays to JSON
        dates_js = to_safe_json(master_bench['Date'].dt.strftime('%Y-%m-%d'))
        spy_js = to_safe_json(master_bench['SPY_Cum'] * 100)
        qqq_js = to_safe_json(master_bench['QQQ_Cum'] * 100)
        estate_js = to_safe_json(master_bench['Estate_Cum'] * 100)
        
        silo_a_js = to_safe_json(master_bench['U23144948_Cum'] * 100)
        silo_b_js = to_safe_json(master_bench['U23139264_Cum'] * 100)
        silo_c_js = to_safe_json(master_bench['U23154199_Cum'] * 100)
        silo_d_js = to_safe_json(master_bench['U25218481_Cum'] * 100)

        dates_cum_pnl_js = to_safe_json(master_bench['Date'].dt.strftime('%Y-%m-%d'))
        estate_cum_pnl_js = to_safe_json(master_bench['Estate_Cum_PnL'])
        spy_cum_pnl_js = to_safe_json(master_bench['SPY_Cum_PnL'])
        qqq_cum_pnl_js = to_safe_json(master_bench['QQQ_Cum_PnL'])
...
<head>
    ...
    <title>Family Estate Master Allocator v25</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v25</p>
        </div>
    ...
    <script>
        ...
        // Safe Parse JSON wrapper to prevent Javascript Syntax Error crashes
        ...
        function updateUI() {
            ...
            if (benchDates.length > 0) {
                // Global Chart
                let traceEstate = { x: benchDates, y: estateData, name: 'Estate (Cum %)', type: 'scatter', mode: 'lines', line: {color: 'black', width: 4} };
                let traceSPY = { x: benchDates, y: spyData, name: 'SPY (Cum %)', type: 'scatter', mode: 'lines', line: {color: '#3b82f6', width: 2} }; 
                let traceQQQ = { x: benchDates, y: qqqData, name: 'QQQ (Cum %)', type: 'scatter', mode: 'lines', line: {color: '#f97316', width: 2} }; 
            ...
            if (datesPnl.length > 0) {
                let traceA = { x: datesPnl, y: pnlA, name: 'Silo A', type: 'bar', marker: {color: '#1d4ed8'} };
                let traceB = { x: datesPnl, y: pnlB, name: 'Silo B', type: 'bar', marker: {color: '#7e22ce'} };
                let traceC = { x: datesPnl, y: pnlC, name: 'Silo C', type: 'bar', marker: {color: '#15803d'} };
                let traceD = { x: datesPnl, y: pnlD, name: 'Silo D', type: 'bar', marker: {color: '#facc15'} };

                let traceEstateLine = { x: datesCumPnl, y: estateCumPnl, name: 'Estate (Cum PnL USD)', type: 'scatter', mode: 'lines', line: {color: 'black', width: 3}, yaxis: 'y2' };
                ...
                let traceQQQLine = { x: datesCumPnl, y: qqqCumPnl, name: 'QQQ (Cum PnL USD)', type: 'scatter', mode: 'lines', line: {color: '#f97316', width: 2}, yaxis: 'y2' };
                ...
                let pnlLayout = {
                    ...
                    yaxis: { title: 'Daily PnL (USD)', side: 'left', zeroline: true, zerolinecolor: '#000000', zerolinewidth: 1 },
                    yaxis2: { title: 'Cumulative PnL (USD)', side: 'right', overlaying: 'y', showgrid: false, zeroline: false },
                    legend: { orientation: 'h', y: 1.1, x: 0.5, xanchor: 'center' }
                };
                Plotly.react('daily-pnl-chart',[traceA, traceB, traceC, traceD, traceEstateLine, traceSPYLine, traceQQQLine], pnlLayout, {responsive: true});
            }
        }
...
html_content = html_content.replace("VAR_BENCH_DATES", dates_js)
html_content = html_content.replace("VAR_ESTATE_CUM", estate_js)
html_content = html_content.replace("VAR_SPY_CUM", spy_js)
html_content = html_content.replace("VAR_QQQ_CUM", qqq_js)

html_content = html_content.replace("VAR_SILOA_CUM", silo_a_js)
html_content = html_content.replace("VAR_SILOB_CUM", silo_b_js)
html_content = html_content.replace("VAR_SILOC_CUM", silo_c_js)
html_content = html_content.replace("VAR_SILOD_CUM", silo_d_js)

html_content = html_content.replace("VAR_DATES_PNL", dates_pnl_js)
html_content = html_content.replace("VAR_SILOA_PNL", silo_a_pnl)
html_content = html_content.replace("VAR_SILOB_PNL", silo_b_pnl)
html_content = html_content.replace("VAR_SILOC_PNL", silo_c_pnl)
html_content = html_content.replace("VAR_SILOD_PNL", silo_d_pnl)

html_content = html_content.replace("VAR_DATES_CUM_PNL", dates_cum_pnl_js)
html_content = html_content.replace("VAR_ESTATE_CUM_PNL", estate_cum_pnl_js)
html_content = html_content.replace("VAR_SPY_CUM_PNL", spy_cum_pnl_js)
html_content = html_content.replace("VAR_QQQ_CUM_PNL", qqq_cum_pnl_js)
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v25 created.")
