"""
=============================================================================
Script Name: dashboard_v29.py
Purpose: The Streamlit Frontend (Module 20 - Institutional Aesthetics & Tail Hedge).
         - Monte Carlo: Added b4 (Blue/Gray) transparency, increased primary 
           line thicknesses by 50%, forced gridlines to absolute top layer.
         - Options Ledger: Explicitly multiplied ROC by 100 to fix Excel mismatch.
         - Typography: Standardized Master Estate Aggregation fonts to 28px/900w.
         - Script Discovery: Intelligently detects active filename & strips noisy tags.
         - Tail Hedge: Mechanized rules (90-120 DTE), bucketed into its own Asset Class,
           and implemented Budget vs Deployed accounting in Velocity Panel.
=============================================================================
"""
...
def get_active_scripts():
    ...
    for p in patterns:
        ...
        if matches:
            # FIX: Grab the file with the most recent Windows modification time
            latest = max(matches, key=os.path.getmtime)
            scripts.append(os.path.basename(latest))
        else:
            scripts.append(p.replace('_v*.py', '_vX.py')) # Fallback placeholder
    return " • ".join(scripts)
...
if not journal_raw_df.empty:
    ...
    for _, row in journal_raw_df[pd.isnull(journal_raw_df['Close Date'])].iterrows():
        try:
            ...
            # --- NEW: 200% Premium Stop Loss Monitor ---
            ...
            if dte_rem > 0 and str(dte_rem) != 'Closed':
                ...
                # Stop Loss Threshold is 2x the initial premium collected
...
# 2. SMA Warnings & Tail Hedge Trigger
...
# 3. Regime, Cash Drag & Tail Hedge Alert
...
# NEW: 9th Alert (Constant Tail Hedge Armor & Laddering Schedule)
# We do not time the market (VIX/Regimes). We maintain constant, rolling armor.
# A standard 120-DTE deep OTM SPY put (Delta < 5) costs roughly $400.
hedge_unit_cost = 400.0

if th_available >= hedge_unit_cost:
    # Calculate how many tranches/rungs of the ladder you can afford
    tranches_due = int(th_available // hedge_unit_cost)
    
    if tranches_due == 1:
        alerts.append(f"🛡️ **Tail Hedge Deployment Due:** You have **${th_available:,.0f}** available in house money. Purchase **1 tranche** (~$400) of 120-DTE deep OTM S&P 500 Puts (Delta < 5) to add a new rung to your insurance ladder.")
    else:
        alerts.append(f"🛡️ **Tail Hedge Backlog Detected:** You have **${th_available:,.0f}** available. DO NOT deploy as a lump sum. Split this into **{tranches_due} staggered tranches** (~$400 each) across 90, 120, and 150+ DTE to build a continuous Black Swan ladder.")
...
if not attr_df.empty:
    ...
    th_df_vel = pos_df[pos_df['asset_class'] == 'Tail Hedge']
    ...
    with col_vel:
        ...
        st.metric("Tail Hedge Deployed", f"${th_deployed:,.0f}")
...
with c_txt:
    ...
    if regime_str == 'Green':
        ...
        st.write("<span style='font-size: 12px;'>**Options (Silos A & C):** Clear to sell new XSP/XND Put Spreads up to 20% margin cap. Monitor Tail Hedge Budget.</span>", unsafe_allow_html=True)
        ...
        st.write("<span style='font-size: 12px;'>**Options (Silos A & C):** Ensure 90-120 DTE Tail Hedges are deployed to protect physical assets.</span>", unsafe_allow_html=True)
...
if sim_length > 0:
    ...
    with col_mc_chart:
        ...
        # 1. Plot Spaghettis first so they are placed at the absolute bottom layer
        for i in range(200):
        ...
        # 2. Plot Primary Traces on top with 50% increased thickness
...
if not journal_raw_df.empty:
    ...
    # FIX: Multiply by 100 to correct Streamlit formatter string misalignment with raw Excel computation.
...
if not journal_raw_df.empty:
    ...
    # For comparison, drop the calculated fields as they are volatile based on today's date
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            # --- NEW: Volatility (VIX) Stress Tester ---
            with col_2d:
            ...
            with col_3d:
                ...
                fig_3d.add_trace(go.Surface(
                    ...
                    colorscale=[[0, 'green'],[1, 'green']], opacity=0.2, showscale=False, hoverinfo='skip'
                ))
                fig_3d.add_trace(go.Surface(
                    x=[[K_l, K_l],[K_l, K_l]], y=[[0, init_dte],[0, init_dte]], z=[[z_min, z_min],[z_max, z_max]],
                    colorscale=[[0, 'red'],[1, 'red']], opacity=0.2, showscale=False, hoverinfo='skip'
                ))
                ...
                fig_3d.add_trace(go.Surface(
                    ...
                    colorscale=[[0, 'yellow'],[1, 'yellow']], opacity=0.25, showscale=False, hoverinfo='skip'
                ))
                ...
                fig_3d.add_trace(go.Surface(
                    ...
                    z=[[0, 0], [0, 0]],
                    colorscale=[[0, 'gray'],[1, 'gray']], opacity=0.4, showscale=False, hoverinfo='skip'
                ))
                ...
                # --- v29 UX FIX: Legend Orientation & 180-Degree Camera Rotation ---
                custom_camera = dict(
                    eye=dict(x=-1.25, y=-1.25, z=1.25) # Rotates the default camera 180 degrees horizontally
                )
