"""
=============================================================================
Script Name: attribution_engine_v15.py
...
def get_bucket(sym, sec_type, pos, record_date):
    ...
    # 2. Handle True Yield (Treasuries & USD Cash)
    if 'IB01' in s:
        return 'a1_yield'
        
    # 3. Segregate Foreign Exchange Noise from Yield
    if sec_type == 'CASH':
        if 'USD' in s:
            return 'a1_yield'  # Pure USD Cash balance
        else:
            return 'a4_alpha'  # Foreign Currency Cash (EUR, GBP, SEK). Send FX PnL to Alpha.
...
def run_attribution():
    print("========================================================")
    print("   ESTATE P&L ATTRIBUTION ENGINE (v15 WAL MODE)")
