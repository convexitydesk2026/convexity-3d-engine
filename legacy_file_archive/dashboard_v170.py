...
streamlit run dashboard_v170.py
CHANGELOG (v170):
- STAGE 77 (v170): Fixed Plotly rendering bug in Section 3B. Synchronized Stop-Loss polarity and bar stacking logic for Short positions.
- STAGE 76 (v169): Injected hardcoded Macroeconomic Catalysts (FOMC & CPI) into the Unified Estate Calendar for zero-latency rendering.
- STAGE 75 (v168): Built the Unified Estate Calendar & HUD. Tracks Earnings, 45-DTE Rolls, 21-DTE Ejects, Expirations, and Day-10 Time Stops.
- STAGE 74 (v167): Hardened the 3D Topography Tranche ID string-matching logic to be bulletproof against spacing and capitalization variations.
- STAGE 73 (v166): Added Market Capitalization and Average Daily Volume to the C2 Pitch Dossier Generator (Zero-cost API extraction).
- STAGE 72 (v165): Silenced Streamlit deprecation warnings by migrating legacy `use_container_width` arguments to `width="stretch"`.
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v36.py")
...
with st.sidebar:
    ...
    st.markdown("""
    - [🔝 Top / Alerts](#top)
    - [0. Unified Estate Calendar](#sec0)
...
if not df_alpha.empty:
    ...
    for _, r in df_alpha.iterrows():
        ...
        m = r['Market Value']
        if r['Shares'] > 0:
            base_vals.append(min(c, m))
            green_tops.append(max(0, m - c))
            red_tops.append(max(0, c - m))
        else:
            # For Short positions, Plotly stacks negative values downwards.
            base_vals.append(max(c, m))
            green_tops.append(min(0, c - m))
            red_tops.append(min(0, m - c))
    ...
    for i, r in df_alpha.iterrows():
        ...
        if r['Total SL Value'] > 0.01:
            ...
            if r['Shares'] < 0:
                sl_val = -sl_val  # Invert polarity for short positions
                
            fig_alpha.add_shape(type="line", x0=i-0.5, x1=i+0.4, y0=sl_val, y1=sl_val, line=dict(color="black", width=2))
            fig_alpha.add_trace(go.Scatter(x=[i-0.5], y=[sl_val], mode='markers', marker=dict(color='black', size=10), showlegend=False, hovertemplate=f"Stop Loss Val: ${abs(sl_val):,.0f}<extra></extra>"))
        else:
    ...
    if not active_vrp.empty:
        ...
        for idx, r in active_vrp.iterrows():
            try:
                ...
                acc = 'U23144948' if 'siloa' in tranche else ('U23154199' if 'siloc' in tranche else None)
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            with col_2d:
                ...
                acct_filter = 'U23144948' if 'siloa' in tranche_id else 'U23154199' if 'siloc' in tranche_id else None
