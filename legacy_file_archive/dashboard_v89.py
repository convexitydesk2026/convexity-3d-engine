...
3. Launch the UI by running exactly:
   streamlit run dashboard_v89.py

CHANGELOG (v89):
         - STAGE 3: Deployed the Options Pre-Flight Checklist (Margin Capacity & VIX Guardrail).
         - STAGE 2: Alpha Risk Calculator deployed into the Sidebar.
         - STAGE 1: Deployed Strategy Filtration in Post-Mortem Analytics (Expectancy Tab).
         - MASTER BUILD: Consolidated all v86 enhancements (HUD, Heatmap, Expectancy, Discipline Tracker).
--- CONTEXT AFTER ---
         - BUG FIX: Corrected Plotly 'title_font' syntax deprecation error in Capital Discipline chart.
=============================================================================
"""
...
def process_metrics(df_acc, rf_rate):
    ...
    # v89 Absolute Truth Math: Start NAV is inherently 0
...
def load_and_process_data():
    ...
    try:
        ...
        # v89 FIX: Robust NAV Forward Fill (Solves the Phantom Volatility Jump)
    ...
    # Global tracking arrays (v89: Pure Absolute Math across all Silos. Kills the Whipsaw)
            ...
            if not closed_df.empty:
                ...
                if not filtered_df.empty:
                    ...
                    fig5.add_trace(go.Scatter(
                        x=wins['Close Date'], y=wins['Total P&L (USD)'], mode='markers',
                        marker=dict(color='#bbf7d0', size=14, line=dict(width=1.5, color='#16a34a')),
                        name='Winning Trades',
                        text=wins['Ticker'],
                        customdata=np.column_stack((wins.get('Tranche ID', ''), wins.get('Return on Capital (ROC) %', 0))),
                        hovertemplate="<b>%{text}</b> (%{customdata[0]})<br>Closed: %{x}<br>PnL: $%{y:,.0f}<br>ROC: %{customdata[1]:.1f}%<extra></extra>"
                    ))
                    
                    fig5.add_trace(go.Scatter(
                        x=losses['Close Date'], y=losses['Total P&L (USD)'], mode='markers',
                        marker=dict(color='#fecaca', size=14, line=dict(width=1.5, color='#dc2626')),
                        name='Losing Trades',
                        text=losses['Ticker'],
                        customdata=np.column_stack((losses.get('Tranche ID', ''), losses.get('Return on Capital (ROC) %', 0))),
                        hovertemplate="<b>%{text}</b> (%{customdata[0]})<br>Closed: %{x}<br>PnL: $%{y:,.0f}<br>ROC: %{customdata[1]:.1f}%<extra></extra>"
                    ))
                    ...
                    fig5.update_layout(
                        title="Post-Mortem: Realized Trade Distribution",
