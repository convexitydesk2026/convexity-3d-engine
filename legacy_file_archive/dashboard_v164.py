...
streamlit run dashboard_v164.py
CHANGELOG (v164):
- STAGE 71 (v164): Built the TradingView Screener Ingestion Panel. Automatically detects TV_Screener.csv and allows 1-click batch importing to the Stalking list.
- STAGE 70 (v163): Built a Global Campaign Tag Editor to safely bypass the Streamlit data_editor multi-select limitation.
- STAGE 69 (v162): Fixed yfinance news parsing bug in the Dossier Generator and injected zero-cost enhancements (Earnings, ATR Stop Distance, Implied R/R, Silo Allocation Breakdown).
- STAGE 68 (v161): Built the C2 Quant Pitch Dossier Generator. Compiles Macro, Technical, Fundamental, and Relative Strength data into a downloadable Markdown (.md) file.
- STAGE 67 (v160): Added a visual legend for the Alpha Engine Gears (0-5) below the Daily PnL chart.
- STAGE 66 (v159): Hardened Privacy Mode to plot pure percentages (preventing starting NAV leaks), hide Options bubbles, and added a Start Date watermark.
- STAGE 65 (v158): Added Privacy Mode toggle to hide Estate/Silo PnL and added RSP to the Daily PnL chart.
- STAGE 64 (v157): Added QQQ to the 10-Day Distribution Tracker and silenced yfinance auto_adjust FutureWarnings.
- STAGE 63 (v156): Built the 10-Day Distribution Tracker (CDD). Injected visual sparklines into the Master Weather Station and programmed 3-day/4-day consecutive red candle alerts.
=============================================================================
"""
...
def get_fundamentals_and_news(symbol):
    """Fetches Forward P/E, Short Interest, and recent news headlines."""
    try:
        ...
        return fwd_pe, short_float, news_str
    except Exception:
        return "N/A", "N/A", "- Error fetching news.\n"
...
privacy_mode = st.toggle("🙈 Privacy Mode (Hide Estate & Silo PnL)", value=False)
...
if not privacy_mode:
    ...
    fig_pnl.add_annotation(x=last_dt, y=rsp_val, text=f"{(rsp_val/global_metrics['nav'])*100:.1f}%<br>${rsp_val:,.0f}", showarrow=False, xanchor='left', yref='y2', bgcolor='#16a34a', font=dict(color='white', size=11))
else:
...
"""
st.markdown(gear_legend_html, unsafe_allow_html=True)
...
with st.expander("🏷️ Global Campaign Tag Editor", expanded=False):
...
if os.path.exists(tv_csv_path):
    with st.expander("📈 TradingView Screener Ingestion", expanded=True):
        try:
            ...
            if ticker_col:
                ...
                with c_imp:
                    if st.button("🔭 Send Selected to Stalk List", use_container_width=True):
                ...
                with c_del:
                    if st.button("🗑️ Delete CSV", use_container_width=True):
...
with st.expander("🔭 View Pre-Trade Staging Area (Pipeline)", expanded=False):
    ...
    if not df_staging.empty:
        ...
        with c_pitch2:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button(f"⚙️ Compile Data for {pitch_sym}", use_container_width=True):
                with st.spinner("Compiling Macro, Technical, and Fundamental data..."):
                    ...
                    # Fundamentals & News
                    fwd_pe, short_float, news_str = get_fundamentals_and_news(pitch_sym)
...
if not df_open.empty:
    ...
    st.markdown("<br><b>Active Campaigns Overview & Global Editor</b>", unsafe_allow_html=True)
