...
streamlit run dashboard_v128.py
CHANGELOG (v128):
...
df_closed = pd.read_sql_query("SELECT id, open_date, close_date, symbol, type, regime_in, sector, industry, entry_price, initial_stop, tags, thesis, days_active, total_pnl, r_multiple, grade FROM alpha_campaigns WHERE status='Closed' ORDER BY close_date DESC", conn_journal)
...
if not df_closed.empty:
    ...
    # Safe string-comparison loop to prevent infinite reload loops caused by Styler formatting
    changed = False
    for i, row in edited_closed.iterrows():
        orig_tags = str(df_closed.at[i, 'tags']) if pd.notna(df_closed.at[i, 'tags']) else ""
        new_tags = str(row['tags']) if pd.notna(row['tags']) else ""
        orig_thesis = str(df_closed.at[i, 'thesis']) if pd.notna(df_closed.at[i, 'thesis']) else ""
        new_thesis = str(row['thesis']) if pd.notna(row['thesis']) else ""
        
        if orig_tags != new_tags or orig_thesis != new_thesis:
            cj.execute("UPDATE alpha_campaigns SET tags=?, thesis=? WHERE id=?", (new_tags, new_thesis, df_closed.at[i, 'id']))
            changed = True
