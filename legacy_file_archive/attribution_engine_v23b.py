"""
=============================================================================
Script Name: attribution_engine_v23b.py
Purpose: Parses the official IBKR Flex Query (MTM and CNAV).
         - V23b FIX: Strict Conid filter in MTMP to eliminate Cash/FX double-counting.
...
def run_attribution():
    print("========================================================")
    print("   ESTATE P&L ATTRIBUTION ENGINE (v23b FULL AUTOMATION)")
        ...
        for row in reader:
            ...
            # -- EXTRACT DATA --
                ...
                # --- MARK-TO-MARKET PERFORMANCE SECTION (MTMP) ---
                    ...
                    conid_str = str(row[mtmp_idx.get('Conid', -1)]).strip()
                    
                    # V23b FIX: Strict Conid Enforcer. 
                    # If IBKR does not assign a Contract ID, it is a subtotal, a summary, or an FX/Cash 
                    # translation row that is already fully captured in the CNAV section. Drop it.
                    if not conid_str:
                        continue
    ...
    # -------------------------------------------------------------------------
    # WRITE CASH FLOWS & ATTRIBUTION (v23b: DYNAMIC STITCHING)
    # -------------------------------------------------------------------------
    ...
    print("[+] SUCCESS: v23b Engine Complete. Infinite Data Stitching Deployed.\n")
