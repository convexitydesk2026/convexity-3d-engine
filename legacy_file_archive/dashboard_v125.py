...
streamlit run dashboard_v125.py
CHANGELOG (v125):
- STAGE 33 (v125): Fixed TradingView Heatmap CSS compression and duplication. Migrated widget from deprecated Streamlit components to a native Base64 iframe injection to silence terminal warnings and enforce absolute height boundaries.
- STAGE 32 (v124): Fixed TradingView Heatmap CSS compression by explicitly injecting 100% height/width parameters into the widget's JSON payload.
- STAGE 31 (v123): Injected zero-latency TradingView HTML5 S&P 500 Heatmap widget into the Institutional Flow section.
- STAGE 30 (v122): Implemented the Edge Analytics (Pivot Tables) module. Dynamically explodes comma-separated tags from closed campaigns to isolate Win Rate, Avg R-Multiple, and PnL per strategy.
- STAGE 29 (v121): Added double backslashes to LaTeX escapes to silence Python 3.12+ SyntaxWarnings in the terminal.
- STAGE 28 (v120): Escaped LaTeX math-formatting triggers inside the Scale-In calculator info boxes.
- STAGE 27 (v119): Upgraded Risk Calculator to permit Tranche Accumulation (Price Discovery). Removed the hard block on lower entries, replacing it with strict blended-risk mathematics to enforce the 1R macro limit. The script now natively scrapes the true live stop-loss from the exchange to prevent manipulation.
- STAGE 26 (v118): Fixed R-Multiple math anomaly. Upgraded the max_shares SQL query with a 30-day lookback window to accurately capture peak position size for campaigns logged post-entry.
- STAGE 25 (v117): Expanded UI Calendar to display daily settled trade counts directly from the clearinghouse SQLite ledger.
- STAGE 24 (v116): Abolished live TWS API fallback for PnL grading to strictly enforce T+1 Flex Query clearinghouse logic. Fixed global share aggregation for accurate R-Multiple risk base.
- STAGE 23 (v115): Implemented T+1 Settlement Delay Engine. Campaigns now enter 'Pending Settlement ⏳' if clearinghouse PnL hasn't arrived. Fixed multi-silo share aggregation for campaign exits.
- STAGE 22 (v114): Enforced strict data-hygiene on institutional tags (No spaces allowed). Updated core tag 'ATR Ext' to 'ATR-Ext'.
- STAGE 21 (v113): Upgraded the CFO Tag Dictionary into a dynamic, self-learning ecosystem. It now auto-extracts custom tags from the SQLite database, merging them with the core list.
=============================================================================
"""
...
if not df_open.empty:
    # 1. Establish the Core Institutional Tags
    core_tags = {"13F", "VCP", "ALCC", "Q-EP", "20SMA", "50SMA", "200SMA", "M-FLOW", "ATR-Ext"}
    
    # 2. Dynamically Extract all Custom Tags from the SQLite Database (Open and Closed Campaigns)
    ...
    for row in db_tags_rows:
        ...
        if raw_tag_str:
            # Split by comma, strip blank spaces, and add to the unique set
            split_tags = [t.strip() for t in raw_tag_str.split(',')]
            for t in split_tags:
                if t: custom_tags.add(t)
                
    # 3. Merge Core and Custom tags, then sort them alphabetically for a clean UI
    all_tags = sorted(list(core_tags.union(custom_tags)))
    
    st.markdown(f"<div style='font-size: 13px; color: #475569; margin-bottom: 12px; background-color: #f8fafc; padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1;'><b>📚 CFO Dynamic Tag Dictionary:</b> <code>{', '.join(all_tags)}</code><br><i>(Type your tags into the cell below separated by commas. The system will automatically learn and memorize new tags.)</i></div>", unsafe_allow_html=True)
    
    edited_open = st.data_editor(
        df_open,
        column_config={
            "id": None,
            "open_date": st.column_config.TextColumn("Date", disabled=True),
            "symbol": st.column_config.TextColumn("Symbol", disabled=True),
            "status": st.column_config.TextColumn("Status", disabled=True),
            "type": st.column_config.TextColumn("Type", disabled=True),
            "regime_in": st.column_config.TextColumn("Regime", disabled=True),
            ...
            "initial_stop": st.column_config.NumberColumn("Initial Stop", format="%.2f", min_value=0.0),
            "tags": st.column_config.TextColumn("Tags (Comma-Separated)"),
            "thesis": st.column_config.TextColumn("Thesis / Comment"),
            "days_active": st.column_config.NumberColumn("Days", disabled=True)
        },
        disabled=["id", "open_date", "symbol", "status", "type", "regime_in", "sector", "industry", "sma_20", "sma_50", "sma_200", "entry_price", "days_active"],
        hide_index=True,
        key="open_camp_editor",
        width="stretch"
    )
    
    if not edited_open.equals(df_open):
        for i, row in edited_open.iterrows():
            cj.execute("""
                UPDATE alpha_campaigns 
                SET initial_stop=?, tags=?, thesis=?
                WHERE id=?
            """, (row['initial_stop'], row['tags'], row['thesis'], row['id']))
        conn_journal.commit()
        st.rerun()
else:
...
df_closed = pd.read_sql_query("SELECT open_date, close_date, symbol, type, regime_in, sector, industry, entry_price, initial_stop, tags, thesis, days_active, total_pnl, r_multiple, grade FROM alpha_campaigns WHERE status='Closed' ORDER BY close_date DESC", conn_journal)
...
if not df_closed.empty:
    ...
    st.dataframe(styled_closed, hide_index=True, width="stretch")
