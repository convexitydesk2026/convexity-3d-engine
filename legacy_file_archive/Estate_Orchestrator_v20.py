r"""
=============================================================================
Script Name: Estate_Orchestrator_v20.py
...
def process_ibkr_data(sections):
    ...
    if cash_df is not None:
        ...
        comm_col = get_col(cash_df,['Commissions', 'Commission'])
        curr_col = get_col(cash_df, ['CurrencyPrimary', 'Currency'])
        if cash_date_col and comm_col and curr_col:
    ...
    if pnl_df is not None:
        ...
        val_col = get_col(pnl_df,['Total Realized and Unrealized P/L', 'Total Realized and Unrealized P/L (Base)', 'TotalFifoPnl', 'TotalMTM', 'FifoPnl'])
        if pnl_date_col and sym_col and val_col:
    ...
    if not combined.empty:
        ...
        for dt in dates:
            ...
            # Use real daily mapping if it exists (for future 1-day pulls), otherwise use theoretical proxy for 365-day backfill
            if dt in daily_a1_map and dt == current_pull_date:
                a1_yield = daily_a1_map[dt]
            else:
                # Theoretical IB01 Yield Backfill (~4.5% annual on roughly 80% of Silo A & C)
                prev_nav_a = prev_b_data['NAV'].sum() if 'prev_b_data' in locals() else 0 # Placeholder for safety
                # We extract the actual A and C NAVs
                silo_a_nav = combined[(combined['Date'] == prev_dt) & (combined['AccountID'] == 'U23144948')]['NAV'].sum()
                silo_c_nav = combined[(combined['Date'] == prev_dt) & (combined['AccountID'] == 'U23154199')]['NAV'].sum()
                est_ib01 = (silo_a_nav + silo_c_nav) * 0.80
                a1_yield = est_ib01 * (0.045 / 252)

            # Extract fees if available
            a5_fees = daily_fees_map.get(dt, 0.0)
            
            override_alpha_flow = daily_a4_override_map.get(dt, 0.0)
            
            # Gross Alpha Calculation (Adding absolute fees back to Alpha)
            a4_net = pnl_b + pnl_d + override_alpha_flow
            ...
            if existing_row:
                # Always update the row to force the new theoretical a1 math
...
# 4. Live Allocations (WITH DATE FILTER & FX FIX)
    live_pos = {
    ...
    if pos_df is not None:
        # Crucial Fix: Filter to ONLY the absolute latest day to prevent 263-day compounding
        pos_date_col = get_col(pos_df,['ReportDate', 'Date', 'ToDate'])
        if pos_date_col:
            pos_df[pos_date_col] = pd.to_datetime(pos_df[pos_date_col], errors='coerce')
            max_d = pos_df[pos_date_col].max()
            pos_df = pos_df[pos_df[pos_date_col] == max_d].copy()
            
        pos_acc_col = get_col(pos_df, ['ClientAccountID', 'Account ID', 'AccountId'])
        sym_col = get_col(pos_df, ['Symbol'])
        
        if pos_acc_col and sym_col:
            ...
            for _, row in pos_df.iterrows():
                ...
                asset_col = get_col(pos_df, ['AssetClass', 'Asset Class'])
                curr_col = get_col(pos_df, ['CurrencyPrimary', 'Currency'])
                val_base_col = get_col(pos_df, ['PositionValueInBase', 'Position Value in Base'])
                val_local_col = get_col(pos_df, ['PositionValue', 'Position Value', 'Value'])
                fx_col = get_col(pos_df, ['FXRateToBase', 'FX Rate To Base'])
                qty_col = get_col(pos_df, ['Quantity', 'Position'])
                
                asset = str(row.get(asset_col, '')).upper() if asset_col else ''
                curr = str(row.get(curr_col, '')).upper() if curr_col else 'USD'
                
                # FX Logic Fix: Prioritize 'In Base' value. If missing, multiply local by FX.
                usd_val = 0.0
                if val_base_col and pd.notna(row.get(val_base_col)) and str(row.get(val_base_col)).strip() != '':
                    try: usd_val = float(row.get(val_base_col, 0))
                    except: pass
                else:
                    try: local_val = float(row.get(val_local_col, 0)) if val_local_col else 0.0
                    except: local_val = 0.0
                    try: fx = float(row.get(fx_col, 1.0)) if fx_col and pd.notna(row.get(fx_col)) and str(row.get(fx_col)).strip() != '' else 1.0
                    except: fx = 1.0
                    usd_val = local_val * fx
                
                try: qty = float(row.get(qty_col, 0)) if qty_col else 0.0
                except: qty = 0.0
                ...
                if silo == 'D':
                    ...
                    elif asset == 'STK' or asset == '':
                        # Fix: USD stocks -> CFD bucket. SEK/EUR stocks -> INTL bucket.
    ...
    cash_df = sections.get('Cash Report')
    if cash_df is not None:
        c_date_col = get_col(cash_df,['ReportDate', 'Date', 'ToDate', 'FromDate'])
        if c_date_col:
            cash_df[c_date_col] = pd.to_datetime(cash_df[c_date_col], errors='coerce')
            max_c_d = cash_df[c_date_col].max()
            cash_df = cash_df[cash_df[c_date_col] == max_c_d].copy()
            
        cash_acc_col = get_col(cash_df, ['ClientAccountID', 'Account ID', 'AccountId'])
        end_cash_col = get_col(cash_df,['EndingCashInBase', 'Ending Cash in Base', 'EndingCash', 'Ending Cash'])
        curr_col = get_col(cash_df, ['CurrencyPrimary', 'Currency'])
        
        if cash_acc_col and end_cash_col and curr_col:
...
if __name__ == "__main__":
    print("========================================================")
    print("     ESTATE ORCHESTRATOR v20 - INITIALIZING SYNC...      ")
    print("========================================================")
