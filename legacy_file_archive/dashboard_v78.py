...
3. Launch the UI by running exactly:
   streamlit run dashboard_v78.py

CHANGELOG (v78):
         - FIXED: Quarantined Delta-NAV fallback to prevent phantom PnL injections.
         - FIXED: Replaced LEFT JOIN with OUTER JOIN for Cash Flows to fix dropped dates.
=============================================================================
"""
...
def load_and_process_data():
    ...
    try:
        ...
        # v78 FIX: OUTER JOIN to prevent dropping cash flows on days TWS wasn't synced
        df = pd.merge(df, daily_transfers, on=['date', 'account'], how='outer')
        
        # Forward-fill NAV per account for the newly injected dates
        df = df.sort_values(['account', 'date']).reset_index(drop=True)
        df['nav'] = df.groupby('account')['nav'].ffill().fillna(0)
    ...
    if not global_df.empty:
        ...
        try:
            ...
            if not temp_attr.empty:
                ...
                # V78 FIX: Stop row-by-row fallback pollution. Only use delta_nav_pnl for today's live session.
                global_df['daily_pnl'] = global_df['true_daily_pnl'].fillna(0.0)
                last_idx = global_df.index[-1]
                if pd.isna(global_df.at[last_idx, 'true_daily_pnl']):
                    global_df.at[last_idx, 'daily_pnl'] = delta_nav_pnl.at[last_idx]
