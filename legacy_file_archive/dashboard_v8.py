"""
=============================================================================
Script Name: dashboard_v8.py
Purpose: The Streamlit Frontend (Module 8).
         - Integrates GAAP Balance Sheet & Gross Asset Allocation (Pie/Bar).
         - Adds Strategy & Execution Footers to the Live Portfolio.
         - Fixes Yellow Regime text color for visibility.
=============================================================================
"""
...
def load_and_process_data():
    ...
    # Categorize Assets for GAAP/Pie Charts
...
def load_benchmarks(start_date, end_date):
    ...
    def get_regime_and_tor(row):
        ...
        if row['SPY'] < row['sma_20'] and row['sma_20'] < row['sma_10'] or vix > 25:
            ...
            return 'Yellow', 3 if vix < 20 else (2 if vix <= 25 else 1)
...
if not pos_df.empty:
    ...
    # We use Absolute Value for the Pie Chart to properly visualize Margin Impact sizes
    ...
    with col_bar:
        ...
        for asset in bar_df.columns:
            fig_bar.add_trace(go.Bar(name=asset, x=silo_names, y=bar_df[asset], marker_color=COLOR_PALETTE.get(asset, '#cbd5e1')))
        fig_bar.update_layout(barmode='relative', title="GAAP Balance Sheet per Silo (USD)", plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=20, r=20, t=40, b=20), yaxis=dict(zeroline=True, zerolinecolor='black', gridcolor='LightGray'))
        st.plotly_chart(fig_bar, width='stretch')

    with col_pie:
        fig_pie = go.Figure(data=[go.Pie(labels=pie_df['asset_class'], values=pie_df['market_value'], hole=.4, marker=dict(colors=[COLOR_PALETTE.get(a, '#cbd5e1') for a in pie_df['asset_class']]))])
        fig_pie.update_layout(title="Gross Asset Allocation", margin=dict(l=20, r=20, t=40, b=20))
...
for idx, acc in enumerate(SILO_MAP.keys()):
    ...
    with comp_cols[idx]:
        ...
        # Add the Strategy & Execution Footer directly under the table!
...
# EXACT REGIME SQUARES WITH DYNAMIC TOR TEXT & FIX FOR YELLOW TEXT COLOR
...
fig_pnl.add_trace(go.Scatter(
    ...
    textfont=dict(color=chart_df['regime_txt'], size=10, weight='bold'),
    hoverinfo='text',
    hovertext=[f"Regime: {r} | TOR: {t}" for r, t in zip(chart_df['regime'], chart_df['tor'])],
...
if not attr_df.empty:
    ...
    with col_bar:
        fig_attr_bar = go.Figure(data=[go.Bar(
            ...
            textposition='auto', marker_color=['#3b82f6', '#f97316', '#86efac', '#a855f7', '#ef4444']
        )])
    ...
    with col_line:
        ...
        fig_attr_line.add_trace(go.Scatter(x=line_df['date'], y=line_df['a1_yield'].cumsum(), name='Yield', line=dict(color='#3b82f6', width=2)))
        fig_attr_line.add_trace(go.Scatter(x=line_df['date'], y=line_df['a2_beta'].cumsum(), name='Beta', line=dict(color='#f97316', width=2)))
        fig_attr_line.add_trace(go.Scatter(x=line_df['date'], y=line_df['a3_vrp'].cumsum(), name='VRP', line=dict(color='#86efac', width=2)))
        fig_attr_line.add_trace(go.Scatter(x=line_df['date'], y=line_df['a4_alpha'].cumsum(), name='Alpha', line=dict(color='#a855f7', width=2)))
        fig_attr_line.add_trace(go.Scatter(x=line_df['date'], y=line_df['a5_fees'].cumsum(), name='Fees', line=dict(color='#ef4444', width=2)))
...
st.subheader("5. Estate Montecarlo PnL Simulation - Projections vs History")
...
if sim_length > 0:
    ...
    # MULTICOLOR SPAGHETTI LINES
    for i in range(100):
        r, g, b = random.randint(50, 200), random.randint(50, 200), random.randint(50, 200)
        mc_fig.add_trace(go.Scatter(y=cum_sim[i], mode='lines', line=dict(color=f'rgba({r}, {g}, {b}, 0.25)', width=1), showlegend=False, hoverinfo='skip'))
    
    mc_fig.add_trace(go.Scatter(y=mc_avg_path, name='Avg Path (Stable)', mode='lines', line=dict(color='blue', width=3)))
    
    # MASSIVE ORIGINAL HISTORY LINE
    mc_fig.add_trace(go.Scatter(y=orig_cum, name='Original History', mode='lines', line=dict(color='black', width=6)))
