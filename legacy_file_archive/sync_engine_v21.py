"""
=============================================================================
Script Name: sync_engine_v21.py
...
def sync_tws():
    print("========================================================")
    print("   ESTATE SYNC ENGINE v21 - YFINANCE FX ENGINE")
    print("========================================================")    
    ...
    for acc in accounts:        
        ...
        for item in portfolio_items:
            ...
            if c_curr in ['GBP', 'GBX']:
                price_mult = fx / 100.0  # Prices in Pence, Value in Pounds
                value_mult = fx
            else:
                price_mult = fx
                value_mult = fx

            mkt_price_usd = float(item.marketPrice) * price_mult
            mkt_val_usd = float(item.marketValue) * value_mult
            avg_cost_usd = float(item.averageCost) * price_mult
            unrealized_usd = float(item.unrealizedPNL) * value_mult
            realized_usd = float(item.realizedPNL) * value_mult
