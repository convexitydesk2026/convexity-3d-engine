...
streamlit run dashboard_v105.py
CHANGELOG (v105):
...
with st.expander("📊 Expand Institutional Flow Dashboard", expanded=False):
    ...
    if os.path.exists(html_path):
        ...
        # Wrapped in a scrollable div and rendered natively to bypass the components deprecation warning
        st.components.v1.html(html_data, height=900, scrolling=True) if not hasattr(st, 'html') else st.html(f"<div style='height: 900px; overflow-y: scroll;'>{html_data}</div>")
    else:
