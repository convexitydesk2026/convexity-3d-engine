...
streamlit run dashboard_v142.py
CHANGELOG (v142):
- STAGE 50 (v142): Compacted the Pre-Trade Staging Area into a collapsible expander and locked the data editor height to 10 rows to reduce scroll fatigue.
...
with st.expander("🔭 View Pre-Trade Staging Area (Pipeline)", expanded=False):
    ...
    if not df_staging.empty:
        ...
        st.data_editor(
            ...
            hide_index=True, width="stretch", num_rows="dynamic", height=400, key="staging_editor", on_change=commit_staging_edits
        )
