r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v41.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 41 UPGRADES:
         - Market Regime & Total Open Risk (TOR) Integration.
         - Reads 'Status' and 'Set TOR' from SPY_QQQ_Close.csv.
         - Engineered a dedicated 3rd Y-Axis (y3) at the bottom of the PnL chart 
           to display dynamic, color-coded Regime blocks (Red/Green/Yellow).
         - Preserved scrollZoom and dynamic Y-axis auto-scaling from v37.
         - Preserved v40 Smart Money (Silo D) and Central Bank architecture.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v41.html") # v41 CHANGE: Name updated
...
status_js, tor_js = "[]", "[]" # v41 CHANGE: initialized new JS variables

print("\n========================================================")
print("             CIO DIAGNOSTICS & LOGGING (v41)            ")
print("========================================================")
...
if os.path.exists(spy_qqq_file_path):
    try:
        ...
        # v41 CHANGE: Safely guarantee existence of new Regime columns
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        # v41 CHANGE: Included 'Status' and 'Set TOR' in the merge logic
        ...
        # v41 CHANGE: Extracted Regime arrays as strict JSON dumps to handle strings cleanly
        status_js = json.dumps(master_bench['Status'].fillna('').astype(str).tolist())
        tor_js = json.dumps(master_bench['Set TOR'].replace(r'^\s*$', np.nan, regex=True).fillna('').astype(str).tolist())
...
<head>
    ...
    <title>Estate Master Dashboard v41</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v41</p>
        </div>
    ...
    <script>
        ...
        function updateUI() {
            ...
            document.getElementById('siloD').value = sD.toFixed(2);
        ...
        // v41 CHANGE: Reconstructing the Regime Strings from python dump
        ...
        if (datesPnl && datesPnl.length > 0) {
            ...
            // v41 CHANGE: Build the dedicated Regime & TOR tracking ribbon
            let regimeColors = [];
            let regimeFonts =[];
            let regimeText = [];
            let regimeY =[];
            ...
            for(let i=0; i<datesCumPnl.length; i++) {
                ...
                regimeText.push(t);
                regimeY.push(1); // Constant height 1 for the bottom ribbon
                ...
                else { regimeColors.push('rgba(0,0,0,0)'); regimeFonts.push('#000000'); } // Transparent if no data
            }
            ...
            let traceRegime = {
                ...
                type: 'bar',
                xaxis: 'x',
                yaxis: 'y3', // Dedicated bottom axis
                marker: {color: regimeColors},
                text: regimeText,
                textposition: 'inside',
                insidetextanchor: 'middle',
                insidetextfont: {color: regimeFonts, size: 10, family: 'Arial Black'},
                hoverinfo: 'text',
            ...
            let pnlLayout = {
                ...
                // v41 CHANGE: Squished the main Y-axes to domain [0.08, 1] to make room for y3
                yaxis: { title: 'Daily PnL (USD)', side: 'left', showgrid: true, gridcolor: '#e5e7eb', zeroline: true, zerolinecolor: '#9ca3af', zerolinewidth: 2, fixedrange: true, domain:[0.08, 1] },
                yaxis2: { title: 'Cumulative PnL (USD)', side: 'right', overlaying: 'y', showgrid: false, zeroline: false, fixedrange: true, domain: [0.08, 1] },
                // v41 CHANGE: The new dedicated, fixed 3rd axis for the Regime Ribbon
                yaxis3: { domain: [0, 0.05], visible: false, fixedrange: true },
                legend: { orientation: 'h', y: 1.1, x: 0.5, xanchor: 'center' }
            };
            
            let pnlConfig = {displayModeBar: false, responsive: true, scrollZoom: true};
            // v41 CHANGE: Appended traceRegime to the chart
...
# v41 CHANGE: Replaced Regime Variables
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v41 (Regime Edition) created.")
