"""
=============================================================================
Script Name: dashboard_v7.py
Purpose: The Streamlit Frontend (Module 7).
         - Applies corrected Attribution definitions.
         - Adds multicolor spaghettis & doubles Monte Carlo height.
         - Injects dynamic TOR numbers inside Regime squares.
         - Filters out pre-funding "dead air" on Cumulative Trajectory.
=============================================================================
"""
...
def load_benchmarks(start_date, end_date):
    ...
    def get_regime_and_tor(row):
        ...
        if row['SPY'] < row['sma_20'] and row['sma_20'] < row['sma_10'] or vix > 25:
            tor = 1 if vix < 25 else 0
            return 'Red', tor
        # Green Regime
        elif row['SPY'] > row['sma_10'] and row['sma_10'] > row['sma_20']:
            tor = 5 if vix < 15 else (4 if vix <= 20 else 3)
            return 'Green', tor
        # Yellow Regime
        else:
            tor = 3 if vix < 20 else (2 if vix <= 25 else 1)
            return 'Yellow', tor
...
regime_colors = {'Green': '#166534', 'Yellow': '#eab308', 'Red': '#991b1b'}
chart_df['regime_color'] = chart_df['regime'].map(regime_colors)

fig_pnl.add_trace(go.Scatter(
    x=chart_df['date'], y=[0]*len(chart_df), 
    mode='markers+text', 
    marker=dict(color=chart_df['regime_color'], symbol='square', size=16, line=dict(width=1, color='black')),
    text=chart_df['tor'],
...
if not attr_df.empty:
    ...
    # Pre-Funding Dead Air Filter for Trajectory Line Chart
