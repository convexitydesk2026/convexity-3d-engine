r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v58.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 53 UPGRADES:
         - Appended a comprehensive TOR (Total Open Risk) legend and Regime Key 
           to the Daily PnL Chart to explicitly define risk bounds for Silo B.
         - Hardcoded filename output to v58.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v58.html") # v58 CHANGE
...
def process_metrics(df_acc):
    ...
    # --- v58 FIX: Eliminate the "Dead Air" penalty for Volatility Metrics ---
    # We isolate only the days where capital was actually deployed to calculate Sharpe/MaxDD
...
print("\n========================================================")
print("             CIO DIAGNOSTICS & LOGGING (v58)            ") # v58 CHANGE
print("========================================================")
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        # --- v58 ALIGNMENT LOGIC: Find the exact day actual trading started ---
        # We isolate the very first day the Estate Return had a non-zero reaction to the market
        active_days = master_bench[master_bench['Estate_Ret'] != 0.0]
        if not active_days.empty:
            first_active_idx = active_days.index[0]
            # Step back exactly 1 day to act as the 0% baseline origin
        ...
        # Lock the origin row to absolute zero so compounding is flawlessly aligned
...
<head>
    ...
    <title>Estate Master Dashboard v58</title> <!-- v58 CHANGE -->
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8 relative">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v58</p>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <!-- ========================================================= -->
            <!-- PANEL 6: DEPLOYMENT & CAPACITY TRACKER (v58 UPGRADE)      -->
            <!-- ========================================================= -->
            <div class="xl:col-span-12 mt-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 relative overflow-hidden flex flex-col justify-between">
                            <div>
                                ...
                                <!-- V58 DEPLOYMENT SCHEDULE -->
                                <div class="mt-4 pt-3 border-t border-gray-200 bg-white p-2 rounded border border-gray-100 shadow-sm">
                                    <h5 class="text-[10px] font-black text-gray-700 uppercase mb-1.5 flex justify-between"><span>Upcoming Deployments</span><span class="text-blue-600">Total: $30,000</span></h5>
                                    <ul class="text-[10px] text-gray-600 font-mono space-y-1">
                                        <li class="flex justify-between items-center"><span class="flex items-center"><span class="w-1.5 h-1.5 bg-blue-400 rounded-full mr-1.5"></span>Tranche 1: UCITS DCA</span><span class="font-bold text-gray-800">$10,000 (Mid-May)</span></li>
                                        <li class="flex justify-between items-center"><span class="flex items-center"><span class="w-1.5 h-1.5 bg-blue-400 rounded-full mr-1.5"></span>Tranche 2: UCITS DCA</span><span class="font-bold text-gray-800">$10,000 (End-May)</span></li>
                                        <li class="flex justify-between items-center"><span class="flex items-center"><span class="w-1.5 h-1.5 bg-purple-400 rounded-full mr-1.5"></span>Tranche 3: Alpha Merit</span><span class="font-bold text-gray-800">$10,000 (Standby)</span></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function updateUI() {
            ...
            // =========================================================
            // PANEL 6 LOGIC (Deployment Tracker - v58 UPGRADE)
            // =========================================================
            ...
            if (cashBufferPct >= 40) {
                document.getElementById('trk_cash_status').innerHTML = `<span class="text-green-700 font-black">✓ SAFE:</span> You have <span class="font-black text-green-700">${fmtCur(totalCashBuffer - (totalEstate * 0.40))}</span> available for scheduled deployments.`;
            } else {
            ...
            try {
                ...
                let actionText = "";
                if (lastRegime === 'green') { 
                    actionText = "<span class='text-green-700 font-black text-sm uppercase tracking-wide mb-1 block'>🟢 Green Regime (Risk On)</span><b>Action:</b> Transfer $10,000 from Silo A Cash/IB01 to purchase Physical UCITS (CSPX / CNDX).<br><br><b>Options:</b> Sell new XSP/XND spreads up to remaining margin capacity."; 
                } else if (lastRegime === 'yellow') { 
                    actionText = "<span class='text-yellow-600 font-black text-sm uppercase tracking-wide mb-1 block'>🟡 Yellow Regime (Transition)</span><b>Action:</b> HOLD current equity positions. Do not deploy DCA. Funnel all new external cash deposits strictly into IB01.<br><br><b>Options:</b> Throttle new Put Spreads to strict 45 DTE."; 
                } else if (lastRegime === 'red') { 
                    actionText = "<span class='text-red-600 font-black text-sm uppercase tracking-wide mb-1 block'>🔴 Red Regime (Defense)</span><b>Action:</b> HALT all active equity purchases. Defend the 40% Cash Buffer.<br><br><b>Options:</b> If VIX spikes violently, buy 90-DTE SPX Puts (Taleb Tail-Hedge) from Silo C."; 
                }
        ...
        if (Object.keys(liveAlloc).length > 0) {
            ...
            if (liveAlloc['A']) {
                ...
                document.getElementById('a_xsp').value = liveAlloc['A']['OPT_MARGIN'] || 0; // v58 CHANGE
            }
            ...
            if (liveAlloc['C']) {
                ...
                document.getElementById('c_xsp').value = liveAlloc['C']['OPT_MARGIN'] || 0; // v58 CHANGE
            }
        }
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v58 created.")
