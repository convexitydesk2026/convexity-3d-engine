"""
=============================================================================
Script Name: attribution_engine_v19.py
Purpose: Parses the official IBKR Flex Query (MTM and CNAV).
         ...
         - ADDED: Failsafe circuit breaker to abort commits if 0 records found.
=============================================================================
"""
...
def run_attribution():
    ...
    # -------------------------------------------------------------------------
    # WRITE CASH FLOWS & ATTRIBUTION
    # -------------------------------------------------------------------------
    
    # V19 FIX: Refresh Cash Transfers Table SAFELY
    # We explicitly DO NOT DROP the table. We only delete the automated entries so manual UI entries survive.
    c.execute("CREATE TABLE IF NOT EXISTS cash_transfers (date TEXT, account TEXT, amount REAL, type TEXT, notes TEXT)")
    c.execute("DELETE FROM cash_transfers WHERE notes = 'Official Clearinghouse Flow'")
    ...
    c.execute("DELETE FROM daily_attribution")
    ...
    print("[+] SUCCESS: v19 Engine Complete. Full Automation & Healing Deployed.\n")
