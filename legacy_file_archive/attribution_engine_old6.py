...
Purpose: Calculates daily PnL split across 5 strategic buckets and appends
         ...
         (Phase 9: The Graveyard Catcher & Noise Canceler)
=============================================================================
"""
import sqlite3
import pandas as pd
...
def run_attribution():
    print("========================================================")
    print("   ESTATE P&L ATTRIBUTION ENGINE (PHASE 9)")
    print("   The Graveyard Catcher & Noise Canceler")
    print("========================================================")
    ...
    # 3. Process Phase 9 Allocation Logic
    for i in range(len(dates)):
        ...
        # Build lookup for yesterday's state
        prev_lookup = {}
        prev_buckets = {}
        ...
        for _, r in pos_T.iterrows():
            ...
            if key in prev_lookup:
                # Existing asset: Calculate strict Delta PnL
                prev_U, prev_R = prev_lookup[key]
                dU = r['unrealized_pnl'] - prev_U
                dR = r['realized_pnl'] - prev_R
                tracked_pnl = dU + dR
            else:
                # Newly opened asset: Ignore YTD realized carry-over ghosts from IBKR
                tracked_pnl = r['unrealized_pnl']
                
            alloc[bkt] += tracked_pnl

        # Step B: Reconcile Drift vs True NAV
        true_daily = global_true_pnl.get(curr_date, 0.0)
        tracked_daily = sum(alloc.values())
        drift = true_daily - tracked_daily

        # Step C: The Graveyard Catcher & Noise Canceler
        vanished_keys = set(prev_lookup.keys()) - active_keys
        
        if len(vanished_keys) > 0 and abs(drift) > 0.01:
            # A trade closed! Map the closing profit (Drift) to the vanished assets' buckets
            vanished_buckets = set([prev_buckets[k] for k in vanished_keys])
            share = drift / len(vanished_buckets)
            for b in vanished_buckets:
                alloc[b] += share
        else:
            # No trades closed. The drift is background noise. 
            # DUMP TO FEES REGARDLESS OF SIGN to cancel out ratcheting.
            if abs(drift) > 0.01:
                alloc['a5_fees'] += drift
    ...
    print("[+] Phase 9 Complete. Vanished PnL captured. Ratchet effect destroyed.")
