"""
=============================================================================
Script Name: dashboard_v16.py
Purpose: The Streamlit Frontend (Module 16 - UI Integration & 3D Optics).
         - Fixed DataEditorMixin error by injecting an interactive Checkbox column for selection.
         - Scrubbed deprecated `use_container_width` arguments for `width="stretch"`.
         - Panel 8: Row-selection dynamically triggers Black-Scholes 3D Surface.
         - Panel 6: Added 4 secondary margin/cash sub-gauges for Silo A & C.
         - Panel 4: PnL Fees trace color darkened to red, appended terminal labels.
=============================================================================
"""
...
def get_exact_opt_margin(df_in):
    """Parses IBKR option string to calculate exact required margin footprint."""
...
st.markdown(f"**Script version:** `dashboard_v16.py`")
st.divider()
...
if not attr_df.empty:
    ...
    # Updated a5_fees color to a darker red (#991b1b)
    ...
    with col_line:
        ...
        # Append Annotations dynamically to the end of each line
...
# Retrieve Silo A and Silo C NAVs for the sub-gauges
...
with c1:
    ...
    # 2 Sub-Gauges directly under Bedrock Cash
    sc1, sc2 = st.columns(2)
    
    # Silo A Cash Sub-Gauge
    ...
    fig_A_cash.update_layout(height=130, margin=dict(l=5, r=5, t=30, b=0))
    with sc1: st.plotly_chart(fig_A_cash, width="stretch")

    # Silo C Cash Sub-Gauge
    ...
    fig_C_cash.update_layout(height=130, margin=dict(l=5, r=5, t=30, b=0))
    with sc2: st.plotly_chart(fig_C_cash, width="stretch")
...
with c2:
    ...
    # 2 Sub-Gauges directly under Margin Utilization
    sc3, sc4 = st.columns(2)
    
    # Silo A Margin Sub-Gauge
    ...
    fig_A_margin.update_layout(height=130, margin=dict(l=5, r=5, t=30, b=0))
    with sc3: st.plotly_chart(fig_A_margin, width="stretch")

    # Silo C Margin Sub-Gauge
    ...
    fig_C_margin.update_layout(height=130, margin=dict(l=5, r=5, t=30, b=0))
    with sc4: st.plotly_chart(fig_C_margin, width="stretch")
...
if sim_length > 0:
    ...
    for i in range(200):
        ...
        mc_fig.add_trace(go.Scatter(y=cum_sim[i], mode='lines', line=dict(color=f'rgba({r_c}, {g_c}, {b_c}, 0.40)', width=2), showlegend=False, hoverinfo='skip'))
...
# Python translation of the JavaScript Black-Scholes Engine
def normCDF(x):
...
def fetch_live_data(ticker_symbol):
    """Fetches live underlying price and IV dynamically from Yahoo Finance."""
    try:
...
# 4. Streamlit Interactive Editor & 3D Integration Split
if not journal_raw_df.empty:
    col_ledger, col_viz = st.columns([0.6, 0.4])
    
    with col_ledger:
        # Dynamically inject the View 3D boolean column for "row selection"
        ...
        edited_df = st.data_editor(
            ...
            key="journal_editor",
            column_config={
                "👁️ View 3D": st.column_config.CheckboxColumn("View\n3D", help="Check this box to visualize the contract's topography"),
    ...
    # Clean the dataframe before database comparison/save
    ...
    # Only save to DB if actual data changed (ignoring the UI checkbox)
    ...
    # 6. Black-Scholes 3D Visualizer Trigger
    with col_viz:
        # Detect if the user checked the UI box
        selected_rows = edited_df[edited_df['👁️ View 3D'] == True]
        
        if selected_rows.empty:
            st.info("👈 **Interact:** Check the 'View 3D' box on any contract row in the ledger to render its Live 2D/3D Topography.")
        else:
            # Render logic for the first checked row
            row_data = selected_rows.iloc[0]
            tckr = row_data.get('Ticker', 'XSP')
            raw_dte = row_data.get('Days Remaining', 0)
            
            if str(raw_dte) == 'Closed':
                st.warning(f"**{tckr} Contract is Closed.** Black-Scholes topography is locked for historical trades.")
            else:
                # Dynamic Variables & Safeguards against empty rows
                curr_dte = float(raw_dte) if pd.notna(raw_dte) else 0.0
                init_dte_raw = row_data.get('DTE at Entry', 45)
                init_dte = float(init_dte_raw) if pd.notna(init_dte_raw) else 45.0
                
                K_s_raw = row_data.get('Short Strike', 0)
                K_s = float(K_s_raw) if pd.notna(K_s_raw) else 0.0
                
                K_l_raw = row_data.get('Long Strike', 0)
                K_l = float(K_l_raw) if pd.notna(K_l_raw) else 0.0
                
                qty_raw = row_data.get('Quantity', 1)
                qty = float(qty_raw) if pd.notna(qty_raw) else 1.0
                
                prem_raw = row_data.get('Premium Collected (USD)', 0)
                prem = float(prem_raw) if pd.notna(prem_raw) else 0.0
                ...
                fig_2d.update_layout(title="2D Theta Decay Profile", margin=dict(l=20, r=20, t=40, b=20), height=280, legend=dict(orientation="h", y=-0.2))
                ...
                fig_3d.update_layout(
                    title="3D Topography (Time vs Price)", margin=dict(l=0, r=0, b=0, t=40), height=380,
