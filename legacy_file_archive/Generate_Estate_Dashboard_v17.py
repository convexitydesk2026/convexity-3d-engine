r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v17.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 17 UPGRADES: 
         - Added "Crypto ETPs" slider to Silos A, C, and D.
         - Integrated Crypto ETPs into Bar Chart, Pie Chart, and GAAP math.
         - Retains v16 Time-Weighted Return (TWR) benchmark charts.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v17.html")
...
# 3.c Benchmark Parsing & Integration
...
if not bench_df.empty and not global_df.empty:
    merged_df = pd.merge(global_df[['Date', 'Daily_Return']], bench_df[['Date', 'SPY Close', 'QQQ Close']], on='Date', how='inner')
    
    if not merged_df.empty:
        merged_df = merged_df.sort_values('Date').reset_index(drop=True)
        merged_df['SPY_Ret'] = merged_df['SPY Close'].pct_change()
        merged_df['QQQ_Ret'] = merged_df['QQQ Close'].pct_change()
        
        merged_df.loc[0,['Daily_Return', 'SPY_Ret', 'QQQ_Ret']] = 0.0
        merged_df[['Daily_Return', 'SPY_Ret', 'QQQ_Ret']] = merged_df[['Daily_Return', 'SPY_Ret', 'QQQ_Ret']].fillna(0)
        
        merged_df['Estate_Cum'] = (1 + merged_df['Daily_Return']).cumprod() - 1
        merged_df['SPY_Cum'] = (1 + merged_df['SPY_Ret']).cumprod() - 1
        merged_df['QQQ_Cum'] = (1 + merged_df['QQQ_Ret']).cumprod() - 1
        
        dates_js = merged_df['Date'].dt.strftime('%Y-%m-%d').tolist()
        estate_js = (merged_df['Estate_Cum'] * 100).round(2).tolist()
        spy_js = (merged_df['SPY_Cum'] * 100).round(2).tolist()
        qqq_js = (merged_df['QQQ_Cum'] * 100).round(2).tolist()
...
<head>
    ...
    <title>Family Estate Master Allocator v17</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[1800px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v17</p>
        </div>

        <!-- THE MASTER ESTATE AGGREGATION HEADER -->
        <div class="bg-gray-100 p-4 rounded-xl shadow-md text-center border border-gray-300 mb-6">
        ...
        <!-- BENCHMARK CHART -->
        <div class="bg-gray-100 p-4 rounded-xl shadow-md border border-gray-300 mb-6">
        ...
        <div class="mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="bg-blue-900 text-white p-4 rounded-xl shadow-md text-center border-b-4 border-blue-500 relative">
            ...
            <div class="bg-yellow-100 text-gray-900 p-4 rounded-xl shadow-md text-center border-b-4 border-yellow-400">
                ...
                <div class="bg-yellow-200 rounded py-1 mb-2 border border-yellow-300"><p class="text-sm font-bold text-yellow-900">Bal: VAR_D_NAV_FMT</p></div>
                <div class="grid grid-cols-2 text-xs mt-3 border-t border-yellow-300 pt-3 gap-y-2 font-mono">
...
for acc, prefix in[("ESTATE", "ESTATE"), ("U23144948", "A"), ("U23139264", "B"), ("U23154199", "C"), ("U25218481", "D")]:
    ...
    # Inject Formatted Static Balances for Header display
    nav_val = metrics[acc]["nav"]
    html_content = html_content.replace(f"VAR_{prefix}_NAV_FMT", f"${nav_val:,.2f}")

    # Auto-populate editable NAV inputs (Silos A, B, C, D only)
    if prefix != "ESTATE":
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v17 created.")
