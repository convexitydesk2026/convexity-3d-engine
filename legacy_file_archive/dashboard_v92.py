...
3. Launch the UI by running exactly:
   streamlit run dashboard_v92.py

CHANGELOG (v92):
         - STAGE 8 (v92): Neutralized 'HTTP Error 404' console spam by silencing yfinance exceptions & standardizing FX pairs.
         - STAGE 7b (v91): Added Earnings Dates and Live R-Multiples to Alpha Ledger. MASTER BUILD.
=============================================================================
"""
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v32.py")
...
def process_metrics(df_acc, rf_rate):
    ...
    # v92 Absolute Truth Math: Start NAV is inherently 0
...
def load_and_process_data():
    ...
    try:
        ...
        # v92 FIX: Robust NAV Forward Fill (Solves the Phantom Volatility Jump)
    ...
    # Global tracking arrays (v92: Pure Absolute Math across all Silos. Kills the Whipsaw)
...
with calc_placeholder.container():
    ...
    with st.expander("Position Sizing Engine", expanded=False):
        ...
        calc_entry = st.number_input("Entry Price ($)", min_value=0.0, value=0.0, step=1.0)
        calc_stop = st.number_input("Stop Loss Limit ($)", min_value=0.0, value=0.0, step=1.0, help="For Longs: Low of the day. For Shorts: High of the day.")
        
        if calc_entry > 0 and calc_stop > 0 and calc_nav > 0:
            risk_usd = calc_nav * (calc_risk_pct / 100.0)
            spread = abs(calc_entry - calc_stop)
            ...
            if spread > 0:
                ...
                if valid_dir:
                    shares = int(risk_usd // spread)
                    capital_req = shares * calc_entry
                    cash_remaining = calc_uninvested_cash - capital_req
                    ...
                    <div style="background-color: #f8fafc; padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1; margin-top: 10px;">
                        ...
                        <div style="font-size: 12px; color: #475569;">Risk per Share: <b>${spread:,.2f}</b></div>
                        <div style="font-size: 12px; color: #475569;">Total Capital Required: <b>${capital_req:,.2f}</b></div>
                        <div style="font-size: 12px; color: {'#16a34a' if cash_remaining >= 0 else '#dc2626'}; margin-top: 5px;">Post-Trade Cash: <b>${cash_remaining:,.2f}</b></div>
                    </div>
...
if not df_alpha.empty:
    ...
    # v92: Fetch and inject SMAs, Earnings, and R-Multiples
