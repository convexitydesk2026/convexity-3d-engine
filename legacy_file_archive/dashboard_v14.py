"""
=============================================================================
Script Name: dashboard_v14.py
Purpose: The Streamlit Frontend (Module 14 - Exact Clearinghouse Math).
         - Eradicated the hardcoded $2,500 option margin proxy.
         - Parses exact strike prices from live TWS symbol strings.
         - Matches exact clearinghouse margin footprint natively in Python.
         - Drift detector now flawlessly compares live vs ledger without false alarms.
=============================================================================
"""
...
# --- EXACT MARGIN CALCULATION ENGINE ---
def get_exact_opt_margin(df_in):
    ...
    try:
        # IBKR live sync saves symbols as: XSP_YYYYMMDD_STRIKE_RIGHT (e.g., XSP_20260626_670.0_P)
        df_opt['strike'] = df_opt['symbol'].apply(lambda x: float(x.split('_')[2]))
        
        # Credit put spreads: short leg has negative position, long leg has positive position
...
st.markdown(f"**Script version:** `dashboard_v14.py`")
st.divider()
