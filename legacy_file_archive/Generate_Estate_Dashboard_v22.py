r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v22.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 22 UPGRADES: 
         - Removed cluttered text from the daily PnL histogram bars.
         - Overlaid Cumulative PnL lines (Estate vs SPY vs QQQ) on the histogram.
         - Added a right-side secondary Y-axis for Cumulative PnL tracking.
         - Engineered a "Theoretical NAV Replictor" to perfectly align benchmark
           dollar PnL with dynamic IBKR deposits and withdrawals.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v22.html")
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        # Base the master sequence on the Estate's actual trading days
        ...
        # Forward fill so non-trading days (holidays) don't break the cumulative math
        ...
        # Merge Individual Silos for TWR tracking
        for acc in["U23144948", "U23139264", "U23154199", "U25218481"]:
            if acc in silo_returns:
                master_bench = pd.merge(master_bench, silo_returns[acc], on='Date', how='left')
        
        ret_cols =['Daily_Return', 'U23144948_Ret', 'U23139264_Ret', 'U23154199_Ret', 'U25218481_Ret']
        for c in ret_cols:
        ...
        # 1. Percentage Cumulative Returns (TWR)
        for col in ['SPY_Ret', 'QQQ_Ret'] + ret_cols:
            cum_col = col.replace('_Ret', '_Cum').replace('Daily_Return', 'Estate_Cum')
            master_bench[cum_col] = (1 + master_bench[col]).cumprod() - 1
            
        # 2. Theoretical DOLLAR PnL Logic (The Replicator Engine)
        ...
        master_bench['SPY_Cum_PnL'] = master_bench['SPY_Daily_PnL'].cumsum()
        master_bench['QQQ_Cum_PnL'] = master_bench['QQQ_Daily_PnL'].cumsum()

        # JavaScript Data Exports
        ...
        dates_cum_pnl_js = master_bench['Date'].dt.strftime('%Y-%m-%d').tolist()
        estate_cum_pnl_js = master_bench['Estate_Cum_PnL'].round(2).tolist()
        spy_cum_pnl_js = master_bench['SPY_Cum_PnL'].round(2).tolist()
        qqq_cum_pnl_js = master_bench['QQQ_Cum_PnL'].round(2).tolist()
...
<head>
    ...
    <title>Family Estate Master Allocator v22</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v22</p>
        </div>
...
html_content = html_content.replace("VAR_DATES_CUM_PNL", str(dates_cum_pnl_js))
html_content = html_content.replace("VAR_ESTATE_CUM_PNL", str(estate_cum_pnl_js))
html_content = html_content.replace("VAR_SPY_CUM_PNL", str(spy_cum_pnl_js))
html_content = html_content.replace("VAR_QQQ_CUM_PNL", str(qqq_cum_pnl_js))
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v22 created.")
