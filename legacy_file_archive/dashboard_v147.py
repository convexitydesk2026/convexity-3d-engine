...
streamlit run dashboard_v147.py
CHANGELOG (v147):
...
- STAGE 51 (v143): Upgraded Alpha Risk Ledger (Panel 3B) and Staging Area to fully support omnidirectional (Long/Short) campaigns, including inverted risk/PnL math for short positions.
...
with st.sidebar:
    ...
    if st.button("🌐 Generate Market Flow Report", width="stretch"):
        with st.spinner("Calculating Institutional Flow & Generating PDF... Please wait (~15s)"):
            try:
                subprocess.run(["python", "market_flow_engine_v6.py"], cwd=TARGET_DIR, check=True)
