...
if not df_phys.empty:
    for sym, g in df_phys.groupby('symbol'):
        ...
        cost = g['avg_cost'].sum()
        mkt_price_local = g['market_price'].iloc[0]
        
        # 100% Offline FX Conversion Engine
        fx_rate = mkt_val / (mkt_price_local * shares) if (mkt_price_local * shares) != 0 else 1.0
        mkt_price_usd = mkt_price_local * fx_rate
        avg_cost_usd = cost / shares if shares > 0 else 0
        
        # Fetch Stops
        sym_stops = open_orders_df[(open_orders_df['symbol'] == sym) & (open_orders_df['action'] == 'SELL') & (open_orders_df['order_type'].isin(['STP', 'STP LMT']))].copy() if not open_orders_df.empty else pd.DataFrame()
...
if not df_alpha.empty:
    df_alpha = df_alpha.sort_values('Market Value', ascending=False)
    ...
    fig_alpha.add_trace(go.Bar(
        x=df_alpha['Ticker'], y=base_vals, name='Base Value', 
        marker_color='#e2e8f0', hoverinfo='skip'
    ))
    fig_alpha.add_trace(go.Bar(
        x=df_alpha['Ticker'], y=green_tops, name='Unrealized Profit', 
        marker_color='#bbf7d0', hovertemplate='Profit: $%{y:,.0f}<extra></extra>'
    ))
    fig_alpha.add_trace(go.Bar(
        x=df_alpha['Ticker'], y=red_tops, name='Unrealized Loss', 
        marker_color='#fecaca', hovertemplate='Loss: $%{y:,.0f}<extra></extra>'
    ))
    
    fig_alpha.add_trace(go.Scatter(
        x=df_alpha['Ticker'], y=df_alpha['Cost'], mode='markers',
        marker=dict(symbol='line-ew', size=35, color='black', line=dict(width=4)),
        name='Cost Basis', hovertemplate='Total Cost: $%{y:,.0f}<extra></extra>'
    ))
    
    fig_alpha.add_trace(go.Scatter(
        x=df_alpha['Ticker'], y=df_alpha['Total SL Value'], mode='markers',
        marker=dict(symbol='circle', size=12, color='black', line=dict(color='white', width=1)),
        name='Stop Loss (Total Val)', hovertemplate='Stop Value: $%{y:,.0f}<extra></extra>'
    ))
    
    for i, r in df_alpha.iterrows():
        if r['Total SL Value'] <= 0.01:
            fig_alpha.add_annotation(
                x=r['Ticker'], y=0, text="SL=0", showarrow=False, 
                yshift=-15, font=dict(color="#b91c1c", size=11, weight="bold")
            )
    ...
    display_alpha = df_alpha[['Ticker', 'Shares', 'Spot Price', 'Market Value', 'Cost', 'Avg SL', 'Open Risk', 'Locked Profit']].copy()
    ...
    st.dataframe(display_alpha.style.format({
        'Shares': '{:,.0f}', 'Spot Price': '${:,.2f}', 'Market Value': '${:,.0f}', 
        'Cost': '${:,.0f}', 'Avg SL': '${:,.2f}', 'Open Risk': '${:,.0f}', 'Locked Profit': '${:,.0f}'
    }).map(lambda x: 'color: #dc2626; font-weight:bold;' if isinstance(x, (int, float)) and x < 0 else '', subset=['Open Risk'])
      .map(lambda x: 'color: #16a34a; font-weight:bold;' if isinstance(x, (int, float)) and x > 0 else '', subset=['Locked Profit']), 
