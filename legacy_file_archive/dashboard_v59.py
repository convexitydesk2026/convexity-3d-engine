...
3. Launch the UI by running exactly:
   streamlit run dashboard_v59.py

CHANGELOG (v59):
         - PATCHED: VRP Capital Velocity Visualizer now maps Live TWS Unrealized PnL instead of NaN.
         - PATCHED: Synthetic Beta 3D Engine. Fixed double-negative premium, negative margin bug, and zero-strike scaffolding distortions.
=============================================================================
"""
...
if not pos_df.empty:
    ...
    for _, opt in open_opts.iterrows():
        ...
        # EXEMPTION: Ignore SMH (directional macro shorts) from the Naked Options alert
        if base_tckr == 'SMH':
            continue
...
if not journal_raw_df.empty:
    ...
    if not active_vrp.empty:
        ...
        # v59: Dynamically map LIVE TWS PnL to calculate Live Annualized ROC
        live_roc_list = []
        live_ann_roc_list = []
        ...
        fig_scatter.add_trace(go.Scatter(
            ...
            marker=dict(
                size=marker_sizes,
                color=active_vrp['Live ROC %'],
                colorscale='RdYlGn',
                showscale=True,
                colorbar=dict(title="Live ROC %"),
                line=dict(width=1, color='black')
            ),
            hovertemplate="<b>%{text}</b><br>Days Rem: %{x}<br>Live Ann. ROC: %{y:.1f}%<br>Margin: $%{customdata:,.0f}<extra></extra>",
        ...
        fig_scatter.update_layout(
            title="VRP Capital Velocity Visualizer (Live PnL Projection)",
            xaxis_title="Days Remaining (DTE) →",
            yaxis_title="Live Annualized ROC (%)",
...
if not journal_raw_df.empty:
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            # v59: 3D Engine Native Debit Spread & Call Detection
