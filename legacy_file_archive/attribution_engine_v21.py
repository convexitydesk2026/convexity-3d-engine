"""
=============================================================================
Script Name: attribution_engine_v21.py
...
def run_attribution():
        ...
        for row in reader:
            ...
            # -- EXTRACT DATA --
                ...
                if sec_type == 'CNAV' and cnav_idx:
                    ...
                    # V21 FIX: Ignore Summary Rows to prevent parsing "Total" as an account
                ...
                # --- MARK-TO-MARKET PERFORMANCE SECTION (MTMP) ---
                    ...
                    # V21 FIX: Explicitly ignore "Total" summary rows to prevent massive double-counting
                    if not symbol or str(symbol).strip() == '' or 'total' in str(symbol).lower() or 'total' in str(asset_class).lower():
                        continue
    ...
    # -------------------------------------------------------------------------
    # WRITE CASH FLOWS & ATTRIBUTION (v21: DYNAMIC STITCHING)
    # -------------------------------------------------------------------------
    ...
    if min_date:
        # v21: Only delete official flows that are equal to or newer than the CSV's oldest date.
    ...
    print("[+] SUCCESS: v21 Engine Complete. Infinite Data Stitching Deployed.\n")
