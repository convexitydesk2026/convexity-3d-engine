...
3. Launch the UI by running exactly:
   streamlit run dashboard_v44.py

CHANGELOG (v44):
         - FIX: Automated 21-DTE Gamma Cliff Alerts in Executive Briefing.
         - FIX: YFinance API Redundancy (SQLite fallback for FX, Benchmarks, IRX).
         - FIX: Overhauled Champion Journal UI (Silo dropdown, Undo button, and visible Ledger).
=============================================================================
"""
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v23.py")
...
with c_log:
    ...
    if not champ_hist_df.empty:
        if st.button("⏪ Undo / Delete Last Logged Exit", use_container_width=True):
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
...
if not champ_hist_df.empty:
    ...
    display_hist.rename(columns={
        ...
        'r_multiple': 'R-Multiple', 'notes': 'Notes'
    }, inplace=True)
    display_hist = display_hist[['Exit Date', 'Symbol', 'Silo', '1R Base ($)', 'Realized PnL ($)', 'R-Multiple', 'Notes']]
    ...
    st.dataframe(
        display_hist.style.format({
            ...
            'Realized PnL ($)': '${:,.2f}',
            'R-Multiple': '{:+.2f} R'
        }).map(lambda v: 'color: #166534; font-weight: bold;' if v > 0 else ('color: #991b1b; font-weight: bold;' if v < 0 else ''), subset=['R-Multiple']),
        hide_index=True,
        width="stretch"
    )
else:
