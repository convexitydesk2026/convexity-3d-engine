...
streamlit run dashboard_v144.py
CHANGELOG (v144):
            ...
            for row_idx_str, row_edits in edits.items():
                ...
                if new_entry > 0 and new_stop > 0:
                    if new_type == 'Long' and new_stop >= new_entry:
                        st.toast(f"❌ Invalid Long: Stop ({new_stop}) must be below Entry ({new_entry}). Edit rejected.", icon="🚨")
                        continue # Skip saving this row to the database
                    elif new_type == 'Short' and new_stop <= new_entry:
                        st.toast(f"❌ Invalid Short: Stop ({new_stop}) must be above Entry ({new_entry}). Edit rejected.", icon="🚨")
                        continue # Skip saving this row to the database
                # ------------------------------------------
