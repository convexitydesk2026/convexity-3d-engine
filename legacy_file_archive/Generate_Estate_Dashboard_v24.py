r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v24.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 24 UPGRADES: 
         - BUG FIX: Implemented json.dumps() to sanitize all arrays, preventing 
           silent JavaScript syntax crashes caused by 'nan' or 'inf'.
         - Retains Theoretical NAV Replicator for Dollar PnL Benchmarking.
         - Retains uncluttered Daily PnL Histogram with Cumulative Overlay.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v24.html")
...
if os.path.exists(csv_file_path):
    try:
        ...
        if not pnl_df.empty:
            ...
            dates_pnl_js = json.dumps(pnl_pivot.index.strftime('%Y-%m-%d').tolist())
            silo_a_pnl = json.dumps(pnl_pivot['U23144948'].fillna(0).tolist() if 'U23144948' in pnl_pivot else [0]*len(full_dates))
            silo_b_pnl = json.dumps(pnl_pivot['U23139264'].fillna(0).tolist() if 'U23139264' in pnl_pivot else [0]*len(full_dates))
            silo_c_pnl = json.dumps(pnl_pivot['U23154199'].fillna(0).tolist() if 'U23154199' in pnl_pivot else [0]*len(full_dates))
            silo_d_pnl = json.dumps(pnl_pivot['U25218481'].fillna(0).tolist() if 'U25218481' in pnl_pivot else [0]*len(full_dates))

        print("Data successfully parsed. Global and Silo metrics calculated.")
    except Exception as e:
        print(f"Error parsing CSV: {e}")

# 3.d Benchmark Parsing & Integration (TWR Math & Dollar PnL Math)
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        # Theoretical DOLLAR PnL Logic (The Replicator Engine)
        ...
        # JSON Dumps for Bulletproof JS Array Extraction
        dates_js = json.dumps(master_bench['Date'].dt.strftime('%Y-%m-%d').tolist())
        spy_js = json.dumps((master_bench['SPY_Cum'] * 100).fillna(0).round(2).tolist())
        qqq_js = json.dumps((master_bench['QQQ_Cum'] * 100).fillna(0).round(2).tolist())
        estate_js = json.dumps((master_bench['Estate_Cum'] * 100).fillna(0).round(2).tolist())
        
        silo_a_js = json.dumps((master_bench['U23144948_Cum'] * 100).fillna(0).round(2).tolist())
        silo_b_js = json.dumps((master_bench['U23139264_Cum'] * 100).fillna(0).round(2).tolist())
        silo_c_js = json.dumps((master_bench['U23154199_Cum'] * 100).fillna(0).round(2).tolist())
        silo_d_js = json.dumps((master_bench['U25218481_Cum'] * 100).fillna(0).round(2).tolist())

        dates_cum_pnl_js = json.dumps(master_bench['Date'].dt.strftime('%Y-%m-%d').tolist())
        estate_cum_pnl_js = json.dumps(master_bench['Estate_Cum_PnL'].round(2).tolist())
        spy_cum_pnl_js = json.dumps(master_bench['SPY_Cum_PnL'].round(2).tolist())
        qqq_cum_pnl_js = json.dumps(master_bench['QQQ_Cum_PnL'].round(2).tolist())
...
<head>
    ...
    <title>Family Estate Master Allocator v24</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v24</p>
        </div>
    ...
    <script>
        ...
        function updateUI() {
            ...
            const estateCumPnl = VAR_ESTATE_CUM_PNL;
            const spyCumPnl = VAR_SPY_CUM_PNL;
            const qqqCumPnl = VAR_QQQ_CUM_PNL;
            const datesCumPnl = VAR_DATES_CUM_PNL;
...
# Inject Theoretical Cumulative Dollar PnL
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v24 created.")
