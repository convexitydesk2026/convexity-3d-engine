"""
=============================================================================
Script Name: sync_engine_v27.py
Purpose: Phase 8 of the Local-First Estate Architecture.
         - UPDATED: Fixed IBKR API Error 321. Converted Tick 106 to streaming.
=============================================================================
"""
...
def sync_tws():
    print("========================================================")
    print("   ESTATE SYNC ENGINE v27 - WAL MODE & IV SCANNER")
    print("========================================================")    
    ...
    for acc in accounts:        
        ...
        try:
            ...
            try: ib.reqAccountUpdates(acc)
            except Exception: print(f"[!] Warning: {acc} timed out.")
        ...
        for v in values:
            if v.tag in ['NetLiquidation', 'NetLiquidationByCurrency'] and v.currency == 'USD': net_liq_usd = float(v.value)
            elif v.tag in ['TotalCashBalance', 'CashBalance'] and v.currency == 'USD': total_cash_usd = float(v.value)
            elif v.tag in ['AccruedCash', 'AccruedDividend'] and v.currency == 'USD': accrued_cash_usd = float(v.value)
            elif v.tag == 'AvailableFunds' and v.currency == 'USD': avail_funds_usd = float(v.value)
                
        cursor.execute("INSERT INTO daily_balances VALUES (?, ?, ?, ?, ?, ?)", (today_str, acc, 'USD', net_liq_usd, 0.0, avail_funds_usd))
        cursor.execute("INSERT INTO daily_positions (date, account, symbol, sec_type, position, market_price, market_value, avg_cost, unrealized_pnl, realized_pnl, currency) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                       (today_str, acc, 'USD CASH', 'CASH', total_cash_usd, 1.0, total_cash_usd, total_cash_usd, 0.0, 0.0, 'USD'))

        if accrued_cash_usd != 0: 
            cursor.execute("INSERT INTO daily_positions (date, account, symbol, sec_type, position, market_price, market_value, avg_cost, unrealized_pnl, realized_pnl, currency) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                           (today_str, acc, 'ACCRUED INTEREST/CFD FEES', 'CASH', accrued_cash_usd, 1.0, accrued_cash_usd, accrued_cash_usd, 0.0, 0.0, 'USD'))
    ...
    # --- FIXED: ALPHA WATCHLIST IV SCANNER ---
    ...
    if watchlist:
        ...
        try:
            ...
            # API FIX: Downgrade to "Delayed-Frozen" (Type 4) data.
            # This completely bypasses Error 10089 for users without paid live data subscriptions.
            ib_scanner.reqMarketDataType(4)
            ...
            for row in watchlist:
                ...
                try:
                    ...
                    # API FIX: snapshot=False for streaming generic ticks
                    ticker = ib_scanner.reqMktData(contract, genericTickList='106', snapshot=False)
                    ib_scanner.sleep(2.0) # Wait for live tick to propagate
                    
                    live_iv = 0.0
                    if ticker.impliedVolatility and str(ticker.impliedVolatility).lower() != 'nan':
                        live_iv = float(ticker.impliedVolatility) * 100 
                        
                    ib_scanner.cancelMktData(contract) # Gracefully kill subscription
