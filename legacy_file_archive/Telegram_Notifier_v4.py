"""
=============================================================================
Script Name: Telegram_Notifier_v4.py
Purpose: Headless Streamlit rendering and Telegram transmission.
         - Starts the dashboard in the background invisibly (No Cloud).
         - Waits 30 seconds to guarantee Yahoo Finance and Monte Carlo finish.
         - Snaps a lossless 4K continuous PNG and delivers as a Telegram Document.
=============================================================================
"""
...
def snap_and_send():
    ...
    # Give the local Streamlit server 12 seconds to fully boot up
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # device_scale_factor=2 guarantees crisp 4K Retina resolution text/charts
            ...
            # THE FIX: Wait 30 full seconds for YFinance to download and Monte Carlo to run
            print("[NOTIFIER] Waiting 30 seconds for YFinance and Monte Carlo to render...")
            page.wait_for_timeout(30000)
            
            print("[NOTIFIER] Snapping 4K Full-Page Screenshot...")
            page.screenshot(path=REPORT_PATH, full_page=True)
            browser.close()
            
        print("[NOTIFIER] Transmitting Lossless 4K Document to Telegram...")
        ...
        # ALWAYS kill the Streamlit background process so it doesn't eat RAM
