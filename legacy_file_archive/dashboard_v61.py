...
3. Launch the UI by running exactly:
   streamlit run dashboard_v61.py

CHANGELOG (v61):
         - PATCHED: "Expiration Collision Bug" in margin calculator (added base_tckr to groupby).
         - PATCHED: Macro Core Equities Tracker now recognizes Physical US & Int'l Stocks.
=============================================================================
"""
...
def get_exact_opt_margin(df_in):
    ...
    try:
        ...
        # v61 FIX: Added base_tckr to the groupby!
                ...
                try:
                    ...
                    if len(parts) >= 4:
                        ...
                        if right == 'P' and dte > 60:
                            # v61: Broad Market Whitelist prevents Decapitation Bug for Sector Shorts (like SMH)
...
# v61 FIX: Added Physical US Stocks & International Stocks to Core Assets pool
...
if not journal_raw_df.empty:
    ...
    if not active_vrp.empty:
        ...
        # v61: Dynamically map LIVE TWS PnL to calculate Live Annualized ROC
        live_roc_list = []
        live_ann_roc_list = []
        ...
        with tab4:
            ...
            if not cat_df.empty:
                ...
                for _, r in cat_df.iterrows():
                    names.append(f"{r['Ticker']} {int(r.get('Long Strike',0))} (DTE {r['Days Remaining']})")
                    cost = abs(r.get('Premium Collected (USD)', 0) * 100 * r.get('Quantity', 1))
                    costs.append(-cost)
                    try:
                        S, V = fetch_live_data(r['Ticker'])
                        S_crash = S * 0.70
                        V_crash = min(V * 2.5, 0.80)
                        dte_rem = max(float(r['Days Remaining'])/365.0, 0.001)
                        is_debit = r.get('Premium Collected (USD)', 0) < 0 and r.get('Short Strike', 0) > 0
                        if is_debit:
                            s_p, _, _, _, _ = get_put_greeks(S_crash, r['Short Strike'], dte_rem, LIVE_RF_RATE, V_crash)
                            l_p, _, _, _, _ = get_put_greeks(S_crash, r['Long Strike'], dte_rem, LIVE_RF_RATE, V_crash)
                            payout = (s_p - l_p) * 100 * r['Quantity']
                        else:
                            l_p, _, _, _, _ = get_put_greeks(S_crash, r['Long Strike'], dte_rem, LIVE_RF_RATE, V_crash)
                            payout = l_p * 100 * r['Quantity']
                        payouts.append(max(0, payout - cost))
                    except: payouts.append(0)
...
if not journal_raw_df.empty:
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            # v61: 3D Engine Native Debit Spread & Call Detection
