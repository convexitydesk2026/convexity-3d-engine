"""
=============================================================================
Script Name: sync_engine_v7.py
Purpose: Phase 1 of the Local-First Estate Architecture.
         Properly cycles the IBKR API single-stream limit with cool-downs.
=============================================================================
"""
...
def sync_tws():
    print("========================================================")
    print("     ESTATE SYNC ENGINE v7 - SECURE SEQUENTIAL SYNC")
    print("========================================================")
    ...
    for acc in accounts:
        ...
        # 1. Open the data faucet
        print("[*] Opening data stream (waiting 3 seconds for packets)...")
        ib.client.reqAccountUpdates(True, acc)
        ib.sleep(3.0) 
        
        # 2. Read the populated cache
        values = ib.accountValues(acc)
        portfolio_items = ib.portfolio(acc)
        
        # 3. Close the data faucet
        ib.client.reqAccountUpdates(False, acc)
        
        # 4. CRITICAL COOL-DOWN: Wait for TWS to register the close command
        #    before moving to the next account (bypasses IBKR's 1-stream limit)
        ib.sleep(1.5)

        # Process Balances (Checking all possible IBKR tags)
        net_liq = 0.0
        total_cash = 0.0
        for v in values:
            if v.tag in ['NetLiquidation', 'NetLiquidationByCurrency'] and v.currency in ['BASE', 'USD']:
                net_liq = float(v.value)
            elif v.tag in ['TotalCashBalance', 'CashBalance'] and v.currency in ['BASE', 'USD']:
