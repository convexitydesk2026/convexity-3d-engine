r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v60.py
Purpose: Generates the Interactive Family Estate Dashboard.
         ...
         - Hardcoded filename output to v60.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v60.html") # v60 CHANGE
...
def process_metrics(df_acc):
    ...
    # --- v60 FIX: Eliminate the "Dead Air" penalty for Volatility Metrics ---
...
print("\n========================================================")
print("             CIO DIAGNOSTICS & LOGGING (v60)            ") # v60 CHANGE
print("========================================================")
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        # --- v60 ALIGNMENT LOGIC: Find the exact day actual trading started ---
...
<head>
    ...
    <title>Estate Master Dashboard v60</title> <!-- v60 CHANGE -->
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8 relative">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v60</p>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <!-- ========================================================= -->
            <!-- PANEL 6: DEPLOYMENT & CAPACITY TRACKER (v60 UPGRADE)      -->
            <!-- ========================================================= -->
            <div class="xl:col-span-12 mt-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 relative overflow-hidden flex flex-col justify-between">
                            <div>
                                ...
                                <!-- v60 DYNAMIC DEPLOYMENT SCHEDULE -->
    ...
    <script>
        ...
        function updateUI() {
            ...
            // =========================================================
            // PANEL 6 LOGIC (Deployment Tracker - v60 UPGRADE)
            // =========================================================
            ...
            // v60: Dynamic Tranche Calculator & Date Generator
            ...
            // v60 PnL Attribution Donut Plot
            ...
            if(Object.keys(pnlAttrData).length > 0) {
                ...
                let attrValues = Object.values(pnlAttrData).map(v => Math.max(v, 0)); // Only show positive contributions
                ...
                let donutData =[{
                    ...
                    marker: { colors: attrLabels.map(l => attrColors[l]) }
                }];
                let donutLayout = {
        ...
        if (Object.keys(liveAlloc).length > 0) {
            ...
            if (liveAlloc['A']) {
                ...
                document.getElementById('a_xsp').value = liveAlloc['A']['OPT_MARGIN'] || 0; // v60 CHANGE
            }
            ...
            if (liveAlloc['C']) {
                ...
                document.getElementById('c_xsp').value = liveAlloc['C']['OPT_MARGIN'] || 0; // v60 CHANGE
            }
        }
...
# --- v60 CAPITAL VELOCITY INJECTION ---
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v60 created.")
