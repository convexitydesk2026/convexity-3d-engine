r"""
=============================================================================
Script Name: Estate_Orchestrator_v19.py
...
def process_ibkr_data(sections):
    ...
    # 4. Live Allocations (WITH MULTI-DAY EXTERMINATOR BUG FIX)
    live_pos = {
    ...
    if pos_df is not None:
        ...
        if pos_acc_col and sym_col:
            ...
            pos_df = pos_df.drop_duplicates(subset=[pos_acc_col, sym_col], keep='last')
            ...
            for _, row in pos_df.iterrows():
                ...
                val_col = get_col(pos_df,['PositionValueInBase', 'Position Value in Base', 'PositionValue', 'Position Value', 'Value'])
                ...
                try: val = float(row.get(val_col, 0)) if val_col else 0.0
                except: val = 0.0
                
                fx = 1.0
                if fx_col and pd.notna(row.get(fx_col)) and str(row.get(fx_col)).strip() != '':
                    if val_col and 'base' not in val_col.lower():
                        try: fx = float(row.get(fx_col, 1.0))
                        except: fx = 1.0
...
if __name__ == "__main__":
    print("========================================================")
    print("     ESTATE ORCHESTRATOR v19 - INITIALIZING SYNC...      ")
    print("========================================================")
