...
if not journal_raw_df.empty:
    ...
    if not active_vrp.empty:
        ...
        with tab1:
            ...
            if not vrp_df.empty:
                ...
                fig1 = go.Figure(go.Scatter(
                    ...
                    # PATCH: Added cmid=0 to anchor the color scale so positive is ALWAYS green and negative is ALWAYS red.
        ...
        with tab3:
            ...
            if not sb_df.empty:
                # 1. Generate a stylized Theta decay curve (Value is proportional to the square root of time)
                days_curve = np.linspace(180, 0, 180)
                theta_curve = np.sqrt(days_curve / 180) * 100

                fig3 = go.Figure()
                
                # 2. Plot the didactic background decay curve
                ...
                # 4. Plot the actual Synthetic Beta position on the slope!
                sb_df['y_pos'] = np.sqrt(sb_df['Days Remaining'] / 180) * 100
                
                fig3.add_trace(go.Scatter(
                    x=sb_df['Days Remaining'], y=sb_df['y_pos'], mode='markers+text',
                    text=sb_df['Ticker'] + ' ' + sb_df['Long Strike'].astype(str), textposition="top center",
                    marker=dict(size=25, color='#2563eb', symbol='diamond', line=dict(width=2, color='black')),
                    hovertemplate="<b>%{text}</b><br>Days Rem: %{x}<br>Est. Time Value Retained: %{y:.1f}%<extra></extra>",
