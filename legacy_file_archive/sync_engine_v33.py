"""
=============================================================================
Script Name: sync_engine_v33.py
Purpose: Phase 10 of the Local-First Estate Architecture.
...
def sync_tws():
    ...
    try:
        ...
        # 2. Auto-Promote to 'Open' if a physical position is found
        cursor.execute("""
            UPDATE alpha_campaigns 
            SET status = 'Open 🟢', open_date = ?, entry_price = (SELECT avg_cost FROM daily_positions WHERE daily_positions.symbol = alpha_campaigns.symbol LIMIT 1)
        ...
        # If it's already Open, but the TWS position size is larger than what we previously logged (requires UI logic, but we can flag it here)
        cursor.execute("""
            UPDATE alpha_campaigns 
            SET tranche_added = 1, entry_price = (SELECT avg_cost FROM daily_positions WHERE daily_positions.symbol = alpha_campaigns.symbol LIMIT 1)
            WHERE status = 'Open 🟢' 
            AND symbol IN (SELECT symbol FROM daily_positions WHERE position > 0 AND sec_type IN ('STK', 'CFD'))
        """)
