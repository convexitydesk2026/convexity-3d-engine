...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v16.py")
...
def load_and_process_data():
    ...
    pos_df = pd.read_sql_query(f"SELECT account, symbol, sec_type, position, market_value, avg_cost FROM daily_positions WHERE date = '{live_date.strftime('%Y-%m-%d')}'", conn)
...
if not pos_df.empty:
    ...
    th_deployed = (th_df['position'].abs() * th_df['avg_cost'] * 100).sum()
else:
    th_deployed = 0
...
if not attr_df.empty:
    ...
    th_deployed = (th_df_vel['position'].abs() * th_df_vel['avg_cost'] * 100).sum() if not th_df_vel.empty else 0
    th_available = th_budget - th_deployed
...
with c_action:
    if distance_usd <= 1000:
        ...
        # --- CEILING GOVERNOR LOGIC (v33 Proportional Overflow) ---
        ...
        if str(raw_dte) == 'Closed':
            ...
            with col_2d:
                ...
                <div style="background-color: {bg_css}; padding: 15px; border-radius: 8px; border: 1px solid {color_css}; text-align: center; margin-bottom: 20px;">
                    ...
                    <span style="font-size: 12px; color: #4b5563;">Spread Value: ${curr_spread_price:.2f} | Net Theta: ${net_theta:+.2f}/day | Underlying: ${S_live:,.2f} | IV Override: {iv_override:.1f}%</span>
                </div>
                """.replace('\n', ''), unsafe_allow_html=True)
            ...
            with col_3d:
                ...
                # MAIN SURFACE (Native contour removed, relying on explicit line calculation below)
                ...
                # --- v33 ENHANCEMENT: Intersection Curves (Wireframing the Kill Box) ---
                z_green, z_red = [],[]
                for d in y_3d:
                ...
                fig_3d.add_trace(go.Surface(
                    ...
                    colorscale=[[0, 'green'],[1, 'green']], opacity=0.15, showscale=False, hoverinfo='skip'
                ))
                fig_3d.add_trace(go.Surface(
                    x=[[K_l, K_l],[K_l, K_l]], y=[[0, init_dte],[0, init_dte]], z=[[z_min, z_min],[z_max, z_max]],
                    colorscale=[[0, 'red'],[1, 'red']], opacity=0.15, showscale=False, hoverinfo='skip'
                ))
                fig_3d.add_trace(go.Surface(
                    x=[[x_vals[0], x_vals[-1]], [x_vals[0], x_vals[-1]]], 
                    y=[[half_dte, half_dte],[half_dte, half_dte]], 
                    z=[[z_min, z_min],[z_max, z_max]],
                    colorscale=[[0, 'yellow'],[1, 'yellow']], opacity=0.20, showscale=False, hoverinfo='skip'
                ))
                fig_3d.add_trace(go.Surface(
                    x=[[x_vals[0], x_vals[-1]],[x_vals[0], x_vals[-1]]],
                ...
                fig_3d.add_trace(go.Scatter3d(
                    x=[S_live, S_live], y=[curr_dte, curr_dte], z=[0, unrealized_pnl], 
