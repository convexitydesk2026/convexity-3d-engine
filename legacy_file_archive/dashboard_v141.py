...
streamlit run dashboard_v141.py
CHANGELOG (v141):
- STAGE 49 (v141): Upgraded Staging Area with multi-select tag dropdowns, enabled dynamic row deletions, and synced Dashboard IV alerts with Telegram logic.
    ...
    with c_add: 
        ...
        if st.form_submit_button("Add to Stalk List", use_container_width=True):
            if new_stalk_sym:
                ...
                cj.execute("INSERT INTO alpha_campaigns (symbol, status, tags, thesis) VALUES (?, 'Stalking 🔭', ?, ?)", (new_stalk_sym, tags_str, new_stalk_thes))
                conn_journal.commit()
                st.rerun()
...
if not df_staging.empty:
    ...
    def commit_staging_edits():
        state = st.session_state["staging_editor"]
        edits = state.get("edited_rows", {})
        deletions = state.get("deleted_rows", [])
    ...
    st.data_editor(
        ...
        hide_index=True, width="stretch", num_rows="dynamic", key="staging_editor", on_change=commit_staging_edits
    )
