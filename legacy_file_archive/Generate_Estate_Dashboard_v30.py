r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v30.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 30 UPGRADES (CORRECTED LAUNCHPAD): 
         - Retains ALL v29 UI enhancements (Pastel harmonization, thick 6px/4.5px lines,
           disabled ModeBars, Crimson QQQ, Tax Treatment Matrix column).
         - Integrates "Estate Montecarlo PnL Simulation (10,000 runs)".
         - Uses Vectorized NumPy for instant probability cone generation.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v30.html")
...
# Adjusted to default to 'light' for dark text readability on our new pastel backgrounds
...
if os.path.exists(csv_file_path):
    try:
        ...
        # Calculate Daily PnL Histogram Data
...
# 3.d Benchmark Parsing & Integration
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        print("[OK] Benchmark Data Alignment: SUCCESS.")
        
        # ---------------------------------------------------------
        # 3.e The Vectorized Monte Carlo Engine (10,000 runs)
        # ---------------------------------------------------------
        print("[INFO] Running 10,000-iteration Estate Monte Carlo Simulation...")
        ...
        if sim_length > 0:
            ...
            print("[OK] Monte Carlo Engine: SUCCESS (Vectorized completion).")
        else:
            print("[WARN] Monte Carlo Engine: FAILED (No data length).")
            
    else:
        print("[WARN] Benchmark Data Alignment: FAILED. No overlapping dates found.")
...
<head>
    ...
    <title>Estate Master Dashboard v30</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v30</p>
        </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <div class="xl:col-span-6 flex flex-col gap-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-4">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <div class="bg-purple-300 text-gray-900 p-4 rounded-xl shadow-md text-center border-b-4 border-purple-400 flex-grow">
            ...
            <div class="xl:col-span-6 flex flex-col gap-6">
                ...
                <div class="bg-white p-4 rounded-xl border shadow-sm flex-grow">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                        ...
                        <!-- Silo A -->
                        <div class="bg-blue-50 p-3 rounded-lg border border-blue-400 shadow-inner">
                            ...
                            <label class="text-xs font-semibold text-gray-900" id="a_ib01_label">IB01: </label><input type="range" id="a_ib01" min="0" max="100" step="0.01" value="100.0" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="a_cspx_label">CSPX: </label><input type="range" id="a_cspx" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="a_cndx_label">CNDX: </label><input type="range" id="a_cndx" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="a_crypto_label">Crypto ETPs: </label><input type="range" id="a_crypto" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="a_cash_label">Cash: </label><input type="range" id="a_cash" min="-50" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            
                            <label class="text-xs font-bold text-red-600" id="a_opt_mkt_label">Opt. Mkt Val (Liability): </label><input type="range" id="a_opt_mkt" min="-20" max="0" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            ...
                            <label class="text-xs font-bold text-green-700" id="a_xsp_label">Options Margin: </label><input type="range" id="a_xsp" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                        </div>

                        <!-- Silo B -->
                        <div class="bg-purple-50 p-3 rounded-lg border border-purple-400 shadow-inner">
                            <h4 class="font-black text-gray-900 mb-2 text-sm border-b border-purple-400 pb-1">Silo B</h4>
                            <p class="text-[10px] font-bold text-gray-800 mb-1 uppercase tracking-wide">Physical Balance Sheet</p>
                            <label class="text-xs font-semibold text-gray-900" id="b_ib01_label">IB01: </label><input type="range" id="b_ib01" min="0" max="100" step="0.01" value="41.15" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="b_cash_label">Cash: </label><input type="range" id="b_cash" min="-50" max="100" step="0.01" value="58.85" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="b_cfd_label">CFDs: </label><input type="range" id="b_cfd" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="b_intl_label">Intl Stocks: </label><input type="range" id="b_intl" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            <p class="text-[10px] text-red-600 font-bold mt-1 h-4" id="b_warning"></p>
                        </div>

                        <!-- Silo C -->
                        <div class="bg-green-50 p-3 rounded-lg border border-green-400 shadow-inner">
                            <h4 class="font-black text-gray-900 mb-2 text-sm border-b border-green-400 pb-1">Silo C</h4>
                            <p class="text-[10px] font-bold text-gray-800 mb-1 uppercase tracking-wide">Physical Balance Sheet</p>
                            <label class="text-xs font-semibold text-gray-900" id="c_ib01_label">IB01: </label><input type="range" id="c_ib01" min="0" max="100" step="0.01" value="100.0" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="c_cspx_label">CSPX: </label><input type="range" id="c_cspx" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="c_cndx_label">CNDX: </label><input type="range" id="c_cndx" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="c_crypto_label">Crypto ETPs: </label><input type="range" id="c_crypto" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            <label class="text-xs font-semibold text-gray-900" id="c_cash_label">Cash: </label><input type="range" id="c_cash" min="-50" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            
                            <label class="text-xs font-bold text-red-600" id="c_opt_mkt_label">Opt. Mkt Val (Liability): </label><input type="range" id="c_opt_mkt" min="-20" max="0" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            ...
                            <label class="text-xs font-bold text-green-700" id="c_xsp_label">Options Margin: </label><input type="range" id="c_xsp" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                        </div>

                        <!-- Silo D -->
                        <div class="bg-yellow-50 p-3 rounded-lg border border-yellow-400 shadow-inner">
            ...
            <!-- DAILY PNL HISTOGRAM (12 Columns wide) -->
            ...
            <!-- NEW MONTE CARLO CHART (12 Columns wide) -->
            <div class="xl:col-span-12 mt-2">
                <div class="bg-white border rounded-xl p-4 shadow-sm overflow-hidden">
    ...
    <script>
        ...
        // NATIVE JS PARSER: Completely immune to Syntax Errors
        function safeParse(jsonStr) {
            try {
                if (!jsonStr || jsonStr.startsWith('VAR_')) return[]; 
                return JSON.parse(jsonStr);
            } catch (e) {
                console.error("Data parse shielded. Reason:", e);
                return[];
            }
        }
        
        // Harmonized Silo Palette
        const palette = {
            siloA: '#93c5fd', // blue-300
            siloB: '#d8b4fe', // purple-300
            siloC: '#86efac', // green-300
            siloD: '#fde047', // yellow-300
            cspx: '#f97316', cndx: '#8b5cf6', 
            cfd_intl: '#a855f7', optMktVal: '#ef4444', crypto: '#0ea5e9'
        };

        const instruments =[
            { inst: "USD Cash", type: "Currency", risk: "Risk-Free", alpha: "Zero", sharpe: "Stabilizer", strat: "Liquidity", jur: "US (IBKR)", tax: "Exempt (Bank Deposit)", min: "1%", max: "100%", rec: "Splendid", class: "splendid", comm: "Uninvested USD held in IBKR. Mandatory margin collateral." },
            { inst: "IB01", type: "UCITS ETF", risk: "Risk-Free", alpha: "Zero", sharpe: "High", strat: "Collateral", jur: "Ireland", tax: "Exempt (Irish Domicile)", min: "10%", max: "100%", rec: "Splendid", class: "splendid", comm: "Irish-domiciled short-term US Treasury fund. Accumulates ~4.5% tax-free." },
            { inst: "XSP Put Spreads", type: "Index Option", risk: "Moderate", alpha: "High (VRP)", sharpe: "High", strat: "Weekly Income", jur: "US (Cboe)", tax: "Exempt (Cash-Settled)", min: "0%", max: "25%", rec: "Splendid", class: "splendid", comm: "Cash-settled S&P 500 options. 100% safe from IRS." },
            { inst: "XND Put Spreads", type: "Index Option", risk: "Mod/High", alpha: "High", sharpe: "Moderate", strat: "Satellite Income", jur: "US (Cboe)", tax: "Exempt (Cash-Settled)", min: "0%", max: "10%", rec: "Great", class: "great", comm: "Micro-Nasdaq 100. Cash-settled. IRS Safe. Higher volatility than XSP." },
            { inst: "CSPX", type: "UCITS ETF", risk: "Moderate", alpha: "Zero", sharpe: "Baseline", strat: "Long Term", jur: "Ireland", tax: "Exempt (Irish Domicile)", min: "0%", max: "60%", rec: "Great", class: "great", comm: "Irish-domiciled S&P 500. Shields against 40% Estate Tax." },
            { inst: "CNDX", type: "UCITS ETF", risk: "Aggressive", alpha: "High", sharpe: "Moderate", strat: "Long Term", jur: "Ireland", tax: "Exempt (Irish Domicile)", min: "0%", max: "40%", rec: "Great", class: "great", comm: "Irish-domiciled Nasdaq 100. Shields against 40% Estate Tax. High beta tech exposure." },
            { inst: "BTC/ETH ETPs", type: "Crypto ETP", risk: "Aggressive", alpha: "High", sharpe: "Volatile", strat: "Uncorrelated", jur: "Europe (Jersey/CH)", tax: "Exempt (Offshore Wrapper)", min: "0%", max: "5%", rec: "Good", class: "good", comm: "Offshore crypto wrappers (e.g. CoinShares). IRS safe spot exposure." },
            { inst: "US Tech CFDs", type: "OTC Contract", risk: "Aggressive", alpha: "High", sharpe: "Negative", strat: "Swing Trading", jur: "UK/Offshore", tax: "Exempt (OTC Derivative)", min: "0%", max: "3%", rec: "Good", class: "good", comm: "Synthetic derivatives. 0% IRS risk. Quarantined to Silo B." },
            { inst: "/MES Put Spreads", type: "Futures Option", risk: "Moderate", alpha: "Highest (SPAN)", sharpe: "High", strat: "Capital Efficiency", jur: "US (CME)", tax: "Exempt (Section 1256/Cash)", min: "0%", max: "25%", rec: "Contingent", class: "contingent", comm: "Contingent on mastering XSP mechanics. SPAN margin halves collateral, doubling ROC." },
            { inst: "Managed Futures (CTAs)", type: "UCITS Fund", risk: "Moderate", alpha: "Crisis Alpha", sharpe: "High (Uncorrelated)", strat: "Trend Following", jur: "Ireland", tax: "Exempt (Irish Domicile)", min: "0%", max: "15%", rec: "Contingent", class: "contingent", comm: "Contingent on risk tolerance change. Shorts commodities & bonds to protect during crashes." },
            { inst: "XSP LEAPS", type: "Index Option", risk: "Aggressive", alpha: "Low", sharpe: "Negative", strat: "Leverage", jur: "US (Cboe)", tax: "Exempt (Cash-Settled)", min: "0%", max: "0%", rec: "Bad", class: "bad", comm: "IRS safe, but mathematical drag of Theta and lost dividends destroys edge." },
            { inst: "Physical US Stocks", type: "Stock", risk: "Extreme", alpha: "High", sharpe: "Baseline", strat: "Swing", jur: "US", tax: "LETHAL (40% Estate Tax)", min: "0%", max: "0%", rec: "Avoid", class: "avoid", comm: "LETHAL. Triggers 40% US Estate Tax and 30% Dividend Withholding." },
            { inst: "US Spot BTC/ETH", type: "US ETF", risk: "Extreme", alpha: "N/A", sharpe: "N/A", strat: "N/A", jur: "US", tax: "LETHAL (40% Estate Tax)", min: "0%", max: "0%", rec: "Avoid", class: "avoid", comm: "LETHAL. Standard ETFs (IBIT/FBTC) are US-situs property. Will trigger Estate Tax confiscation." },
            { inst: "TQQQ", type: "Physical ETF", risk: "Extreme", alpha: "Negative", sharpe: "Negative", strat: "Speculation", jur: "US", tax: "LETHAL (40% Estate Tax)", min: "0%", max: "0%", rec: "Avoid", class: "avoid", comm: "LETHAL. Widow-maker. Combines IRS Tax Trap with massive Beta Slippage decay." }
        ];
        ...
        function updateUI() {
            ...
            // SMART BAR CHART LOGIC (Harmonized Colors)
            let barData =[
            ...
            Plotly.react('bar-chart', barData, barLayout, {responsive: true, displayModeBar: false});
            ...
            Plotly.react('pie-chart', pieData, pieLayout, {responsive: true, displayModeBar: false});
            ...
            const siloAData = safeParse('VAR_SILOA_CUM');
            const siloBData = safeParse('VAR_SILOB_CUM');
            const siloCData = safeParse('VAR_SILOC_CUM');
            const siloDData = safeParse('VAR_SILOD_CUM');

            if (benchDates && benchDates.length > 0) {
                // Global Chart
                let traceEstate = { x: benchDates, y: estateData, name: 'Estate (Cum %)', type: 'scatter', mode: 'lines', line: {color: 'black', width: 6} };
                let traceSPY = { x: benchDates, y: spyData, name: 'SPY (Cum %)', type: 'scatter', mode: 'lines', line: {color: '#3b82f6', width: 2} }; 
                let traceQQQ = { x: benchDates, y: qqqData, name: 'QQQ (Cum %)', type: 'scatter', mode: 'lines', line: {color: '#dc2626', width: 2} }; 
                ...
                Plotly.react('benchmark-chart',[traceEstate, traceSPY, traceQQQ], benchLayout, {responsive: true, displayModeBar: false});
                ...
                let trSiloA = { x: benchDates, y: siloAData, name: 'Silo A', type: 'scatter', mode: 'lines', line: {color: 'black', width: 4.5}, showlegend: false };
                Plotly.react('chart-silo-a',[trSiloA, trSPY_sm, trQQQ_sm], layoutSmall, {responsive: true, displayModeBar: false});

                let trSiloB = { x: benchDates, y: siloBData, name: 'Silo B', type: 'scatter', mode: 'lines', line: {color: 'black', width: 4.5}, showlegend: false };
                Plotly.react('chart-silo-b',[trSiloB, trSPY_sm, trQQQ_sm], layoutSmall, {responsive: true, displayModeBar: false});

                let trSiloC = { x: benchDates, y: siloCData, name: 'Silo C', type: 'scatter', mode: 'lines', line: {color: 'black', width: 4.5}, showlegend: false };
                Plotly.react('chart-silo-c',[trSiloC, trSPY_sm, trQQQ_sm], layoutSmall, {responsive: true, displayModeBar: false});

                let trSiloD = { x: benchDates, y: siloDData, name: 'Silo D', type: 'scatter', mode: 'lines', line: {color: 'black', width: 4.5}, showlegend: false };
                Plotly.react('chart-silo-d',[trSiloD, trSPY_sm, trQQQ_sm], layoutSmall, {responsive: true, displayModeBar: false});
            }
            ...
            if (datesPnl && datesPnl.length > 0) {
                ...
                Plotly.react('daily-pnl-chart',[traceA, traceB, traceC, traceD, traceEstateLine, traceSPYLine, traceQQQLine], pnlLayout, {responsive: true, displayModeBar: false});
            }
            
            // ---------------------------------------------------------
            // MONTE CARLO ENGINE LOGIC
            // ---------------------------------------------------------
            const mcVisuals = safeParse('VAR_MC_VISUALS');
            const mcAvg = safeParse('VAR_MC_AVG');
            ...
            if (mcOrig && mcOrig.length > 0) {
                ...
                // 100 Random Visual Spaghetti Lines
                mcVisuals.forEach(trace => {
                    ...
                    mcTraces.push({
                        ...
                        type: 'scatter',
                        mode: 'lines',
                        line: {color: `rgba(${r}, ${g}, ${b}, 0.2)`, width: 1},
                        showlegend: false,
                        hoverinfo: 'skip'
                    });
                });
                
                // Average Expected Path
                mcTraces.push({
                    y: mcAvg,
                    name: 'Avg Path (Stable)',
                    type: 'scatter',
                    mode: 'lines',
                ...
                Plotly.react('montecarlo-chart', mcTraces, mcLayout, {responsive: true, displayModeBar: false});
            }
        }
...
# Inject Native JS Arrays via string shielding
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v30 created.")
