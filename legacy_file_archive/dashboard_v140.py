...
streamlit run dashboard_v140.py
CHANGELOG (v140):
- STAGE 48 (v140): Implemented the Pre-Trade Staging Area (Stalking/Waiting Pipeline), Pre-Flight Risk Budget, and 5-Day Earnings Blackout flags.
- STAGE 47 (v139): Rewrote the Macro Regime Filter to track RSP (Equal Weight S&P 500) against its 50-day and 200-day SMAs, eliminating the "skittish" 10-day SPY whipsaw effect.
- STAGE 46 (v138): Injected a CSV export button for Active Campaigns to allow the CIO to download and review open positions, theses, and risk metrics dynamically.
- STAGE 45 (v137): Updated internal subprocess calls to trigger the new market_flow_engine_v5.py architecture.
- STAGE 44 (v136): Fixed Python 3.12+ SyntaxWarning for invalid escape sequences by double-escaping the dollar signs in the Active Campaigns markdown string.
- STAGE 43 (v135): Fixed a Markdown parsing bug where multiple dollar signs in the Active Campaigns panel were interpreted as LaTeX math formulas, causing font and color inconsistencies.
- STAGE 42 (v134): Upgraded the Strategy Tag Glossary into a fully interactive Tag Manager. Added a dedicated text input form to create new tags and enabled dynamic row deletion in the data editor to permanently prune unused tags.
- STAGE 41 (v133): The Great UI Compaction. Formatted large deep-dive panels into collapsible expanders to reduce scroll fatigue. Removed 'use_container_width' to silence Streamlit deprecation warnings. Simplified the Tag Engine by removing mathematical auto-tags to eliminate database clutter.
...
with st.sidebar:
    ...
    if st.button("🌐 Generate Market Flow Report", width="stretch"):
        with st.spinner("Calculating Institutional Flow & Generating PDF... Please wait (~15s)"):
            try:
                subprocess.run(["python", "market_flow_engine_v5.py"], cwd=TARGET_DIR, check=True)
...
st.markdown("##### 🔭 Pre-Trade Staging Area (Pipeline)")

# Add New Stalking Candidate UI
with st.form("add_stalk_form", clear_on_submit=True):
    c_sym, c_tags, c_thes, c_add = st.columns([1, 2, 3, 1])
    with c_sym: new_stalk_sym = st.text_input("Ticker", placeholder="e.g. MU").upper()
    with c_tags: new_stalk_tags = st.text_input("Tags", placeholder="e.g. Q-EP, UpVol")
    ...
    with c_add: 
        ...
        if st.form_submit_button("Add to Stalk List", use_container_width=True):
            if new_stalk_sym:
                cj.execute("INSERT INTO alpha_campaigns (symbol, status, tags, thesis) VALUES (?, 'Stalking 🔭', ?, ?)", (new_stalk_sym, new_stalk_tags, new_stalk_thes))
                conn_journal.commit()
                st.rerun()

df_staging = pd.read_sql_query("SELECT id, symbol, status, target_entry, planned_stop, tags, thesis FROM alpha_campaigns WHERE status IN ('Stalking 🔭', 'Waiting ⏳', 'Armed 🎯')", conn_journal)
...
if not df_staging.empty:
    ...
    def commit_staging_edits():
        edits = st.session_state["staging_editor"].get("edited_rows", {})
        if not edits: return
    ...
    st.data_editor(
        ...
        hide_index=True, width="stretch", key="staging_editor", on_change=commit_staging_edits
    )
