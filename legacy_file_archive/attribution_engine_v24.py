"""
=============================================================================
Script Name: attribution_engine_v24.py
Purpose: Parses the official IBKR Flex Query (MTM and CNAV).
         - V24 FIX: Banned all MTMP Subtotals (`if not sym_str: continue`).
         - V24 FIX: Banned MTMP 'CASH' and 'FX' asset classes to stop Interest double-counting.
         - V24 FIX: Migrated FX Translation and Net FX Trading PnL extraction directly to CNAV.
         - V23a FIX: Cross-references Options Journal for definitive Beta/Tail classification.
=============================================================================
"""
...
def get_bucket(sym, underlying, sec_type, p_c, strike, synth_beta_keys, tail_hedge_keys):
    ...
    if sec_type == 'OPT':
        # Option A: Database Cross-Reference (Absolute Truth)
        if p_c == 'C' and (u, strike) in synth_beta_keys:
            return 'a2_beta'
        if p_c == 'P' and (u, strike) in tail_hedge_keys:
            return 'a4_alpha' # Tail Hedges belong in Alpha/Protection bucket
            
        # Fallback heuristics just in case journal is missing
        if 'VIX' in s and p_c == 'C': 
            return 'a4_alpha'
            
        return 'a3_vrp' # Default for short-dated VRP options
...
def run_attribution():
    print("========================================================")
    print("   ESTATE P&L ATTRIBUTION ENGINE (v24 FULL AUTOMATION)")
    ...
    print("[*] 1. Fetching Master Options Journal for Database Cross-Referencing...")
    synth_beta_keys = set()
    tail_hedge_keys = set()
    try:
        conn_j = sqlite3.connect(DB_PATH)
        c_j = conn_j.cursor()
        c_j.execute("SELECT `Tranche ID`, Ticker, `Long Strike` FROM options_journal")
        for tranche, tckr, k_l in c_j.fetchall():
            tranche_str = str(tranche).lower()
            t_str = str(tckr).upper()
            try: strike = float(k_l)
            except: strike = 0.0
            
            if 'beta' in tranche_str or 'call' in tranche_str:
                synth_beta_keys.add((t_str, strike))
            elif 'tail' in tranche_str or 'hedge' in tranche_str:
                tail_hedge_keys.add((t_str, strike))
        conn_j.close()
        print(f"    - Found {len(synth_beta_keys)} Synthetic Beta and {len(tail_hedge_keys)} Tail Hedge blueprints.")
    except Exception as e:
        print(f"    - [!] Could not load options journal for cross-referencing: {e}")

    print("[*] 2. Parsing MTM Data, Cash Flows, & Historic NAVs from CSV...")
        ...
        for row in reader:
            ...
            # -- EXTRACT DATA --
                ...
                if sec_type == 'CNAV' and cnav_idx:
                    ...
                    # v23 FIX: Ignore Summary Rows to prevent parsing "Total" as an account
                    ...
                    # 1b. Parse Direct Cash PnL (Interest, Dividends, Fees, Tax, FX)
                    ...
                    fx_translation = safe_float(row[cnav_idx.get('FxTranslation', -1)])
                    net_fx_trading = safe_float(row[cnav_idx.get('NetFxTrading', -1)])
                    ...
                    if abs(fx_translation) > 0:
                        pnl_records.append({'date': parsed_dt, 'bucket': 'a4_alpha', 'pnl': fx_translation})
                    if abs(net_fx_trading) > 0:
                        pnl_records.append({'date': parsed_dt, 'bucket': 'a4_alpha', 'pnl': net_fx_trading})
                ...
                # --- MARK-TO-MARKET PERFORMANCE SECTION (MTMP) ---
                    ...
                    strike = safe_float(row[mtmp_idx.get('Strike', -1)])
                    report_dt = row[mtmp_idx.get('ReportDate', 0)]
                    
                    total_pnl = safe_float(row[mtmp_idx.get('Total', 37)])
                    
                    # V23 FIX: Aggressive anti-double-counting filter for IBKR Summary Rows
                    ac_str = str(asset_class).strip().lower()
                    sym_str = str(symbol).strip().lower()
                    acc_str = str(row[mtmp_idx.get('ClientAccountID', 2)]).strip().lower()
                    desc_str = str(row[mtmp_idx.get('Description', 8)]).strip().lower()

                    if 'total' in ac_str or 'total' in sym_str or 'total' in acc_str or 'total' in desc_str:
                        continue
                        
                    # V24 FIX: Ban ALL Subtotals. If there is no symbol, it is a summary row.
                    if not sym_str:
                        continue
                        
                    # V24 FIX: Ban MTMP Cash/FX. FX and Interest are now cleanly extracted from CNAV.
                    if 'cash' in ac_str or 'fx' in ac_str:
                        continue
                        
                    if abs(total_pnl) > 0.001:
                        full_sym = f"{symbol} {underlying}".strip()
                        bucket = get_bucket(full_sym, underlying, asset_class, p_c, strike, synth_beta_keys, tail_hedge_keys)
                        pnl_records.append({
    ...
    # -------------------------------------------------------------------------
    # WRITE CASH FLOWS & ATTRIBUTION (v24: DYNAMIC STITCHING)
    # -------------------------------------------------------------------------
    ...
    if min_date:
        # v23: Only delete official flows that are equal to or newer than the CSV's oldest date.
    ...
    print("[+] SUCCESS: v24 Engine Complete. Phantom PnL eliminated.\n")
