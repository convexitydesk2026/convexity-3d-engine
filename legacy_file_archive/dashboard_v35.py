...
3. Launch the UI by running exactly:
   streamlit run dashboard_v35.py

CHANGELOG (v35):
         - NEW: Ceiling Governor added to the Deployment Command Center. 
           Dynamically monitors Crypto & Gold caps (5%). Once reached, it zeros
           out their flow and mechanically redirects the overflow to CSPX.
         - Deployment Command Center: Proportional Silo Routing active.
         - Button Ledger active to prevent EOD double-spending.
         - Options Topography: Legend moved to bottom. 3D Camera rotated 180deg.
         - Tail Hedge Budgeting: Budgets deduct via avg_cost (Cost Basis).
=============================================================================
"""
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v17.py")
...
# --- Calculate Tail Hedge Budget for Alerts ---
tot_vrp = attr_df['a3_vrp'].sum() if not attr_df.empty else 0
th_budget = tot_vrp * 0.10

if not pos_df.empty:
    th_df = pos_df[pos_df['asset_class'] == 'Tail Hedge']
    th_deployed = (th_df['position'].abs() * th_df['avg_cost']).sum()
else:
    th_deployed = 0

th_available = th_budget - th_deployed

# 1. Options Warnings & 200% Premium Stop Loss Monitor
if not journal_raw_df.empty:
    ...
    for _, row in journal_raw_df[pd.isnull(journal_raw_df['Close Date'])].iterrows():
        try:
            ...
            if days_to_target <= 7:
                alerts.append(f"⏱️ **Options Gamma Risk:** Contract {tckr} ({row['Short Strike']}/{row['Long Strike']}) is {max(0, int(days_to_target))} days away from 50% DTE threshold.")

            prem = float(row['Premium Collected (USD)'])
            K_s = float(row['Short Strike'])
            K_l = float(row['Long Strike'])
            dte_rem = float(row['Days Remaining'])
            
            if dte_rem > 0 and str(dte_rem) != 'Closed':
                S_live, iv_live = fetch_live_data(tckr)
                r_rate = LIVE_RF_RATE
                iv_dec = (iv_live / 100.0) if iv_live > 1.0 else iv_live
                T_curr = dte_rem / 365.0
                
                s_p, _, _, _, _ = get_put_greeks(S_live, K_s, T_curr, r_rate, iv_dec)
                l_p, _, _, _, _ = get_put_greeks(S_live, K_l, T_curr, r_rate, iv_dec)
                curr_spread_price = s_p - l_p
                
                stop_loss_threshold = prem * 2.0
                warning_threshold = stop_loss_threshold * 0.80
                
                if curr_spread_price >= stop_loss_threshold:
                    alerts.append(f"🛑 **STOP LOSS TRIGGERED:** {tckr} ({K_s}/{K_l}) spread price (${curr_spread_price:.2f}) has breached the 200% Premium limit (${stop_loss_threshold:.2f}). Close immediately.")
                elif curr_spread_price >= warning_threshold:
                    alerts.append(f"⚠️ **Stop Loss Warning:** {tckr} ({K_s}/{K_l}) spread price (${curr_spread_price:.2f}) is approaching the 200% Premium limit (${stop_loss_threshold:.2f}).")
        except: pass

# 2. SMA Warnings
if not bench_df.empty:
    last_spy = bench_df['SPY'].iloc[-1]
    last_vix = bench_df['^VIX'].iloc[-1]
...
strats =[
    "<b>STRATEGY & EXECUTION (Silo A):</b><br>Adheres to 40% cash buffer. Execute XSP/XND Put Spreads weekly at 45-50 DTE near -0.20 Delta. Strict 8% premium floor. 50% GTC take-profit. <b>Tail Hedge Criteria:</b> Allocate 10% of VRP to 90-120 DTE deep OTM S&P 500 Puts (Delta < 5). Triggered by VIX crash or Market Regime flipping to Red.",
    "<b>STRATEGY & EXECUTION (Silo B):</b><br><b>Regime Filter:</b> Green (3-5R TOR), Yellow (1-3R), Red (0-1R). <b>Entry:</b> Progressive Exposure Table (Merit-Based). Explicitly excluded from Master DCA.",
    "<b>STRATEGY & EXECUTION (Silo C):</b><br>Adheres to 40% cash buffer. Execute XSP/XND Put Spreads weekly at 45-50 DTE near -0.20 Delta. Strict 8% premium floor. 50% GTC take-profit. <b>Tail Hedge Criteria:</b> Allocate 10% of VRP to 90-120 DTE deep OTM S&P 500 Puts (Delta < 5). Triggered by VIX crash or Market Regime flipping to Red.",
...
target_pct = 60.0
target_usd = global_metrics['nav'] * (target_pct / 100.0)
core_assets =['CSPX', 'CNDX', 'ITWN', 'CSKR', 'CNYA', 'Crypto', 'Gold']
current_usd = pos_df[pos_df['asset_class'].isin(core_assets)]['market_value'].sum() if not pos_df.empty else 0
current_pct = (current_usd / global_metrics['nav'] * 100) if global_metrics['nav'] > 0 else 0
...
allowance_usd = global_metrics['nav'] * pacing_pct
...
with c_stat:
    ...
    <div style="background-color: #f8fafc; padding: 20px; border-radius: 8px; border: 1px solid #cbd5e1; height: 100%;">
        <h4 style="margin-top: 0; color: #0f172a;">Core Equities Tracker</h4>
        ...
        <div style="font-size: 13px; color: #475569;">
            <b>Silo B Exclusion:</b> Active Swing CFDs are ring-fenced per the Progressive Exposure Table and strictly excluded from this macro DCA matrix.
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with c_action:
    if distance_usd <= 1000:
        ...
        # --- CEILING GOVERNOR LOGIC (v35 Proportional Overflow) ---
        ...
        cryp_pct_live = (cryp_usd / global_metrics['nav'] * 100) if global_metrics['nav'] > 0 else 0
        gold_pct_live = (gold_usd / global_metrics['nav'] * 100) if global_metrics['nav'] > 0 else 0
        ...
        # Base Core Weights (Totaling 90%)
        base_cspx = 0.50
        base_cndx = 0.25
        base_asia = 0.15
        total_core = base_cspx + base_cndx + base_asia
        
        # Proportional Distribution of Overflow
        ...
        total_AC_nav = nav_A + nav_C
        weight_A = nav_A / total_AC_nav if total_AC_nav > 0 else 0.5
        weight_C = nav_C / total_AC_nav if total_AC_nav > 0 else 0.5
...
c_gb_cash, c_sa_cash, c_sc_cash, c_gb_marg, c_sa_marg, c_sc_marg = st.columns(6)

with c_gb_cash:
    fig_gauge_cash = go.Figure(go.Indicator(
        mode="gauge+number+delta", 
        value=pct_cash, 
        title={'text': "Bedrock Cash Buffer", 'font': {'size': 14}}, 
...
for i, r in pos_df.iterrows():
    ...
    if ac == 'Crypto': 
        ...
        alloc_map['SGLN / IGLN (Gold)'] = alloc_map.get('SGLN / IGLN (Gold)', 0) + r['market_value']
    elif ac == 'Cash': 
        ...
        alloc_map['Deep OTM Tail Hedge'] = alloc_map.get('Deep OTM Tail Hedge', 0) + r['market_value']
    else: 
    ...
    with col_mc_chart:
        ...
        for i in range(200):
            mc_fig.add_trace(go.Scatter(
                y=cum_sim[i], 
                mode='lines', 
                line=dict(color=random.choice(spaghetti_colors), width=1.5), 
                showlegend=False, 
                hoverinfo='skip'
            ))
        ...
        if str(raw_dte) == 'Closed':
            ...
            with col_2d:
                iv_override = st.slider(
                    f"🌪️ {tckr} Volatility (IV) Stress Tester %", 
                    min_value=5.0, max_value=80.0, value=float(iv_live_raw), step=0.5, 
                    help="Simulate a Volatility Shock. Default value is locked to the live market VIX."
                )
            ...
            # Make the plot dynamically stretch to include the live price (with a 10-point buffer)
            ...
            with col_2d:
                # --- NEW v35: LIVE TWS P&L EXTRACTION ---
                ...
                if acct_filter:
                    leg_mask = pos_df['account'] == acct_filter
                else:
                    leg_mask = pd.Series(True, index=pos_df.index)
                    
                # Match the exact short and long option legs in the database (handles both integer and float strike formats)
                ...
                # Sum the live actual P&L from TWS
                ...
                fig_2d.update_layout(
                    title="2D Theta Decay Profile & Gamma Cliff", 
                    margin=dict(l=20, r=20, t=40, b=20), 
                    height=500, 
            ...
            with col_3d:
                # Force daily resolution (step = 1) for accurate DTE gridlines
                step = 1
                ...
                # MAIN SURFACE (Z-axis contours added for P&L "water levels")
                ...
                # Add Daily DTE gridlines (skipping special timeline traces to avoid overwriting them)
                skip_days = [int(init_dte), int(curr_dte), int(init_dte / 2.0), 0]
                for idx_d, d in enumerate(y_3d):
                    if int(d) not in skip_days:
                        fig_3d.add_trace(go.Scatter3d(
                            x=x_vals, y=[d]*len(x_vals), z=z_3d[idx_d],
                            mode='lines', line=dict(color='black', width=1),
                            showlegend=False, hoverinfo='skip'
                        ))
           
                # --- v35 ENHANCEMENT: Intersection Curves (Wireframing the Kill Box) ---
                z_green, z_red = [],[]
                for d in y_3d:
                    T_3d = max(d / 365.0, 0.0001)
                    # Green Curve (Short Strike Z-Values)
                    ...
                    # Red Curve (Long Strike Z-Values)
                ...
                # Yellow Curve (50% DTE Z-Values)
                half_dte = init_dte / 2.0
                T_half = max(half_dte / 365.0, 0.0001)
                z_yellow =[]
                for p in x_vals:
                    t_s_y, _, _, _, _ = get_put_greeks(p, K_s, T_half, r_rate, iv_dec)
                    t_l_y, _, _, _, _ = get_put_greeks(p, K_l, T_half, r_rate, iv_dec)
                    z_yellow.append((prem - (t_s_y - t_l_y)) * qty * 100)

                # --- NEW: Exact Z=0 (Gray) Interpolation Curve ---
                ...
                for idx_d, d in enumerate(y_3d):
                    ...
                    for i in range(len(x_vals)-1):
                        if (z_row[i] >= 0 and z_row[i+1] < 0) or (z_row[i] <= 0 and z_row[i+1] > 0):
                            ...
                            break # Move to next DTE once the crossing is found

                # Plotting the Calculated Intersection Curves
                ...
                # Plotting the True Gray Breakeven Line
                if x_zero:
                    fig_3d.add_trace(go.Scatter3d(x=x_zero, y=y_zero, z=z_zero_line, mode='lines', name='Breakeven Limit', line=dict(color='gray', width=6), showlegend=False, hoverinfo='skip'))
                # ----------------------------------------------------------------------
                
                # PLANES
                fig_3d.add_trace(go.Surface(
                    x=[[K_s, K_s],[K_s, K_s]], y=[[0, init_dte],[0, init_dte]], z=[[z_min, z_min],[z_max, z_max]],
                    colorscale=[[0, 'green'],[1, 'green']], opacity=0.23, showscale=False, hoverinfo='skip'
                ))
                fig_3d.add_trace(go.Surface(
                    x=[[K_l, K_l],[K_l, K_l]], y=[[0, init_dte],[0, init_dte]], z=[[z_min, z_min],[z_max, z_max]],
                    colorscale=[[0, 'red'],[1, 'red']], opacity=0.23, showscale=False, hoverinfo='skip'
                ))
                fig_3d.add_trace(go.Surface(
                    x=[[x_vals[0], x_vals[-1]], [x_vals[0], x_vals[-1]]], 
                    y=[[half_dte, half_dte],[half_dte, half_dte]], 
                    z=[[z_min, z_min],[z_max, z_max]],
                    colorscale=[[0, 'yellow'],[1, 'yellow']], opacity=0.30, showscale=False, hoverinfo='skip'
                ))
                fig_3d.add_trace(go.Surface(
                    x=[[x_vals[0], x_vals[-1]],[x_vals[0], x_vals[-1]]],
                    y=[[0, 0],[init_dte, init_dte]],
                    z=[[0, 0],[0, 0]],
                    colorscale=[[0, 'gray'],[1, 'gray']], opacity=0.30, showscale=False, hoverinfo='skip'
                ))

                # TIMELINE TRACES
                fig_3d.add_trace(go.Scatter3d(
                    x=x_vals, y=[init_dte]*len(x_vals), z=y_init, 
                    mode='lines', name='Entry Day', line=dict(color='#dc2626', width=5, dash='dash')
                ))
                fig_3d.add_trace(go.Scatter3d(
                    x=x_vals, y=[curr_dte]*len(x_vals), z=y_curr, 
                    mode='lines', name='Today', line=dict(color='#2563eb', width=7)
                ))

                # CURRENT PRICE ANCHOR
                fig_3d.add_trace(go.Scatter3d(
                    x=[S_live], y=[curr_dte], z=[tws_unrealized_pnl], 
                    mode='markers', name='Current Price', marker=dict(color='black', size=6)
                ))
                
                # 50% TAKE-PROFIT TARGET CROSSHAIR
                target_pnl = (prem / 2.0) * qty * 100
                fig_3d.add_trace(go.Scatter3d(
                    x=[S_live], y=[curr_dte], z=[target_pnl], 
                    mode='markers', name='50% Target', marker=dict(color='#16a34a', size=15, symbol='cross')
                ))

                # Anchor Drop Line (Visual Tether to the Breakeven Floor)
                fig_3d.add_trace(go.Scatter3d(
                    x=[S_live, S_live], y=[curr_dte, curr_dte], z=[0, tws_unrealized_pnl], 
                    mode='lines', name='Anchor Line', line=dict(color='black', width=3, dash='dot'), showlegend=False, hoverinfo='skip'
                ))
                
                # Anchor Floor Cross (Marks the exact Z=0 intersection)
                fig_3d.add_trace(go.Scatter3d(
                    x=[S_live], y=[curr_dte], z=[0],
                    mode='markers', name='Zero Floor Anchor', marker=dict(color='black', size=5, symbol='cross'), showlegend=False, hoverinfo='skip'
                ))
