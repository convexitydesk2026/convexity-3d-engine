...
3. Launch the UI by running exactly:
   streamlit run dashboard_v93.py

CHANGELOG (v93):
         - STAGE 9 (v93): Fixed 3D Topography scaling by parsing live TWS indices natively. 
         - STAGE 8b (v93): Built out the requested Alpha Foreign Currency Risk Calculator.
         - STAGE 8 (v92): Neutralized 'HTTP Error 404' console spam by silencing yfinance exceptions & standardizing FX pairs.
=============================================================================
"""
...
# Silence all yfinance warnings and logs
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v34.py")
...
def get_risk_free_rate():
    ...
    try:
        with open(os.devnull, 'w') as devnull, contextlib.redirect_stdout(devnull), contextlib.redirect_stderr(devnull):
            irx = yf.Ticker('^IRX').history(period='5d', progress=False)['Close'].iloc[-1]
...
def get_fx_rate(currency):
    ...
    # Use universal Yahoo Finance mapping for direct Foreign -> USD conversion
    pair = f"{currency}USD=X"
    try:
        import contextlib
        with open(os.devnull, 'w') as devnull, contextlib.redirect_stdout(devnull), contextlib.redirect_stderr(devnull):
            rate = float(yf.Ticker(pair).history(period='1d', progress=False)['Close'].iloc[-1])
...
def fetch_live_data(ticker_symbol):
    ...
    # 1. Fetch flawlessly accurate 3D Topography indices directly from the local TWS sync
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        ...
        if 'XSP' in t_str or 'SPX' in t_str:
            xsp = get_db_val('XSP')
            if xsp and vix:
                conn.close()
                return xsp, vix
        elif 'XND' in t_str or 'NDX' in t_str:
            ndx = get_db_val('NDX')
            if ndx and vix:
                conn.close()
                return ndx / 100.0, vix
        else:
            sym = get_db_val(t_str)
            if sym and vix:
                conn.close()
                return sym, vix
        conn.close()
    ...
    # 2. Fallback to yfinance if local DB is totally empty
    try:
        import contextlib
        with open(os.devnull, 'w') as devnull, contextlib.redirect_stdout(devnull), contextlib.redirect_stderr(devnull):
            if 'XSP' in t_str:
                spx = yf.Ticker('^SPX').history(period='5d', progress=False)['Close'].iloc[-1]
                vix = yf.Ticker('^VIX').history(period='5d', progress=False)['Close'].iloc[-1]
                return float(spx) / 10.0, float(vix)
            elif 'XND' in t_str:
                ndx = yf.Ticker('^NDX').history(period='5d', progress=False)['Close'].iloc[-1]
                vxn = yf.Ticker('^VXN').history(period='5d', progress=False)['Close'].iloc[-1]
                return float(ndx) / 100.0, float(vxn)
            else:
                spot_price = yf.Ticker(t_str).history(period='5d', progress=False)['Close'].iloc[-1]
                vix = yf.Ticker('^VIX').history(period='5d', progress=False)['Close'].iloc[-1]
...
def process_metrics(df_acc, rf_rate):
    ...
    # v93 Absolute Truth Math: Start NAV is inherently 0
...
def get_stock_smas(symbol):
    ...
    try:
        with open(os.devnull, 'w') as devnull, contextlib.redirect_stdout(devnull), contextlib.redirect_stderr(devnull):
            ...
            hist = yf.Ticker(clean_sym).history(period='100d', progress=False)
...
def load_and_process_data():
    ...
    try:
        ...
        # v93 FIX: Robust NAV Forward Fill (Solves the Phantom Volatility Jump)
    ...
    # Global tracking arrays (v93: Pure Absolute Math across all Silos. Kills the Whipsaw)
...
with calc_placeholder.container():
    ...
    with st.expander("Position Sizing Engine", expanded=False):
        ...
        calc_stop = st.number_input(f"Stop Loss Limit ({calc_currency})", min_value=0.0, value=0.0, step=1.0, help="For Longs: Low of the day. For Shorts: High of the day.")
        ...
        if calc_entry > 0 and calc_stop > 0 and calc_nav > 0:
            ...
            risk_usd = calc_nav * (calc_risk_pct / 100.0)
            ...
            if spread_usd > 0:
                ...
                if valid_dir:
                    shares = int(risk_usd // spread_usd)
                ...
                st.warning("Entry and Stop Loss cannot be identical.")
    st.markdown("---")
...
if not df_alpha.empty:
    ...
    # v93: Fetch and inject SMAs, Earnings, and R-Multiples
