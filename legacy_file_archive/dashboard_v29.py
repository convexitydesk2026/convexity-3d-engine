...
Purpose: The Streamlit Frontend (Family Office Estate Architecture)
         ...
         - Tail Hedge Budgeting (v28 patch): Budgets deduct via avg_cost (Cost Basis).
         - NEW: Deployment Command Center. Regime-paced DCA matrix tracking 
           the 60% Core Equity transition with SQLite Cooldown Ledger.
         - NEW: SGLN/IGLN (Physical Gold) explicitly tracked as 5% Alternative.
=============================================================================
"""
...
def load_and_process_data():
    ...
    # ADDED avg_cost to query for Tail Hedge logic
    ...
    def categorize(sym, sec, pos):
        ...
        if 'SGLN' in s or 'IGLN' in s: return 'Gold' # ADDED GOLD
...
# --- Calculate Tail Hedge Budget for Alerts (v28 Patch) ---
...
if not attr_df.empty:
    ...
    # Tail Hedge Budget Accounting (v28 Patch)
...
with c_action:
    if distance_usd <= 1000:
        ...
        <div style="background-color: {box_color}; padding: 20px; border-radius: 8px; border: 1px solid {border_color};">
            ...
            <div style="font-size: 14px; margin-bottom: 15px;">Pacing Schedule: <b>{pacing_pct*100:.1f}% NAV per week</b></div>
            <b style="font-size: 18px;">🛒 Weekly Shopping List: ${actual_deploy:,.0f}</b>
            <ul style="font-size: 15px; margin-top: 10px; margin-bottom: 0px;">
                <li><b>CSPX (50%):</b> ${actual_deploy * 0.50:,.0f}</li>
                <li><b>CNDX (25%):</b> ${actual_deploy * 0.25:,.0f}</li>
                <li><b>Asian Satellites (15%):</b> ${actual_deploy * 0.15:,.0f} <i>(ITWN / CSKR / CNYA)</i></li>
                <li><b>Crypto ETPs (5%):</b> ${actual_deploy * 0.05:,.0f}</li>
                <li><b>Physical Gold (5%):</b> ${actual_deploy * 0.05:,.0f} <i>(SGLN / IGLN)</i></li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
