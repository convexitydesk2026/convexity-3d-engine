"""
=============================================================================
Script Name: dashboard_v5.py
Purpose: The Streamlit Frontend (Module 5).
         - Fixes the HTML parser bug in the Master Aggregation Panel.
         - Adds the robust 10,000-run Monte Carlo Simulation Panel.
=============================================================================
"""
...
# FIX: Removed the blank lines inside the HTML string to prevent the Streamlit Markdown parser bug.
html_metrics = f"""
...
if sim_length > 0:
    ...
    # Plot 100 random paths (Light Grey)
    for i in range(100):
        mc_fig.add_trace(go.Scatter(y=cum_sim[i], mode='lines', line=dict(color='rgba(150, 150, 150, 0.1)', width=1), showlegend=False, hoverinfo='skip'))
    
    # Plot Average and Original
    mc_fig.add_trace(go.Scatter(y=mc_avg_path, name='Avg Path (Stable)', mode='lines', line=dict(color='blue', width=3)))
    mc_fig.add_trace(go.Scatter(y=orig_cum, name='Original History', mode='lines', line=dict(color='black', width=3)))
