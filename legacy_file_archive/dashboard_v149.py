...
streamlit run dashboard_v149.py
CHANGELOG (v149):
- STAGE 56 (v149): Implemented the Global High-Water Mark (HWM) Risk Budget.
- STAGE 56 (v149): Implemented 48-Hour Revenge Trading API Lockout.
- STAGE 56 (v149): Implemented 1.5x ATR Volatility Floor & 5% Absolute Notional Cap.
- STAGE 56 (v149): Implemented Overnight 25% Gap Stress Test & Checkbox.
- STAGE 56 (v149): Fixed Zombie Campaign SQL emoji string bugs.
...
- STAGE 55 (v147): Updated subprocess call to point to market_flow_engine_v7.py (BLUF Executive Summary restructure).
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v35.py")
...
with st.sidebar:
    ...
    if st.button("🌐 Generate Market Flow Report", width="stretch"):
        with st.spinner("Calculating Institutional Flow & Generating PDF... Please wait (~15s)"):
            try:
                subprocess.run(["python", "market_flow_engine_v7.py"], cwd=TARGET_DIR, check=True)
...
def get_atr(symbol, period=14):
    try:
        clean_sym = symbol.split()[0]
        df = yf.Ticker(clean_sym).history(period=f"{period+5}d")
...
def load_benchmarks(start_date, end_date):
    ...
    # Multi-Factor SMAs for the Macro Regime Filter
    bench_df['rsp_sma_50'] = bench_df['RSP'].rolling(window=50).mean()
    bench_df['rsp_sma_200'] = bench_df['RSP'].rolling(window=200).mean()
    bench_df['qqq_sma_50'] = bench_df['QQQ'].rolling(window=50).mean()
    bench_df['qqq_sma_200'] = bench_df['QQQ'].rolling(window=200).mean()
    
    bench_df = bench_df[bench_df['date'] >= pd.to_datetime(start_date)].copy()
    
    def get_regime_and_tor(row):
        vix = row['^VIX']
        rsp, spy, qqq = row['RSP'], row['SPY'], row['QQQ']
        rsp_50, rsp_200 = row['rsp_sma_50'], row['rsp_sma_200']
        spy_50, spy_200 = row['sma_50'], row['sma_200']
        qqq_50, qqq_200 = row['qqq_sma_50'], row['qqq_sma_200']
        
        if pd.isna(rsp_50) or pd.isna(rsp_200) or pd.isna(spy_50) or pd.isna(qqq_50): 
            return 'Yellow', 2
            
        # DOWNTREND (Red): Any index breaks 200 SMA OR VIX > 22
        if spy < spy_200 or qqq < qqq_200 or rsp < rsp_200 or vix > 22:
            return 'Red', 0
            
        # UPTREND (Green): All indices > 50 SMA AND VIX < 18
        elif spy > spy_50 and qqq > qqq_50 and rsp > rsp_50 and vix < 18:
            return 'Green', 5 if vix < 15 else 4
            
        # RANGEBOUND (Yellow): Mixed signals or VIX 18-22
        else: 
            return 'Yellow', 2
...
with calc_placeholder.container():
    ...
    # 1. Calculate Global High-Water Mark (HWM) & Live Budget
    # Protocol v7.0 Inception Reset (July 16, 2026)
    # We only calculate the HWM from this date forward to avoid being locked out by legacy drawdowns.
    v7_inception_date = pd.to_datetime('2026-07-16')
    ...
    if not global_df.empty:
        ...
        if not v7_df.empty:
            ...
            hwm = global_metrics['nav']
    else:
        hwm = global_metrics['nav']
        
    seed_buffer = hwm * 0.005
    live_risk_budget = (global_metrics['nav'] - hwm) + seed_buffer
    
    # 2. Check 48-Hour Revenge Trading Lockout (v7.0 Inception Filter)
    conn_lock = sqlite3.connect(DB_PATH)
    c_lock = conn_lock.cursor()
    c_lock.execute("SELECT COUNT(*) FROM alpha_campaigns WHERE status IN ('Closed', 'Closed 🏁') AND total_pnl < 0 AND close_date >= date('now', '-2 days') AND close_date >= '2026-07-16'")
    recent_losses = c_lock.fetchone()[0]
    conn_lock.close()
    
    # FIX 1: Cast to native Python bool to prevent Streamlit protobuf TypeError
    is_locked = bool(recent_losses >= 3 or live_risk_budget <= 0)
    
    budget_color = "#16a34a" if live_risk_budget > 0 else "#dc2626"
    ...
    <div style='background-color: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #cbd5e1; margin-bottom: 15px;'>
        <div style='font-size: 12px; color: #64748b; font-weight: bold; text-transform: uppercase;'>Global HWM Risk Budget</div>
        <div style='font-size: 24px; font-weight: 900; color: {budget_color};'>&#36;{live_risk_budget:,.0f}</div>
        <div style='font-size: 12px; color: #475569; margin-top: 5px;'>HWM: &#36;{hwm:,.0f} | Seed Buffer (0.5%): &#36;{seed_buffer:,.0f}</div>
    </div>
    """, unsafe_allow_html=True)
    
    if is_locked:
        lock_reason = "3 consecutive losses in 48h." if recent_losses >= 3 else "HWM Risk Budget depleted."
        st.error(f"🚨 **ALPHA ENGINE LOCKED:** {lock_reason} Trading halted until recovery.")

    with st.expander("Position Sizing Engine", expanded=not is_locked):
        ...
        live_regime_ui = temp_chart_ui['regime'].iloc[-1] if not temp_chart_ui.empty else 'Yellow'
        
        calc_silo = st.selectbox("Target Silo", ['U23144948 (Silo A)', 'U23139264 (Silo B)', 'U23154199 (Silo C)', 'U25218481 (Silo D)'], key="calc_silo", disabled=is_locked)
        ...
        is_ipo = st.checkbox("Flag as IPO / Unproven Asset (< 6mo or < $2B MC)", disabled=is_locked)
        
        calc_mode = st.radio("Trade Mode", ["Initial Entry", "Scale-In (Pyramid)"], horizontal=True, label_visibility="collapsed", disabled=is_locked)
        c1, c2 = st.columns(2)
        
        # Dynamic Risk 'R' Enforcement
        if live_regime_ui == 'Red':
            calc_risk_pct = 0.0
            c1.number_input("Risk %", value=0.0, disabled=True, help="Regime RED: Alpha Engine Halted.")
        else:
            max_r = 0.25 if live_regime_ui == 'Green' else 0.125
            calc_risk_pct = c1.number_input("Risk %", min_value=0.01, max_value=max_r, value=max_r, step=0.025, format="%.3f", disabled=is_locked, help=f"Regime {live_regime_ui}: Max Risk capped at {max_r}%.")
        ...
        if calc_mode == "Scale-In (Pyramid)":
            ...
            if active_alpha.empty:
                ...
                calc_sym = st.selectbox("Target Asset", active_alpha['symbol'].unique(), disabled=is_locked)
                ...
                if is_violation:
                    ...
                    if max_cash_shares > 0:
                        desired_shares = st.number_input("Shares to Purchase", min_value=1, max_value=max_cash_shares, value=1, step=1, disabled=is_locked)
                    ...
                    input_sl = st.number_input(f"Enter NEW Stop Loss ({calc_currency})", min_value=0.0, value=float(live_stop_local), step=1.0, disabled=is_locked)
            ...
            calc_sym_init = st.text_input("Ticker Symbol", disabled=is_locked).upper()
            calc_dir = c2.selectbox("Direction", ["Long", "Short"], disabled=is_locked)
            calc_currency = st.selectbox("Asset Currency", ['USD', 'SEK', 'TWD', 'HKD', 'KRW', 'CNY', 'JPY', 'EUR', 'GBP', 'CAD', 'ILS'], disabled=is_locked)
            calc_entry = st.number_input(f"Entry Price ({calc_currency})", min_value=0.0, value=0.0, step=1.0, disabled=is_locked)
            calc_stop = st.number_input(f"Stop Loss Limit ({calc_currency})", min_value=0.0, value=0.0, step=1.0, disabled=is_locked)

        if calc_entry > 0 and calc_stop > 0 and calc_nav > 0 and not is_locked:
            fx_rate = get_fx_rate(calc_currency)
            ...
            target_sym_atr = calc_sym if calc_mode == "Scale-In (Pyramid)" else calc_sym_init
            if target_sym_atr:
                atr_val = get_atr(target_sym_atr)
                min_stop_dist = atr_val * 1.5
                actual_dist = abs(calc_entry - calc_stop)
                if actual_dist < min_stop_dist and atr_val > 0:
                    st.error(f"🚨 **Stop-loss is too tight!** Minimum allowed distance (1.5x ATR) is **{min_stop_dist:.2f} {calc_currency}**. You must give the asset room to breathe.")
                    valid_dir = False
            ...
            if valid_dir:
                ...
                if valid_dir and proposed_shares > 0 and (calc_mode != "Scale-In (Pyramid)" or is_valid_pyramid):
                    ...
                    max_notional_usd = global_metrics['nav'] * (0.02 if is_ipo else 0.05)
        ...
        for _, r in open_positions.iterrows():
            ...
            if not cj.fetchone():
                ...
                cj.execute("""
                    ...
                    VALUES (?, ?, 'Open', ?, ?, ?, ?, ?, ?, ?, ?)
                """, (sym, trade_type, datetime.date.today().isoformat(), regime_str, sec, ind, s20, s50, s200, entry_usd))
...
with st.expander("🔭 View Pre-Trade Staging Area (Pipeline)", expanded=False):
    
    # Fetch absolute list of all tags for the dropdown
    ...
    if not df_staging.empty:
        ...
        if regime_str == 'Green': regime_allowance = global_metrics['nav'] * 0.05 # 5% TOR
        elif regime_str == 'Yellow': regime_allowance = global_metrics['nav'] * 0.03 # 3% TOR
...
if not df_open.empty:
    ...
    with st.expander("🛠️ Action Required: Set Initial Stops & Tags for New Campaigns", expanded=False):
        ...
        for idx, row in df_open.iterrows():
            with st.container():
                # Safely handle None/NaN values from legacy zombie campaigns
                safe_entry = float(row['entry_price']) if pd.notna(row['entry_price']) else 0.0
                safe_sma20 = float(row['sma_20']) if pd.notna(row['sma_20']) else 0.0
                safe_sma50 = float(row['sma_50']) if pd.notna(row['sma_50']) else 0.0
                
                # FIX: Use the safe variables and HTML entity &#36; to prevent SyntaxWarnings
                st.markdown(f"**[{row['type']}] {row['symbol']}** | Entry: &#36;{safe_entry:.2f} | 20 SMA: &#36;{safe_sma20:.2f} | 50 SMA: &#36;{safe_sma50:.2f} | Sector: {row['sector']}")
                ...
                with c_save:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("Save", key=f"save_{row['id']}", width="stretch"):
    ...
    display_open = df_open.drop(columns=['id', 'tags', 'thesis', 'initial_stop', 'tranche_added'], errors='ignore')
    
    # FIX: Force numeric conversion and fill None/NaN with 0.0 before Pandas styling
    cols_to_fill = ['sma_20', 'sma_50', 'sma_200', 'entry_price']
    ...
    st.download_button("💾 Download Active Campaigns & Theses (CSV)", data=csv_open_data, file_name=f"Alpha_Journal_Open_{datetime.date.today().isoformat()}.csv", mime="text/csv")
...
df_closed = pd.read_sql_query("SELECT id, open_date, close_date, symbol, type, regime_in, sector, industry, entry_price, initial_stop, tags, thesis, days_active, total_pnl, r_multiple, grade FROM alpha_campaigns WHERE status IN ('Closed', 'Closed 🏁') ORDER BY close_date DESC, id DESC", conn_journal)

if not df_closed.empty:
    exp_closed = st.expander("📚 View Closed Campaigns, Post-Mortem & Analytics", expanded=False)
