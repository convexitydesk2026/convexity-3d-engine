...
3. Launch the UI by running exactly:
   streamlit run dashboard_v69.py

CHANGELOG (v69):
         - ADDED: Alpha Conviction Watchlist UI (Dynamic IV Scanner integration).
         - ADDED: VIX Crush Gauge and Alerting for Tail Hedge deployment.
         - UPDATED: 22-DTE Eve of Destruction preparation alert.
=============================================================================
"""
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v25.py")
...
def fetch_live_data(ticker_symbol):
    ...
    # v69: Safeguard against empty cells during live data-entry
...
with st.sidebar:
    ...
    with st.expander("Manage Alpha Watchlist"):
        ...
        with st.form("add_watchlist_form", clear_on_submit=True):
            ...
            wl_target_iv = st.number_input("Target IV Threshold (%)", min_value=1.0, value=45.0, step=1.0)
            if st.form_submit_button("Add to Scanner"):
                if wl_ticker:
                    c.execute("INSERT OR REPLACE INTO alpha_watchlist (symbol, target_iv, current_iv) VALUES (?, ?, (SELECT current_iv FROM alpha_watchlist WHERE symbol=?))", (wl_ticker, wl_target_iv, wl_ticker))
                    conn.commit()
                    st.success(f"Added {wl_ticker} at {wl_target_iv}% target.")
                    st.cache_data.clear()
                    st.rerun()
...
def get_exact_opt_margin(df_in):
    ...
    try:
        ...
        # v69 FIX: Added base_tckr to the groupby!
        ...
        try:
            ...
            if str(dte_rem) != 'Closed':
                ...
                if dte_int <= 21:
                    ...
                    alerts.append(f"⚡ **EVE OF DESTRUCTION (22 DTE):** {tckr} ({row['Short Strike']}/{row['Long Strike']}) hits the Gamma Cliff tomorrow. Prepare to execute mechanical ejection.")
                elif dte_int <= 30:
...
if th_available >= 400.0:
    ...
    if is_vix_crush:
        alerts.append(f"🟢 **VIX CRUSH DETECTED (VIX: {vix_live:.2f}):** Tail insurance is cheap! You have **${th_available:,.0f}** available. Deploy **{tranches_due} tranche(s)** of 120-DTE Deep OTM S&P 500 Puts immediately.")
    else:
        if tranches_due == 1:
    ...
    if not wl_df_alert.empty and regime_str in ['Red', 'Yellow']:
        for _, w_row in wl_df_alert.iterrows():
            if w_row['current_iv'] >= w_row['target_iv']:
                alerts.append(f"🎯 **ALPHA OPPORTUNITY ({w_row['symbol']}):** Live IV ({w_row['current_iv']:.1f}%) has breached your target ({w_row['target_iv']:.1f}%). Market regime is {regime_str}. Sell Conviction CSPs now.")
except Exception:
    pass
...
# v69 FIX: Added Physical US Stocks & International Stocks to Core Assets pool
...
if not journal_raw_df.empty:
    ...
    if not active_vrp.empty:
        ...
        # v69: Dynamically map LIVE TWS PnL to calculate Live Annualized ROC
        live_roc_list = []
        live_ann_roc_list = []
...
if not journal_raw_df.empty:
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            # v69: 3D Engine Native Debit Spread & Call Detection
