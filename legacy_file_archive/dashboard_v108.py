...
streamlit run dashboard_v108.py
CHANGELOG (v108):
...
- STAGE 15 (v107): Silenced Streamlit component deprecation terminal warnings via Base64 Data URI iframe. Halved the Macro Governor risk logic per regime and added a Hard Block for Regime 0.
- STAGE 14 (v106): Fixed CSS Bleeding bug. Reverted Section 1C to components.html (iframe sandbox) to prevent the market flow stylesheet from leaking out and collapsing the Master Estate Aggregation table.
=============================================================================
"""
...
with st.sidebar:
    ...
    if st.button("🌐 Generate Market Flow Report", width="stretch"):
        with st.spinner("Calculating Institutional Flow & Generating PDF... Please wait (~15s)"):
            try:
                subprocess.run(["python", "market_flow_engine_v3.py"], cwd=TARGET_DIR, check=True)
...
with st.expander("📊 Expand Institutional Flow Dashboard", expanded=False):
    ...
    if os.path.exists(html_path):
        ...
        # Encoded to Base64 and passed to native iframe to permanently silence Streamlit deprecation warnings
        # while preserving the CSS sandbox that protects the Master Aggregation table.
        b64_html = base64.b64encode(html_data.encode('utf-8')).decode('utf-8')
        components.iframe(f"data:text/html;base64,{b64_html}", height=900, scrolling=True)
    else:
