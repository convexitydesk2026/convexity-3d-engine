...
3. Launch the UI by running exactly:
   streamlit run dashboard_v79.py

CHANGELOG (v79):
         - ARCHITECTURE UPGRADE: Implemented "Conservation of Mass" Accounting.
         - FIXED: Robust NAV forward-fill perfectly neutralizes phantom volatility jumps.
=============================================================================
"""
...
def load_and_process_data():
    ...
    try:
        ...
        # V79 FIX: Robust NAV Forward Fill (Solves the Phantom Volatility Jump)
    ...
    if not global_df.empty:
        ...
        try:
            ...
            if not temp_attr.empty:
                ...
                # v78 FIX: OUTER JOIN to prevent dropping official PnL on days TWS wasn't synced
                global_df = pd.merge(global_df, temp_attr[['date', 'true_daily_pnl']], on='date', how='outer')
                global_df = global_df.sort_values('date').reset_index(drop=True)
                
                # Forward-fill NAV and ensure 0 flow for injected dates
                global_df['nav'] = global_df['nav'].ffill().fillna(0)
                global_df['net_flow'] = global_df['net_flow'].fillna(0.0)
                global_df['prev_nav'] = global_df['nav'].shift(1)

                # V79 FIX: "Conservation of Mass" Accounting
                # We completely decouple the Global PnL from the fragile Pandas shift() math.
                global_df['daily_pnl'] = global_df['true_daily_pnl'].fillna(0.0)
                
                # Absolute Truth Calculation: Total PnL = Current NAV - Total Net Flows
                live_nav = global_df['nav'].iloc[-1]
                total_flows = global_df['net_flow'].sum()
                total_all_time_pnl = live_nav - total_flows
                
                # Today's live intraday PnL is mathematically forced to be the exact remainder
                total_historical_mtm = global_df['true_daily_pnl'].sum()
                today_live_pnl = total_all_time_pnl - total_historical_mtm
                
                # Inject today's live PnL safely into the last row
                last_idx = global_df.index[-1]
                global_df.at[last_idx, 'daily_pnl'] = today_live_pnl
                ...
                global_metrics['pnl'] = total_all_time_pnl
            else:
                global_df['prev_nav'] = global_df['nav'].shift(1)
