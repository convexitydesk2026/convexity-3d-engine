...
Purpose: Calculates daily PnL split across 5 strategic buckets and appends
         ...
         (Phase 10: Data Gap Handler & Ghost PnL Prevention)
=============================================================================
"""
import sqlite3
import pandas as pd
...
def run_attribution():
    print("========================================================")
    print("   ESTATE P&L ATTRIBUTION ENGINE (PHASE 10)")
    print("   Data Gap Handler & Graveyard Catcher")
    print("========================================================")
    ...
    # 3. Process Phase 10 Allocation Logic
    for i in range(len(dates)):
        ...
        for _, r in pos_T.iterrows():
            ...
            if key in prev_lookup:
                ...
                # Newly appeared asset. DO NOT absorb historical Unrealized PnL.
                tracked_pnl = 0.0 
                
            alloc[bkt] += tracked_pnl

        # Step B: Reconcile Drift
        true_daily = global_true_pnl.get(curr_date, 0.0)
        tracked_daily = sum(alloc.values())
        drift = true_daily - tracked_daily

        # Step C: Intelligent Resolution
        vanished_keys = set(prev_lookup.keys()) - active_keys
        
        # Condition 1: BLIND START (The Database Gap)
        # If we had NO positions yesterday, we can't track drift. Route to Beta.
        if len(prev_lookup) == 0:
            alloc['a2_beta'] += drift
            
        # Condition 2: ASSET VANISHED (The Graveyard Catcher)
        # A trade closed! Map its final profit to its bucket.
        ...
        # Condition 3: THE NOISE CANCELER
        # No assets vanished. Differentiate Data Gaps vs True Fees.
        else:
            if abs(drift) > 500:
                # A drift > $500 without closed trades is a data gap or major dividend.
                alloc['a2_beta'] += drift
            else:
                # Normal background broker noise / cash interest
                alloc['a5_fees'] += drift
    ...
    print("[+] Phase 10 Complete. Historical Database gaps bridged successfully.")
