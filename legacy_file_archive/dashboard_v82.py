...
3. Launch the UI by running exactly:
   streamlit run dashboard_v82.py

CHANGELOG (v82):
         - UI UPGRADE: Sidebar Markdown Quick Jump menu added.
         - UI UPGRADE: Wrapped massive static Matrices in collapsed Expanders to save space.
         - BUG FIX: Corrected indentation bug that would hide the Options Matrix if a CSP was active.
=============================================================================
"""
...
with st.sidebar:
    ...
    st.markdown("""
    - [Master Aggregation](#master-estate-aggregation)
    - [1. Capital Breakdown](#1-estate-capital-breakdown-gaap-allocation-sectors)
    - [2. Portfolio Composition](#2-live-portfolio-composition-from-tws)
    - [3. Daily PnL per Silo](#3-daily-pnl-per-silo)
    - [4. PnL Attribution](#4-pnl-attribution-capital-velocity)
    - [5. Command Center](#5-deployment-command-center-transition-to-60-40)
    - [6. Margin & Risk](#6-capital-deployment-margin-capacity-tracker)
    - [7. Instrument Matrix](#7-the-master-instrument-matrix-tax-alpha-sharpe-grading)
    - [8. Monte Carlo](#8-estate-montecarlo-pnl-simulation-projections-vs-history)
    - [9A. Options Matrix](#9a-the-master-options-matrix-cfo-briefing)
    - [9B. Options Ledger](#9b-the-options-performance-ledger-topography-engine)
    """)
    st.markdown("---")
...
def process_metrics(df_acc, rf_rate):
    ...
    # v82 Absolute Truth Math: Start NAV is inherently 0
...
def load_and_process_data():
    ...
    try:
        ...
        # v82 FIX: Robust NAV Forward Fill (Solves the Phantom Volatility Jump)
    ...
    # Global tracking arrays (v82: Pure Absolute Math across all Silos. Kills the Whipsaw)
...
<div style="background-color: #f3f4f6; padding: 20px; border-radius: 12px; border: 1px solid #e5e7eb; margin-bottom: 20px;">
    <h4 id="master-estate-aggregation" style="text-align: center; color: #1f2937; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 2px; font-size: 24px;">Master Estate Aggregation</h4>
