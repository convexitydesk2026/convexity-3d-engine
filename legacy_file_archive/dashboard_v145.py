...
streamlit run dashboard_v145.py
CHANGELOG (v145):
...
with st.expander("🔭 View Pre-Trade Staging Area (Pipeline)", expanded=False):
    ...
    if not df_staging.empty:
        ...
        def commit_staging_edits():
            ...
            for row_idx_str, row_edits in list(edits.items()):
                idx = int(row_idx_str)
                ...
                if new_entry > 0 and new_stop > 0:
                    if new_type == 'Long' and new_stop >= new_entry:
                        ...
                        del st.session_state["staging_editor"]["edited_rows"][row_idx_str] # Force UI to revert
                        continue 
