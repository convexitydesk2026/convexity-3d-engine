r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v18.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 18 UPGRADES: 
         - Transformed UI to a Landscape/Side-by-Side Control Center layout.
         - Added 4 individual Silo Benchmark charts (Silo vs SPY vs QQQ).
         - Mathematically isolated TWR (Time-Weighted Returns) for each silo.
         - Matched silo chart backgrounds and dimensions to their headers.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v18.html")
...
# 3.c Benchmark Parsing & Integration (TWR Math for Estate + Individual Silos)
...
<head>
    ...
    <title>Family Estate Master Allocator v18</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v18</p>
        </div>

        <!-- 12-COLUMN LANDSCAPE GRID -->
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-8">
            
            <!-- LEFT COLUMN: Operations & Performance (7 Columns wide) -->
            <div class="xl:col-span-7 flex flex-col gap-6">
                ...
                <div class="bg-gray-50 p-4 rounded-xl border shadow-sm">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div><label class="block text-sm font-bold text-gray-700">Silo A ($)</label><input type="number" id="siloA" value="VAR_NAV_A" class="mt-1 w-full rounded-md p-2 border" oninput="updateUI()"></div>
                        <div><label class="block text-sm font-bold text-gray-700">Silo B ($)</label><input type="number" id="siloB" value="VAR_NAV_B" class="mt-1 w-full rounded-md p-2 border" oninput="updateUI()"></div>
                        <div><label class="block text-sm font-bold text-gray-700">Silo C ($)</label><input type="number" id="siloC" value="VAR_NAV_C" class="mt-1 w-full rounded-md p-2 border" oninput="updateUI()"></div>
                        <div><label class="block text-sm font-bold text-gray-700">Silo D ($)</label><input type="number" id="siloD" value="VAR_NAV_D" class="mt-1 w-full rounded-md p-2 border" oninput="updateUI()"></div>
                    </div>
                </div>
                ...
                <div class="bg-white p-4 rounded-xl border shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="bg-blue-50 p-3 rounded-lg border border-blue-200 shadow-inner">
                            ...
                            <p class="text-xs font-bold text-gray-500 mb-1 uppercase tracking-wide">Physical Balance Sheet</p>
                            ...
                            <label class="text-xs font-semibold" id="a_cash_label">Cash: </label><input type="range" id="a_cash" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            ...
                            <p class="text-xs font-bold text-gray-500 mt-2 mb-1 uppercase tracking-wide border-t border-blue-200 pt-2">Margin Overlay</p>
                        ...
                        <div class="bg-purple-50 p-3 rounded-lg border border-purple-200 shadow-inner">
                            ...
                            <label class="text-xs font-semibold" id="b_cash_label">Cash: </label><input type="range" id="b_cash" min="0" max="100" step="0.01" value="58.85" class="w-full mb-1" oninput="updateUI()">
                        ...
                        <div class="bg-green-50 p-3 rounded-lg border border-green-200 shadow-inner">
                            ...
                            <label class="text-xs font-semibold" id="c_cash_label">Cash: </label><input type="range" id="c_cash" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
                            ...
                            <p class="text-xs font-bold text-gray-500 mt-2 mb-1 uppercase tracking-wide border-t border-green-200 pt-2">Margin Overlay</p>
                        ...
                        <div class="bg-yellow-100 p-3 rounded-lg border border-yellow-400 shadow-inner">
                            ...
                            <p class="text-xs font-bold text-gray-600 mb-1 uppercase tracking-wide">Physical Balance Sheet</p>
                            ...
                            <label class="text-xs font-semibold text-gray-900" id="d_crypto_label">Crypto ETPs: </label><input type="range" id="d_crypto" min="0" max="100" step="0.01" value="0.0" class="w-full mb-1" oninput="updateUI()">
            ...
            <!-- RIGHT COLUMN: Diagnostics & Theory (5 Columns wide) -->
            <div class="xl:col-span-5 flex flex-col gap-6">
                ...
                <!-- MASTER INSTRUMENT MATRIX -->
                <div class="bg-white border rounded-xl p-4 shadow-sm overflow-x-auto">
                    <h3 class="text-xl font-bold mb-4 text-gray-800 border-b pb-2">3. The Master Instrument Matrix</h3>
                    <table class="min-w-full bg-white text-[11px] whitespace-nowrap">
                        <thead class="bg-slate-800 text-white">
                            <tr>
                                <th class="py-2 px-2 text-left">Instrument</th>
                                <th class="py-2 px-2 text-left">Type</th>
                                <th class="py-2 px-2 text-left">Risk Profile</th>
                                <th class="py-2 px-2 text-left">Alpha Potential</th>
                                <th class="py-2 px-2 text-left">Sharpe / Vol Impact</th>
                                <th class="py-2 px-2 text-left">Trading Strategy</th>
                                <th class="py-2 px-2 text-left">Jurisdiction</th>
                                <th class="py-2 px-2 text-center">CIO Min<br>Alloc. %</th>
                                <th class="py-2 px-2 text-center">CIO Max<br>Alloc. %</th>
                                <th class="py-2 px-2 text-center">CIO<br>Grading</th>
                                <th class="py-2 px-2 text-left">Noteworthy Comments</th>
                            </tr>
                        </thead>
    ...
    <script>
        ...
        const instruments =[
            ...
            { inst: "XUSE & EIMI", type: "UCITS ETF", risk: "Moderate", alpha: "Moderate", sharpe: "High", strat: "Long Term", jur: "Ireland", min: "0%", max: "40%", rec: "Great", class: "great", comm: "Geographic diversification. Hedges USD decline." },
        ...
        function populateTable() {
            ...
            instruments.forEach(i => {
                html += `<tr class="hover:bg-gray-50 border-b border-gray-100">
                    <td class="py-2 px-2 font-semibold text-gray-900">${i.inst}</td>
                    <td class="py-2 px-2">${i.type}</td>
                    <td class="py-2 px-2">${i.risk}</td>
                    <td class="py-2 px-2">${i.alpha}</td>
                    <td class="py-2 px-2 font-medium text-indigo-700">${i.sharpe}</td>
                    <td class="py-2 px-2">${i.strat}</td>
                    <td class="py-2 px-2">${i.jur}</td>
                    <td class="py-2 px-2 font-bold text-center text-gray-500">${i.min}</td>
                    <td class="py-2 px-2 font-bold text-center">${i.max}</td>
                    <td class="py-2 px-2 ${i.class} text-center rounded shadow-sm">${i.rec}</td>
                    <td class="py-2 px-2 text-xs text-gray-700">${i.comm}</td>
                </tr>`;
            });
        ...
        function updateUI() {
            ...
            let a_tot = a_ib01 + a_cspx + a_xuse + a_eimi + a_crypto + a_cash + a_opt_mkt;
            ...
            const amt_A_eimi = sA * (a_eimi/100); const amt_A_crypto = sA * (a_crypto/100); const amt_A_cash = sA * (a_cash/100); 
            ...
            let c_tot = c_ib01 + c_cspx + c_xuse + c_eimi + c_crypto + c_cash + c_opt_mkt;
            ...
            const amt_C_eimi = sC * (c_eimi/100); const amt_C_crypto = sC * (c_crypto/100); const amt_C_cash = sC * (c_cash/100); 
            ...
            let d_tot = d_ib01 + d_cspx + d_xuse + d_eimi + d_crypto + d_cash;
            ...
            const amt_D_eimi = sD * (d_eimi/100); const amt_D_crypto = sD * (d_crypto/100); const amt_D_cash = sD * (d_cash/100);
            ...
            let rawPieData =[
                ...
                {v: tot_xuse, l: `XUSE: ${fmtCur(tot_xuse)}`, c: palette.xuse},
                {v: tot_eimi, l: `EIMI: ${fmtCur(tot_eimi)}`, c: palette.eimi},
...
for acc, prefix in[("ESTATE", "ESTATE"), ("U23144948", "A"), ("U23139264", "B"), ("U23154199", "C"), ("U25218481", "D")]:
    ...
    html_content = html_content.replace(f"VAR_{prefix}_NAV_FMT", f"${nav_val:,.2f}")
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v18 created.")
