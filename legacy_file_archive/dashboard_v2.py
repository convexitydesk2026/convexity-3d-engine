"""
=============================================================================
Script Name: dashboard_v2.py
Purpose: The Streamlit Frontend (Module 2).
         Adds the "Dead Air" filter, fixes console warnings, 
         and renders the 4 Individual Silo Performance Panels.
=============================================================================
"""
...
def load_and_process_data():
    ...
    global_df = global_df[global_df['nav'] > 0].copy() # Filter global dead air
    global_df['cum_return'] = (1 + ((global_df['nav'] - global_df['cash_flow'] - global_df['nav'].shift(1)) / global_df['nav'].shift(1).replace(0, np.nan)).fillna(0)).cumprod() - 1
    ...
    for acc in SILO_MAP.keys():
        ...
        if not acc_df.empty:
            acc_df['cum_return'] = (1 + ((acc_df['nav'] - acc_df['cash_flow'] - acc_df['nav'].shift(1)) / acc_df['nav'].shift(1).replace(0, np.nan)).fillna(0)).cumprod() - 1
        silo_dfs[acc] = acc_df

    return global_df, global_metrics, silo_dfs, silo_metrics

@st.cache_data(ttl=3600)
def load_benchmarks(start_date, end_date):
    # auto_adjust=False fixes the warning
...
# Load Data
global_df, global_metrics, silo_dfs, silo_metrics = load_and_process_data()
...
fig.update_layout(
    yaxis_ticksuffix="%", margin=dict(l=20, r=20, t=30, b=20),
