...
3. Launch the UI by running exactly:
   streamlit run dashboard_v90c.py

CHANGELOG (v90c):
         - STAGE 7 (v90c): Deployed the SMA Trailing Stop Tracker for the Alpha Risk Ledger.
         - STAGE 6 (v90b): Deployed the Pre-Earnings Ejection Protocol via yfinance caching.
         - STAGE 5 (v90a): Injected RNG Seeding into the Monte Carlo engine to stabilize VaR calculations.
         - STAGE 4: Upgraded Post-Mortem Analytics into a 4D Behavioral Bubble Chart.
...
def process_metrics(df_acc, rf_rate):
    ...
    # v90 Absolute Truth Math: Start NAV is inherently 0
...
def load_and_process_data():
    ...
    try:
        ...
        # v90 FIX: Robust NAV Forward Fill (Solves the Phantom Volatility Jump)
    ...
    # Global tracking arrays (v90: Pure Absolute Math across all Silos. Kills the Whipsaw)
...
if not df_alpha.empty:
    ...
    # v90c: Fetch and inject SMAs for the Trailing Stop Audit
    sma20_list, sma50_list = [], []
    for tckr in display_alpha['Ticker']:
    ...
    def style_alpha_row(row):
        ...
        for col in ['Locked Profit', 'Unlocked Profit', 'Total Profit']:
            if row[col] > 0:
                styles[col] = 'color: #16a34a; font-weight:bold;'
            elif row[col] < 0:
                styles[col] = 'color: #dc2626; font-weight:bold;'
                
        # The Trailing Stop Audit Warning (Highlight Yellow if lagging behind 20 SMA)
