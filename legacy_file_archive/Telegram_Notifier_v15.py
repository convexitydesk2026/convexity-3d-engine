"""
=============================================================================
Script Name: Telegram_Notifier_v15.py
Purpose: Headless Streamlit rendering and Telegram transmission.
         - NEW: Active CFO Push Notifications (22-DTE, VIX Crush, Alpha IV).
         - Updated to target dashboard_v68.py
=============================================================================
"""
...
def evaluate_cfo_alerts():
    """Scans the database and APIs for high-priority CFO alerts."""
    ...
    try:
        ...
        try:
            ...
            for _, row in wl_df.iterrows():
                if row['current_iv'] >= row['target_iv'] and row['current_iv'] > 0:
                    priority_alerts.append(f"🎯 *ALPHA OPPORTUNITY ({row['symbol']}):*\nLive IV ({row['current_iv']:.1f}%) has breached your target ({row['target_iv']:.1f}%). Ready for Conviction CSPs.")
        except Exception as e:
    ...
    # Transmit Standalone Text Message if Alerts Exist
    if priority_alerts:
...
def snap_and_send():
    # 1. Check for standalone alerts first
    evaluate_cfo_alerts()
    
    print("\n[NOTIFIER] Spinning up headless Streamlit server (Invisible Background Process)...")
    # v15 Update: Pointing to dashboard_v70.py
    streamlit_process = subprocess.Popen(["streamlit", "run", "dashboard_v70.py", "--server.port", "8599", "--server.headless", "true"],
        cwd=TARGET_DIR,
