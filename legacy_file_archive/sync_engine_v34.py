"""
=============================================================================
Script Name: sync_engine_v34.py
Purpose: Phase 11 of the Local-First Estate Architecture.
         - UPDATED: v34 - Upgraded Alpha Campaign Status Pipeline to support Short positions (position != 0) and automatically assign Long/Short direction upon promotion.
         - UPDATED: v33 - Added Alpha Campaign Status Pipeline Automation (Auto-Promotion).
         - UPDATED: v32a - Suppressed yfinance logger to neutralize 404 spam.
         - RESTORED: Pristine PEP-8 Formatting (No minification).
         - NEW LOGIC: "Fetch-First" Database Write. Deletes old account 
           records ONLY AFTER the new data is successfully fetched and 
           validated by the Failsafe. Prevents ghosting on timeouts.
         - MAINTAINED: BASE currency NetLiquidation (Foreign Asset Fix).
         - MAINTAINED: Historical IV Scanner Loophole.
=============================================================================
"""
...
def sync_tws():
    print("========================================================")
    print("   ESTATE SYNC ENGINE v32 - FETCH-FIRST LOGIC")
    print("========================================================")    
    ...
    # We can safely delete ALL open orders for today and replace them, 
    # because reqAllOpenOrders() fetches globally and rarely drops data.
    ...
    # -------------------------------------------------------------------------
    # PER-ACCOUNT FETCH & VALIDATION LOGIC
    # -------------------------------------------------------------------------
    for acc in accounts:        
        ...
        try:
            ...
            try: 
                ...
                print(f"[!] Warning: {acc} timed out on account updates.")
        ...
        # Prioritize the global BASE currency for accurate foreign asset representation
        ...
        # --- THE STRICT DRAWDOWN FAILSAFE ---
        if net_liq_final < 100.0:
            print(f"[!] FAILSAFE TRIGGERED: {acc} returned ${net_liq_final:.2f}. Likely API timeout.")
            print(f"    -> Aborting database write for {acc}. Previous records remain intact.")
            if ib.isConnected(): 
                ib.disconnect()
            continue
            
        # =========================================================================
        # DATABASE WRITE PHASE (Only executes if Failsafe passes)
        # =========================================================================
        
        # 1. Now that we have verified data, delete ONLY THIS ACCOUNT'S old data for today
        ...
        # 2. Insert the verified balances
        cursor.execute(
        ...
        # 3. Insert the verified positions
    ...
    # -------------------------------------------------------------------------
    # ALPHA CAMPAIGN STATUS PIPELINE AUTOMATION
    # -------------------------------------------------------------------------
    print("[*] Processing Alpha Campaign Status Pipeline...")
    try:
        # 1. Auto-Promote to 'Armed' if a resting order is found
        cursor.execute("""
            UPDATE alpha_campaigns 
            SET status = 'Armed 🎯' 
            WHERE status IN ('Stalking 🔭', 'Waiting ⏳') 
            AND symbol IN (SELECT symbol FROM open_orders)
        """)
        
        # 2. Auto-Promote to 'Open' if a physical position is found (Handles Longs and Shorts)
        cursor.execute("""
            UPDATE alpha_campaigns 
            SET status = 'Open 🟢', 
                open_date = ?, 
                entry_price = (SELECT avg_cost FROM daily_positions WHERE daily_positions.symbol = alpha_campaigns.symbol LIMIT 1),
                type = CASE WHEN (SELECT position FROM daily_positions WHERE daily_positions.symbol = alpha_campaigns.symbol LIMIT 1) > 0 THEN 'Long' ELSE 'Short' END
            WHERE status IN ('Stalking 🔭', 'Waiting ⏳', 'Armed 🎯') 
            AND symbol IN (SELECT symbol FROM daily_positions WHERE position != 0 AND sec_type IN ('STK', 'CFD'))
        """, (today_str,))
    ...
    # -------------------------------------------------------------------------
    # ALPHA WATCHLIST IV SCANNER (Historical Request Bypass)
    # -------------------------------------------------------------------------
