...
streamlit run dashboard_v130.py
CHANGELOG (v130):
...
if not df_closed.empty:
    ...
    # v130 FIX: Bypass Styler formatting issues by reading edits directly from Streamlit session state
    changed = False
    if "closed_camp_editor" in st.session_state:
        edits = st.session_state["closed_camp_editor"].get("edited_rows", {})
        if edits:
            for row_idx, row_edits in edits.items():
                db_id = df_closed.at[row_idx, 'id']
                
                if "tags" in row_edits:
                    raw_tag = row_edits["tags"]
                    new_tags = str(raw_tag) if raw_tag is not None else ""
                else:
                    new_tags = str(df_closed.at[row_idx, 'tags']) if pd.notna(df_closed.at[row_idx, 'tags']) else ""
                    
                if "thesis" in row_edits:
                    raw_thesis = row_edits["thesis"]
                    new_thesis = str(raw_thesis) if raw_thesis is not None else ""
                else:
                    new_thesis = str(df_closed.at[row_idx, 'thesis']) if pd.notna(df_closed.at[row_idx, 'thesis']) else ""
                
                cj.execute("UPDATE alpha_campaigns SET tags=?, thesis=? WHERE id=?", (new_tags, new_thesis, db_id))
                changed = True

    if changed:
        conn_journal.commit()
        del st.session_state["closed_camp_editor"]
        st.rerun()
