...
with calc_placeholder.container():
    ...
    # 2. Check 48-Hour Revenge Trading Lockout
    conn_lock = sqlite3.connect(DB_PATH)
    c_lock = conn_lock.cursor()
    c_lock.execute("SELECT COUNT(*) FROM alpha_campaigns WHERE status IN ('Closed', 'Closed 🏁') AND total_pnl < 0 AND close_date >= date('now', '-2 days')")
