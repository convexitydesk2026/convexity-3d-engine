...
3. Launch the UI by running exactly:
   streamlit run dashboard_v64.py

CHANGELOG (v64):
         - PATCHED (BUG C): Fixed mathematical PnL mismatch. Replaced delta-NAV guessing with 100% verified clearinghouse data from attribution_engine.
         - PATCHED: Hardened global_df aggregation via pivot_table ffill() to prevent extreme NAV drops on missed silo syncs.
=============================================================================
"""
...
def fetch_live_data(ticker_symbol):
    # v64: Safeguard against empty cells during live data-entry
...
def get_exact_opt_margin(df_in):
    ...
    try:
        ...
        # v64 FIX: Added base_tckr to the groupby!
    ...
    if not global_df.empty:
        ...
        try:
            ...
            if not temp_attr.empty:
                ...
                global_df['daily_pnl'] = global_df['true_daily_pnl'].fillna(0.0)
                global_df['daily_return'] = global_df['daily_pnl'] / global_df['prev_nav'].replace(0, np.nan)
                
                # Override the global metrics total PnL with official data
...
# v64 FIX: Added Physical US Stocks & International Stocks to Core Assets pool
...
if not journal_raw_df.empty:
    ...
    if not active_vrp.empty:
        ...
        # v64: Dynamically map LIVE TWS PnL to calculate Live Annualized ROC
        live_roc_list = []
        live_ann_roc_list = []
...
if not journal_raw_df.empty:
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            # v64: 3D Engine Native Debit Spread & Call Detection
