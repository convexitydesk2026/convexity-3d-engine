...
streamlit run dashboard_v132.py
CHANGELOG (v132):
- STAGE 40 (v132): Integrated Tag Glossary, Algorithmic Auto-Tagging for Mathematical Edge, Retro-Tagging capability, and a Multiselect UI for open campaigns to streamline user entry.
- STAGE 39 (v131): Defeated the Streamlit Styler memory wipe bug. Bound an `on_change` callback to the Closed Campaigns editor to intercept and save SQLite edits strictly before the app reruns.
- STAGE 38 (v130): Bypassed Streamlit Styler edit-mapping failures by reading table edits directly from the raw `st.session_state` JSON payload. Retroactive tag enrichment now saves perfectly.
- STAGE 37 (v129): Defeated the infinite Streamlit editor loop. Enforced strict deterministic SQLite sorting (`ORDER BY close_date DESC, id DESC`) and added a session_state wipe to prevent ghost-edits from transferring across row indices.
- STAGE 36 (v128): Unlocked 'tags' and 'thesis' columns in the Closed Campaigns table for retrospective data enrichment. Implemented a targeted edit-validation loop to preserve cryptographically locked financial columns.
- STAGE 35 (v127): Fixed the Calendar UI rendering bug. Restored 12-space indentation to `st.markdown` to place it safely back inside the collapsible `st.expander` container.
- STAGE 34 (v126): Reorganized the Quick Navigation sidebar to perfectly match the visual flow of the UI. Added missing anchors for Market Flow (1C) and the Accountability Journal (10). 
=============================================================================
"""
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v32a.py")
...
with st.sidebar:
    ...
    st.markdown("""
    - [🔝 Top / Alerts](#top)
    - [Master Aggregation](#master-agg)
    - [1B. Return Heatmap](#sec1b)
    - [1C. Global Market Flow](#sec1c)
    - [1. Capital Breakdown](#sec1)
    - [2. Portfolio Composition](#sec2)
    - [3. Daily PnL per Silo](#sec3)
    - [3B. Alpha & Risk Ledger](#sec3b)
    - [4. PnL Attribution](#sec4)
    - [5. Command Center](#sec5)
    - [6. Margin & Risk](#sec6)
    - [6B. Advanced Risk Metrics](#sec6b)
    - [6C. S.W.A.N. Stress Test](#sec6c)
    - [7. Instrument Matrix](#sec7)
    - [8. Monte Carlo Simulation](#sec8)
    - [9A. Options Matrix](#sec9a)
    - [9B. Options Ledger](#sec9b)
    - [10. Accountability Journal](#sec10)
    """)
    st.markdown("---")
    ...
    if st.button("🌐 Generate Market Flow Report", width="stretch"):
        with st.spinner("Calculating Institutional Flow & Generating PDF... Please wait (~15s)"):
            try:
                subprocess.run(["python", "market_flow_engine_v4.py"], cwd=TARGET_DIR, check=True)
...
def process_metrics(df_acc, rf_rate):
    ...
    # v91 Absolute Truth Math: Start NAV is inherently 0
    ...
    # Total PnL is mathematically forced to strictly equal (Live NAV - All Time Net Flows)
...
def get_exact_opt_margin(df_in):
    ...
    try:
        ...
        # v75 FIX: Added base_tckr to the groupby!
...
with calc_placeholder.container():
    st.markdown("### 🧮 Alpha Risk Calculator")
    with st.expander("Position Sizing Engine", expanded=False):
        calc_silo = st.selectbox("Target Silo", ['U23144948 (Silo A)', 'U23139264 (Silo B)', 'U23154199 (Silo C)', 'U25218481 (Silo D)'], key="calc_silo")
        ...
        c1, c2 = st.columns(2)
        calc_risk_pct = c1.number_input("Risk %", min_value=0.01, max_value=5.0, value=0.10, step=0.05, format="%.2f")
        ...
        if calc_mode == "Scale-In (Pyramid)":
            ...
            if active_alpha.empty:
                ...
                # CFO Truth Query: Look for active resting stops directly on the exchange
                ...
                # Check for Slippage / Removed Stop Violations
                is_violation = False
                ...
                if is_violation:
                    ...
                    if not valid_new_stop and input_sl > 0:
                        ...
                        if risk_per_share_usd <= 0:
                            ...
                            st.markdown(f"**Max additional shares allowed:** {proposed_shares:,}")
                            is_valid_pyramid = True
        else:
            calc_dir = c2.selectbox("Direction", ["Long", "Short"])
            calc_currency = st.selectbox("Asset Currency", ['USD', 'SEK', 'TWD', 'HKD', 'KRW', 'CNY', 'JPY', 'EUR', 'GBP', 'CAD', 'ILS'])
        ...
        if calc_entry > 0 and calc_stop > 0 and calc_nav > 0:
            ...
            if valid_dir:
                # MACRO GOVERNOR INITIALIZATION
                temp_bench = load_benchmarks(global_df['date'].min(), global_df['date'].max())
                temp_chart = pd.merge(global_df, temp_bench, on='date', how='left').ffill().fillna(0)
                regime_tor_val = temp_chart['tor'].iloc[-1] if not temp_chart.empty else 0
                
                # Halved Macro Risk Logic
                daily_tor_pct = regime_tor_val / 2.0  
                max_global_risk_usd = global_metrics['nav'] * (daily_tor_pct / 100.0)

                if daily_tor_pct <= 0.0:
                    st.error("🚨 **REGIME ZERO (Absolute Block):** Allowable Global TOR is 0.0%. Absolutely no new positions can be opened in these circumstances. Protect capital.")
                    valid_dir = False
            ...
            if valid_dir:
                ...
                if valid_dir and proposed_shares > 0 and (calc_mode != "Scale-In (Pyramid)" or is_valid_pyramid):

                    proposed_added_risk_usd = proposed_shares * new_risk_per_share_usd
                    
                    # MACRO GOVERNOR VERIFICATION
                ...
                elif valid_dir and proposed_shares == 0:
...
alerts["info"].append(f"🧭 **EOD Market Regime:** {regime_str}")
...
try:
    ...
    conn_wl.close()
    if not wl_df_alert.empty and regime_str in ['Red', 'Yellow']:
        for _, w_row in wl_df_alert.iterrows():
            if w_row['current_iv'] >= w_row['target_iv']:
                alerts["opportunity"].append(f"🎯 **ALPHA OPPORTUNITY ({w_row['symbol']}):** Live IV ({w_row['current_iv']:.1f}%) has breached your target ({w_row['target_iv']:.1f}%). Market regime is {regime_str}. Sell Conviction CSPs now.")
except Exception:
    pass
...
if not df_phys.empty:
    for sym, g in df_phys.groupby('symbol'):
        ...
        for _, sr in sym_stops.iterrows():
            ...
            # If the calculated Stop Loss is ~100x larger than the actual Spot Price, correct the multiplier.
...
if not df_alpha.empty:
    ...
    def style_alpha_row(row):
        ...
        # The Trailing Stop Audit Warning
        if row['Total Profit'] > 0 and row['20 SMA'] > 0 and row['Avg SL'] < (row['20 SMA'] * 0.99):
    ...
    st.dataframe(display_alpha.style.format({
        ...
        'Total Profit': '${:,.0f}', 'Live R-Mult': '{:+.2f}R'
...
if not journal_raw_df.empty and not global_df.empty and not bench_df.empty:
    ...
    st.info("Insufficient historical data to render Capital Discipline Tracker.")
...
st.subheader("7. The Master Instrument Matrix (Tax, Alpha, & Sharpe Grading)", anchor="sec7")
matrix_data = [
...
with st.expander("📊 Click to expand the Institutional Instrument Matrix & Tax Architecture", expanded=False):
    st.dataframe(
...
def generate_mc_paths(pnl_array):
    # STAGE 5 FIX: Freeze the RNG seed using the exact dataset length.
    # Stops the weekend "bouncing" when the cache is cleared.
...
vix_warn = "🚨 COMPLACENT (Strict Sizing)" if vix_live < 15 else ("🟡 NORMAL" if vix_live < 20 else "🟢 ELEVATED (Prime VRP)")

st.markdown(f"""
<div style="background-color: #f8fafc; border: 1px solid #cbd5e1; border-left: 5px solid #3b82f6; padding: 12px 20px; border-radius: 6px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
    <div>
        <div style="font-size: 12px; color: #64748b; font-weight: bold; text-transform: uppercase;">Pre-Flight Execution Checklist</div>
        <div style="font-size: 15px; color: #0f172a;"><b>Remaining VRP Margin Capacity:</b> <span style="color: {'#dc2626' if remaining_margin < 10000 else '#16a34a'};">${remaining_margin:,.0f}</span> (Hard Cap: 20% NAV)</div>
    </div>
...
if not journal_raw_df.empty:
    ...
    if not active_vrp.empty:
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["🎯 VRP Income Engine", "🛒 Assignment Radar (CSPs)", "🚀 Synthetic Beta Conveyor", "🛡️ Catastrophe Multiplier", "🔬 Closed Trade Expectancy"])
...
col_scan, col_close = st.columns(2)

with col_scan:
    if st.button("🔍 Scan for Undocumented Campaigns", width="stretch"):
        ...
        for _, r in open_positions.iterrows():
            sym = r['symbol']
            cj.execute("SELECT id FROM alpha_campaigns WHERE symbol=? AND status='Open'", (sym,))
            if not cj.fetchone():
                ...
                # -------------------------------------------------------------
                # ALGORITHMIC AUTO-TAGGING ENGINE
                # -------------------------------------------------------------
                auto_tags = []
                
                # 1. Regime
                if regime_str == 'Green': auto_tags.append('Regime_Green')
                elif regime_str == 'Yellow': auto_tags.append('Regime_Yellow')
                elif regime_str == 'Red': auto_tags.append('Regime_Red')
                
                # 2. Moving Averages
                if s20 > 0: auto_tags.append('>20SMA' if entry_usd > s20 else '<20SMA')
                if s50 > 0: auto_tags.append('>50SMA' if entry_usd > s50 else '<50SMA')
                if s200 > 0: auto_tags.append('>200SMA' if entry_usd > s200 else '<200SMA')
                
                # 3. Direction
                auto_tags.append(trade_type)
                
                # 4. Sector
                if sec and sec != 'Unknown Equities':
                    sec_tag = f"Sector_{sec.replace(' ', '')}"
                    auto_tags.append(sec_tag)
                    
                tag_str = ", ".join(auto_tags)
                
                cj.execute("""
                    INSERT INTO alpha_campaigns 
                    (symbol, type, status, open_date, regime_in, sector, industry, sma_20, sma_50, sma_200, entry_price, tags)
                    VALUES (?, ?, 'Open', ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (sym, trade_type, datetime.date.today().isoformat(), regime_str, sec, ind, s20, s50, s200, entry_usd, tag_str))
                
                conn_journal.commit()
                new_campaigns += 1
                
        if new_campaigns > 0:
            st.success(f"Generated {new_campaigns} new campaigns! Please input your Initial Stops and Thesis below.")
            st.rerun()
        else:
            st.info("No undocumented campaigns found in current portfolio.")

with col_close:
    if st.button("🏁 Check for Closed Campaigns", width="stretch"):
        # Fetch both Open and Pending campaigns to process T+1 Settlement Delay
        cj.execute("SELECT id, symbol, open_date, entry_price, initial_stop, status FROM alpha_campaigns WHERE status IN ('Open', 'Pending Settlement ⏳')")
        ...
        if msg_parts:
            ...
            st.info("No actionable changes detected in open campaigns today.")

# -------------------------------------------------------------------------
# STRATEGY TAG GLOSSARY & SOPS
# -------------------------------------------------------------------------

# 1. Mathematical Tag Dictionary (Auto-Descriptions)
MATH_TAG_DICT = {
    'Regime_Green': 'Trade was initiated during a Green Macro Regime (Bull/Low Volatility).',
    'Regime_Yellow': 'Trade was initiated during a Yellow Macro Regime (Transition/Chop).',
    'Regime_Red': 'Trade was initiated during a Red Macro Regime (Bear/High Volatility).',
    '>20SMA': 'Entry price was mathematically verified to be above the 20-day Simple Moving Average.',
    '<20SMA': 'Entry price was mathematically verified to be below the 20-day Simple Moving Average.',
    '>50SMA': 'Entry price was mathematically verified to be above the 50-day Simple Moving Average.',
    '<50SMA': 'Entry price was mathematically verified to be below the 50-day Simple Moving Average.',
    '>200SMA': 'Entry price was mathematically verified to be above the 200-day Simple Moving Average.',
    '<200SMA': 'Entry price was mathematically verified to be below the 200-day Simple Moving Average.',
    'Long': 'Directional bet on asset appreciation.',
    'Short': 'Directional bet on asset depreciation.'
}
...
for row in db_tags_rows:
    ...
    if raw_tag_str:
        ...
        for t in split_tags:
            if t:
                auto_desc = MATH_TAG_DICT.get(t, f"Sector definition for {t.replace('Sector_', '')}" if "Sector_" in t else "")
                cj.execute("INSERT OR IGNORE INTO tag_glossary (tag, description) VALUES (?, ?)", (t, auto_desc))
conn_journal.commit()

with st.expander("📖 Strategy Tag Glossary & SOPs", expanded=False):
    c_glos, c_retro = st.columns([3, 1])
    with c_glos:
        ...
        def commit_glossary_edits():
            edits = st.session_state["glossary_editor"].get("edited_rows", {})
            if not edits: return
        ...
        st.data_editor(
            df_glossary,
            column_config={
                "tag": st.column_config.TextColumn("Tag (Auto-Harvested)", disabled=True),
    ...
    with c_retro:
        st.markdown("<div style='background-color: #f8fafc; padding: 15px; border-radius: 6px; border: 1px solid #cbd5e1;'><b>Retro-Tag Engine</b><br><br><span style='font-size:12px; color:#475569;'>Click below to scan all historical campaigns and safely inject mathematical tags (SMAs, Regimes, Sectors) without overwriting manual tags.</span></div>", unsafe_allow_html=True)
        if st.button("🔄 Retro-Tag Historical Campaigns", width="stretch"):
            cj.execute("SELECT id, symbol, type, regime_in, sector, sma_20, sma_50, sma_200, entry_price, tags FROM alpha_campaigns")
            hist_camps = cj.fetchall()
            updated = 0
            for c_id, sym, c_type, regime, sec, s20, s50, s200, entry, ex_tags in hist_camps:
                existing = [t.strip() for t in str(ex_tags).split(',')] if ex_tags else []
                math_t = []
                if regime == 'Green': math_t.append('Regime_Green')
                elif regime == 'Yellow': math_t.append('Regime_Yellow')
                elif regime == 'Red': math_t.append('Regime_Red')
                
                if s20 and entry: math_t.append('>20SMA' if entry > s20 else '<20SMA')
                if s50 and entry: math_t.append('>50SMA' if entry > s50 else '<50SMA')
                if s200 and entry: math_t.append('>200SMA' if entry > s200 else '<200SMA')
                
                if c_type: math_t.append(c_type)
                if sec and sec != 'Unknown Equities': math_t.append(f"Sector_{sec.replace(' ', '')}")
                
                new_tags = list(set(existing + math_t))
                new_tags = [t for t in new_tags if t]
                new_tag_str = ", ".join(new_tags)
                
                if new_tag_str != ex_tags:
                    cj.execute("UPDATE alpha_campaigns SET tags=? WHERE id=?", (new_tag_str, c_id))
                    updated += 1
            conn_journal.commit()
            st.success(f"Successfully applied mathematical tags to {updated} historical campaigns.")
            st.rerun()

st.markdown("##### 🟢 Active Campaigns (Action Required: Set Stops & Tags)")
df_open = pd.read_sql_query("SELECT id, open_date, symbol, status, type, regime_in, sector, industry, sma_20, sma_50, sma_200, entry_price, initial_stop, tags, thesis, days_active FROM alpha_campaigns WHERE status IN ('Open', 'Pending Settlement ⏳')", conn_journal)
...
if not df_open.empty:
    ...
    # Multiselect UI Loop for each Open Campaign
    for idx, row in df_open.iterrows():
        with st.container():
            ...
            c_sl, c_tags, c_thes, c_save = st.columns([1.5, 3, 3, 1])
            
            current_tags = [t.strip() for t in str(row['tags']).split(',')] if row['tags'] else []
            # Safety: Ensure current tags exist in options to prevent Streamlit widget errors
            for t in current_tags:
                if t not in all_tags_list:
                    all_tags_list.append(t)
            
            with c_sl:
                new_stop = st.number_input("Initial Stop Loss", value=float(row['initial_stop']), min_value=0.0, step=1.0, key=f"sl_{row['id']}")
            with c_tags:
                sel_tags = st.multiselect("Tags", options=all_tags_list, default=current_tags, key=f"tags_{row['id']}")
            with c_thes:
                new_thesis = st.text_input("Thesis / Comment", value=str(row['thesis']) if pd.notna(row['thesis']) else "", key=f"thes_{row['id']}")
            with c_save:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("Save", key=f"save_{row['id']}", use_container_width=True):
                    new_tag_str = ", ".join(sel_tags)
                    cj.execute("UPDATE alpha_campaigns SET initial_stop=?, tags=?, thesis=? WHERE id=?", (new_stop, new_tag_str, new_thesis, row['id']))
                    conn_journal.commit()
                    st.rerun()
        st.markdown("<hr style='margin: 10px 0; border: none; border-bottom: 1px solid #e5e7eb;'>", unsafe_allow_html=True)
    
    st.markdown("<br><b>Active Campaigns Overview</b>", unsafe_allow_html=True)
    display_open = df_open.drop(columns=['id', 'tags', 'thesis', 'initial_stop'])
    st.dataframe(display_open.style.format({"sma_20": "{:.2f}", "sma_50": "{:.2f}", "sma_200": "{:.2f}", "entry_price": "{:.2f}"}), hide_index=True, width="stretch")
    
else:
    st.caption("No open campaigns currently undocumented.")

st.markdown("##### 🏁 Closed Campaigns (Post-Mortem & Grading)")

# We must select 'id' to map edits back to the specific row in the database
# v129 FIX: Added id DESC to strictly enforce deterministic sorting and prevent infinite editor loops
df_closed = pd.read_sql_query("SELECT id, open_date, close_date, symbol, type, regime_in, sector, industry, entry_price, initial_stop, tags, thesis, days_active, total_pnl, r_multiple, grade FROM alpha_campaigns WHERE status='Closed' ORDER BY close_date DESC, id DESC", conn_journal)

if not df_closed.empty:
    st.session_state['df_closed_cache'] = df_closed.copy()
    
    def commit_closed_edits():
        # v131 FIX: Callback intercepts Styler edits before Streamlit discards them
    ...
    st.download_button("💾 Download Accountability Journal (CSV)", data=csv_data, file_name=f"Alpha_Journal_Closed_{datetime.date.today().isoformat()}.csv", mime="text/csv")
    ...
    if not exploded_df.empty:
        st.markdown("<br>", unsafe_allow_html=True)
        with st.expander("📊 Expand Edge Analytics (Tag-Based Performance)", expanded=False):
            ...
            with c_tbl:
                ...
                st.markdown("**Edge Verification Ledger**")
    ...
    st.caption("No closed campaigns recorded yet.")
