r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v7.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 7: 
         - 0.1% slider granularity.
         - Separates "Physical Assets" (100%) from "Margin Utilization".
         - Red Diamond overlay on Bar Chart to visualize Margin locked.
         - Pie chart stripped of margin to reflect true 100% Estate exposure.
         - Dual labels: Instrument: $Amount (X.X%)
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v7.html")

if not os.path.exists(target_directory):
    os.makedirs(target_directory)

# ---------------------------------------------------------
# 2. The Quantitative Math Engine (XIRR & Sharpe)
# ---------------------------------------------------------
...
def process_account_metrics(df, account_id):
    ...
    return irr_str, sharpe_str

def get_color_class(val_str):
    if "N/A" in val_str or "Awaiting" in val_str or "--" in val_str: return "text-gray-400"
    if "-" in val_str: return "text-red-400"
...
# ---------------------------------------------------------
metrics = {
    "U23144948": {"irr": "Awaiting Data", "sharpe": "--"},
    "U23139264": {"irr": "Awaiting Data", "sharpe": "--"},
    "U23154199": {"irr": "Awaiting Data", "sharpe": "--"},
    "U25218481": {"irr": "Awaiting Data", "sharpe": "--"}
}
...
if os.path.exists(csv_file_path):
    try:
        ...
        for acc in metrics.keys():
            irr, sharpe = process_account_metrics(df, acc)
            metrics[acc] = {"irr": irr, "sharpe": sharpe}
...
<head>
    ...
    <title>Family Estate Master Allocator v7</title>
    ...
    <style>
        ...
        .bg-yellow-theme { background-color: #ca8a04; }
        .border-yellow-theme { border-color: #eab308; }
        .text-yellow-theme { color: #fef08a; }
    </style>
</head>
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[1800px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v7</p>
        </div>
        ...
        <div class="mb-8 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-blue-900 text-white p-4 rounded-xl shadow-md text-center border-b-4 border-blue-500">
                ...
                <p class="text-xs mb-1">Persons 1 and 2<br>U23144948</p>
                <div class="flex justify-between text-sm mt-2 border-t border-blue-700 pt-2 font-mono">
                    <span>IRR: <span class="VAR_COLOR_A_IRR font-bold">VAR_A_IRR</span></span>
                    <span>Sharpe: <span class="VAR_COLOR_A_SHARPE font-bold">VAR_A_SHARPE</span></span>
                </div>
            </div>
            <div class="bg-purple-900 text-white p-4 rounded-xl shadow-md text-center border-b-4 border-purple-500">
                ...
                <p class="text-xs mb-1">Persons 1 and 2<br>U23139264</p>
                <div class="flex justify-between text-sm mt-2 border-t border-purple-700 pt-2 font-mono">
                    <span>IRR: <span class="VAR_COLOR_B_IRR font-bold">VAR_B_IRR</span></span>
                    <span>Sharpe: <span class="VAR_COLOR_B_SHARPE font-bold">VAR_B_SHARPE</span></span>
                </div>
            </div>
            <div class="bg-green-900 text-white p-4 rounded-xl shadow-md text-center border-b-4 border-green-500">
                ...
                <p class="text-xs mb-1">Persons 1 and 3<br>U23154199</p>
                <div class="flex justify-between text-sm mt-2 border-t border-green-700 pt-2 font-mono">
                    <span>IRR: <span class="VAR_COLOR_C_IRR font-bold">VAR_C_IRR</span></span>
                    <span>Sharpe: <span class="VAR_COLOR_C_SHARPE font-bold">VAR_C_SHARPE</span></span>
                </div>
            </div>
            <div class="bg-yellow-theme text-white p-4 rounded-xl shadow-md text-center border-b-4 border-yellow-theme">
                <h4 class="text-sm text-yellow-theme uppercase tracking-widest font-bold">Silo D</h4>
                <p class="text-xs mb-1">Persons 1 and 4<br>U25218481</p>
                <div class="flex justify-between text-sm mt-2 border-t border-yellow-500 pt-2 font-mono">
                    <span>IRR: <span class="VAR_COLOR_D_IRR font-bold">VAR_D_IRR</span></span>
                    <span>Sharpe: <span class="VAR_COLOR_D_SHARPE font-bold">VAR_D_SHARPE</span></span>
                </div>
            </div>
        </div>
        ...
        <div class="mb-8 bg-gray-50 p-4 rounded-xl border shadow-sm">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div><label class="block text-sm font-bold text-gray-700">Silo A ($) - Vault 1</label><input type="number" id="siloA" value="599511" class="mt-1 w-full rounded-md p-2 border" oninput="updateUI()"></div>
                <div><label class="block text-sm font-bold text-gray-700">Silo B ($) - Active</label><input type="number" id="siloB" value="28774" class="mt-1 w-full rounded-md p-2 border" oninput="updateUI()"></div>
                <div><label class="block text-sm font-bold text-gray-700">Silo C ($) - Options</label><input type="number" id="siloC" value="289806" class="mt-1 w-full rounded-md p-2 border" oninput="updateUI()"></div>
                <div><label class="block text-sm font-bold text-gray-700">Silo D ($) - Vault 2</label><input type="number" id="siloD" value="150000" class="mt-1 w-full rounded-md p-2 border" oninput="updateUI()"></div>
            </div>
        </div>
        ...
        <div class="mb-8 bg-white p-4 rounded-xl border shadow-sm">
            ...
            <p class="text-sm text-gray-500 mb-4">Note: Physical assets must equal 100%. Margin limits are independent metrics overlaying your assets.</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-blue-50 p-3 rounded-lg border border-blue-200 shadow-inner">
                    ...
                    <p class="text-xs font-bold text-gray-600 mb-1 uppercase tracking-wide">Physical Assets</p>
                    <label class="text-xs font-semibold" id="a_ib01_label">IB01: </label><input type="range" id="a_ib01" min="0" max="100" step="0.1" value="97.4" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="a_cspx_label">CSPX: </label><input type="range" id="a_cspx" min="0" max="100" step="0.1" value="0.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="a_xuse_label">XUSE: </label><input type="range" id="a_xuse" min="0" max="100" step="0.1" value="0.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="a_eimi_label">EIMI: </label><input type="range" id="a_eimi" min="0" max="100" step="0.1" value="0.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="a_cash_label">Cash: </label><input type="range" id="a_cash" min="0" max="100" step="0.1" value="2.6" class="w-full mb-1" oninput="updateUI()">
                    <p class="text-xs text-red-600 font-bold mt-1 h-4" id="a_warning"></p>
                    
                    <p class="text-xs font-bold text-gray-600 mt-2 mb-1 uppercase tracking-wide border-t border-blue-200 pt-2">Margin Utilization</p>
                    <label class="text-xs font-semibold text-green-700" id="a_xsp_label">XSP Margin: </label><input type="range" id="a_xsp" min="0" max="100" step="0.1" value="20.8" class="w-full mb-1" oninput="updateUI()">
                </div>
                ...
                <div class="bg-purple-50 p-3 rounded-lg border border-purple-200 shadow-inner">
                    ...
                    <p class="text-xs font-bold text-gray-600 mb-1 uppercase tracking-wide">Physical Assets</p>
                    <label class="text-xs font-semibold" id="b_ib01_label">IB01: </label><input type="range" id="b_ib01" min="0" max="100" step="0.1" value="41.6" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="b_cash_label">Cash: </label><input type="range" id="b_cash" min="0" max="100" step="0.1" value="58.4" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="b_cfd_label">CFDs: </label><input type="range" id="b_cfd" min="0" max="100" step="0.1" value="0.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="b_intl_label">Intl Stocks: </label><input type="range" id="b_intl" min="0" max="100" step="0.1" value="0.0" class="w-full mb-1" oninput="updateUI()">
                ...
                <div class="bg-green-50 p-3 rounded-lg border border-green-200 shadow-inner">
                    ...
                    <p class="text-xs font-bold text-gray-600 mb-1 uppercase tracking-wide">Physical Assets</p>
                    <label class="text-xs font-semibold" id="c_ib01_label">IB01: </label><input type="range" id="c_ib01" min="0" max="100" step="0.1" value="99.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="c_cspx_label">CSPX: </label><input type="range" id="c_cspx" min="0" max="100" step="0.1" value="0.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="c_xuse_label">XUSE: </label><input type="range" id="c_xuse" min="0" max="100" step="0.1" value="0.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="c_eimi_label">EIMI: </label><input type="range" id="c_eimi" min="0" max="100" step="0.1" value="0.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="c_cash_label">Cash: </label><input type="range" id="c_cash" min="0" max="100" step="0.1" value="1.0" class="w-full mb-1" oninput="updateUI()">
                    <p class="text-xs text-red-600 font-bold mt-1 h-4" id="c_warning"></p>

                    <p class="text-xs font-bold text-gray-600 mt-2 mb-1 uppercase tracking-wide border-t border-green-200 pt-2">Margin Utilization</p>
                    <label class="text-xs font-semibold text-green-700" id="c_xsp_label">XSP Margin: </label><input type="range" id="c_xsp" min="0" max="100" step="0.1" value="24.2" class="w-full mb-1" oninput="updateUI()">
                </div>

                <!-- Silo D -->
                <div class="bg-yellow-50 p-3 rounded-lg border border-yellow-300 shadow-inner">
                    <h4 class="font-bold text-yellow-900 mb-2 text-sm border-b border-yellow-300 pb-1">Silo D (Vault 2)</h4>
                    <p class="text-xs font-bold text-gray-600 mb-1 uppercase tracking-wide">Physical Assets</p>
                    <label class="text-xs font-semibold" id="d_ib01_label">IB01: </label><input type="range" id="d_ib01" min="0" max="100" step="0.1" value="40.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="d_cspx_label">CSPX: </label><input type="range" id="d_cspx" min="0" max="100" step="0.1" value="30.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="d_xuse_label">XUSE: </label><input type="range" id="d_xuse" min="0" max="100" step="0.1" value="15.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="d_eimi_label">EIMI: </label><input type="range" id="d_eimi" min="0" max="100" step="0.1" value="5.0" class="w-full mb-1" oninput="updateUI()">
                    <label class="text-xs font-semibold" id="d_cash_label">Cash: </label><input type="range" id="d_cash" min="0" max="100" step="0.1" value="10.0" class="w-full mb-1" oninput="updateUI()">
    ...
    <script>
        ...
        const instruments =[
            ...
            { inst: "CSPX", type: "UCITS ETF", risk: "Moderate", alpha: "Zero (Beta 1)", sharpe: "Baseline", strat: "Long Term DCA", jur: "Ireland", min: "0%", max: "60%", rec: "Great", class: "great", comm: "<strong>Irish-domiciled S&P 500 equity fund.</strong><br>Shields gains from 40% Estate Tax and 30% Dividend Withholding." },
            ...
            { inst: "US Tech CFDs", type: "OTC Contract", risk: "Aggressive", alpha: "High", sharpe: "Negative", strat: "Swing Trading", jur: "UK/Offshore", min: "0%", max: "3%", rec: "Good", class: "good", comm: "<strong>Synthetic derivatives tracking physical US tech stocks.</strong><br>0% IRS Estate Tax risk. Subject to overnight margin fees. 3% limit quarantines Minervini trades in Silo B." },
            ...
            { inst: "US Stock Options", type: "Option", risk: "Extreme", alpha: "High", sharpe: "Negative", strat: "Speculation", jur: "US (Cboe)", min: "0%", max: "0%", rec: "Avoid", class: "avoid", comm: "<strong>Contracts granting the right to buy/sell physical US property.</strong><br>LETHAL. IRS gray area. Brokers will freeze the account for months upon death." }
        ];
        ...
        function updateUI() {
            ...
            document.getElementById('a_warning').innerText = Math.abs(a_tot - 100) > 0.05 ? `Asset Total: ${a_tot.toFixed(1)}%. Must = 100%!` : "";
            
            // Silo A - Margin (Independent)
            ...
            const amt_A_eimi = sA * (a_eimi/100); const amt_A_cash = sA * (a_cash/100); const amt_A_xsp = sA * (a_xsp/100);
            
            document.getElementById('a_ib01_label').innerText = `IB01: ${fmtCur(amt_A_ib01)} (${a_ib01.toFixed(1)}%)`;
            document.getElementById('a_cspx_label').innerText = `CSPX: ${fmtCur(amt_A_cspx)} (${a_cspx.toFixed(1)}%)`;
            document.getElementById('a_xuse_label').innerText = `XUSE: ${fmtCur(amt_A_xuse)} (${a_xuse.toFixed(1)}%)`;
            document.getElementById('a_eimi_label').innerText = `EIMI: ${fmtCur(amt_A_eimi)} (${a_eimi.toFixed(1)}%)`;
            document.getElementById('a_cash_label').innerText = `Cash: ${fmtCur(amt_A_cash)} (${a_cash.toFixed(1)}%)`;
            document.getElementById('a_xsp_label').innerText  = `XSP Margin: ${fmtCur(amt_A_xsp)} (${a_xsp.toFixed(1)}%)`;

            // Silo B - Assets (Must = 100%)
            ...
            document.getElementById('b_warning').innerText = Math.abs(b_tot - 100) > 0.05 ? `Asset Total: ${b_tot.toFixed(1)}%. Must = 100%!` : "";
            
            const amt_B_ib01 = sB * (b_ib01/100); const amt_B_cash = sB * (b_cash/100);
            const amt_B_cfd  = sB * (b_cfd/100);  const amt_B_intl = sB * (b_intl/100);
            const amt_B_act  = amt_B_cfd + amt_B_intl; // Grouped for charts

            document.getElementById('b_ib01_label').innerText = `IB01: ${fmtCur(amt_B_ib01)} (${b_ib01.toFixed(1)}%)`;
            document.getElementById('b_cash_label').innerText = `Cash: ${fmtCur(amt_B_cash)} (${b_cash.toFixed(1)}%)`;
            document.getElementById('b_cfd_label').innerText  = `CFDs: ${fmtCur(amt_B_cfd)} (${b_cfd.toFixed(1)}%)`;
            document.getElementById('b_intl_label').innerText = `Intl Stocks: ${fmtCur(amt_B_intl)} (${b_intl.toFixed(1)}%)`;

            // Silo C - Assets (Must = 100%)
            ...
            let c_tot = c_ib01 + c_cspx + c_xuse + c_eimi + c_cash;
            document.getElementById('c_warning').innerText = Math.abs(c_tot - 100) > 0.05 ? `Asset Total: ${c_tot.toFixed(1)}%. Must = 100%!` : "";
            
            // Silo C - Margin (Independent)
            ...
            const amt_C_eimi = sC * (c_eimi/100); const amt_C_cash = sC * (c_cash/100); const amt_C_xsp  = sC * (c_xsp/100);
            
            document.getElementById('c_ib01_label').innerText = `IB01: ${fmtCur(amt_C_ib01)} (${c_ib01.toFixed(1)}%)`;
            document.getElementById('c_cspx_label').innerText = `CSPX: ${fmtCur(amt_C_cspx)} (${c_cspx.toFixed(1)}%)`;
            document.getElementById('c_xuse_label').innerText = `XUSE: ${fmtCur(amt_C_xuse)} (${c_xuse.toFixed(1)}%)`;
            document.getElementById('c_eimi_label').innerText = `EIMI: ${fmtCur(amt_C_eimi)} (${c_eimi.toFixed(1)}%)`;
            document.getElementById('c_cash_label').innerText = `Cash: ${fmtCur(amt_C_cash)} (${c_cash.toFixed(1)}%)`;
            document.getElementById('c_xsp_label').innerText  = `XSP Margin: ${fmtCur(amt_C_xsp)} (${c_xsp.toFixed(1)}%)`;

            // Silo D - Assets (Must = 100%)
            ...
            document.getElementById('d_warning').innerText = Math.abs(d_tot - 100) > 0.05 ? `Asset Total: ${d_tot.toFixed(1)}%. Must = 100%!` : "";
            ...
            document.getElementById('d_ib01_label').innerText = `IB01: ${fmtCur(amt_D_ib01)} (${d_ib01.toFixed(1)}%)`;
            document.getElementById('d_cspx_label').innerText = `CSPX: ${fmtCur(amt_D_cspx)} (${d_cspx.toFixed(1)}%)`;
            document.getElementById('d_xuse_label').innerText = `XUSE: ${fmtCur(amt_D_xuse)} (${d_xuse.toFixed(1)}%)`;
            document.getElementById('d_eimi_label').innerText = `EIMI: ${fmtCur(amt_D_eimi)} (${d_eimi.toFixed(1)}%)`;
            document.getElementById('d_cash_label').innerText = `Cash: ${fmtCur(amt_D_cash)} (${d_cash.toFixed(1)}%)`;
            ...
            // Chart 1: Bar Chart (Assets Stacked, Margin overlaid as scatter points)
            let barData =[
                ...
                { x: ['Silo A', 'Silo C'], y:[amt_A_xsp, amt_C_xsp], name: 'Options Margin Locked', type: 'scatter', mode: 'markers', 
            ...
            // Chart 2: Pie Chart (ONLY PHYSICAL ASSETS, ALWAYS EQUALS 100%)
            let pieValues =[tot_ib01, tot_cspx, tot_xuse, tot_eimi, amt_B_act, tot_cash];
            let fmtPct = (val) => ((val / totalEstate) * 100).toFixed(1) + '%';
            let pieLabels =[
                `IB01: ${fmtCur(tot_ib01)} (${fmtPct(tot_ib01)})`, 
                `CSPX: ${fmtCur(tot_cspx)} (${fmtPct(tot_cspx)})`, 
                `XUSE: ${fmtCur(tot_xuse)} (${fmtPct(tot_xuse)})`, 
                `EIMI: ${fmtCur(tot_eimi)} (${fmtPct(tot_eimi)})`, 
                `Active Swing: ${fmtCur(amt_B_act)} (${fmtPct(amt_B_act)})`, 
                `Total Cash: ${fmtCur(tot_cash)} (${fmtPct(tot_cash)})`
            ];
            
            let pieColors =[palette.ib01, palette.cspx, palette.xuse, palette.eimi, palette.cfd_intl, palette.cash];
            ...
            let pieLayout = { title: `Total Estate Physical Exposure ($${totalEstate.toLocaleString()})`, margin: {t: 50, b: 20, l: 0, r: 0}, paper_bgcolor: 'rgba(0,0,0,0)' };
...
html_content = html_content.replace("VAR_COLOR_D_IRR", get_color_class(metrics["U25218481"]["irr"]))
html_content = html_content.replace("VAR_D_IRR", metrics["U25218481"]["irr"])
html_content = html_content.replace("VAR_COLOR_D_SHARPE", get_color_class(metrics["U25218481"]["sharpe"]))
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v7 created with Math Fixes.")
