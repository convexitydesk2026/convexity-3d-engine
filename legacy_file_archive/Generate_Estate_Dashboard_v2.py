r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v2.py
Purpose: Generates a standalone, interactive HTML dashboard for the Family 
         Estate. 
         VERSION 2: Adds 'CIO Max Allocation %' and reformatted 2-line 
         definitions/comments to the Master Instrument Matrix.
Author: Chief Investment Officer AI Advisor
Date: April 2026

How to Run:
1. Ensure Python is installed on your PC.
2. Save this script as 'Generate_Estate_Dashboard_v2.py'.
3. Run it via terminal or your Python IDE:
   python Generate_Estate_Dashboard_v2.py
4. Navigate to your folder and open 'Family_Estate_Dashboard_v2.html'.
...
file_name = "Family_Estate_Dashboard_v2.html"
...
<head>
    ...
    <title>Family Estate Master Allocator v2</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
                ...
                <table class="min-w-full bg-white text-sm">
                    <thead class="bg-slate-800 text-white">
                        <tr>
                            ...
                            <th class="py-3 px-3 text-left">CIO<br>Grading</th>
    ...
    <script>
        ...
        const instruments =[
            { inst: "XSP Put Credit Spreads", type: "Index Option", risk: "Moderate", alpha: "High (VRP)", strat: "Weekly Income (45 DTE)", jur: "US (Cboe)", alloc: "20%", rec: "Splendid", class: "splendid", 
              comm: "<strong>Cash-settled S&P 500 index options used to collect market fear premium.</strong><br>1,000% safe from IRS. Capped risk. The 20% total estate limit ensures a Black Swan only causes a highly manageable ~10% total portfolio drawdown." },
            
            { inst: "IB01", type: "UCITS ETF", risk: "Risk-Free", alpha: "Zero", strat: "Long Term Holding", jur: "Ireland (LSE)", alloc: "100%", rec: "Splendid", class: "splendid", 
              comm: "<strong>Accumulating Irish-domiciled short-term US Treasury fund.</strong><br>Ultimate USD parking vault. Accumulates 4.5%+ interest tax-free. Allocation can theoretically be 100% during periods of extreme macroeconomic uncertainty." },
            
            { inst: "CSPX", type: "UCITS ETF", risk: "Moderate", alpha: "Zero (Beta 1)", strat: "Long Term DCA", jur: "Ireland (LSE)", alloc: "60%", rec: "Great", class: "great", 
              comm: "<strong>Accumulating Irish-domiciled S&P 500 equity fund.</strong><br>Shields gains from 40% Estate Tax and 30% Dividend Withholding. 60% limit ensures the estate does not become dangerously overweight in a single global economy (USA)." },
            
            { inst: "XUSE & EIMI", type: "UCITS ETF", risk: "Moderate", alpha: "Moderate", strat: "Long Term DCA", jur: "Ireland (LSE)", alloc: "40%", rec: "Great", class: "great", 
              comm: "<strong>Accumulating Irish-domiciled Developed (Ex-US) and Emerging Market funds.</strong><br>True geographic diversification. Hedges against US Dollar decline. 40% limit perfectly matches the neutral global equity market cap weighting." },
            
            { inst: "US Tech CFDs", type: "OTC Contract", risk: "Aggressive", alpha: "High", strat: "Swing Trading", jur: "UK/Offshore", alloc: "3%", rec: "Good", class: "good", 
              comm: "<strong>Over-The-Counter synthetic derivatives tracking physical US tech stocks.</strong><br>0% IRS Estate Tax risk. Subject to overnight margin fees. 3% limit quarantines the aggressive Minervini/Champion strategy safely inside Silo B." },
            
            { inst: "XSP LEAPS", type: "Index Option", risk: "High", alpha: "High", strat: "Multi-Year Swing", jur: "US (Cboe)", alloc: "10%", rec: "Good", class: "good", 
              comm: "<strong>Deep In-The-Money Call options expiring in 1 to 3 years.</strong><br>Capital efficient stock replacement safe from IRS. Suffers from 3.5% annual time/dividend drag. Limited to 10% to prevent excessive Time Decay (Theta) exposure." },
            
            { inst: "Intl Stocks (Direct)", type: "Stock", risk: "Aggressive", alpha: "High", strat: "Swing Trading", jur: "Europe/Asia", alloc: "3%", rec: "Neutral", class: "good", 
              comm: "<strong>Direct ownership of non-US physical equities.</strong><br>Safe from IRS. Wider bid/ask spreads and liquidity constraints compared to US Tech. Same 3% strict quarantine limit as CFDs to protect the core estate." },
            
            { inst: "US Corp Bonds", type: "Bond/Stock", risk: "Moderate", alpha: "Negative", strat: "Income", jur: "US / Ireland", alloc: "0%", rec: "Bad", class: "bad", 
              comm: "<strong>Debt issued by US corporations, or ETFs holding dividend-yielding equities.</strong><br>Di-worsification. Crashes during Black Swans (Credit Widening). 0% allocation because tax drag on yields is too severe for NRAs." },
            
            { inst: "Physical US Stocks", type: "Stock", risk: "Aggressive", alpha: "High", strat: "Swing / Hold", jur: "US (Nasdaq)", alloc: "0%", rec: "Avoid", class: "avoid", 
            ...
            { inst: "US Stock Options", type: "Option", risk: "Extreme", alpha: "High", strat: "Speculation", jur: "US (Cboe)", alloc: "0%", rec: "Avoid", class: "avoid", 
              comm: "<strong>Contracts granting the right to buy/sell physical US property.</strong><br>LETHAL. IRS gray area. Brokers will freeze the account for months upon death. 0% absolute prohibition." }
        ];
        ...
        function populateTable() {
            ...
            instruments.forEach(i => {
                html += `<tr class="hover:bg-gray-50 border-b border-gray-100">
                    <td class="py-4 px-3 font-semibold text-gray-900">${i.inst}</td>
                    <td class="py-4 px-3">${i.type}</td>
                    <td class="py-4 px-3">${i.risk}</td>
                    <td class="py-4 px-3">${i.alpha}</td>
                    <td class="py-4 px-3">${i.strat}</td>
                    <td class="py-4 px-3">${i.jur}</td>
                    <td class="py-4 px-3 font-bold text-center text-lg">${i.alloc}</td>
                    <td class="py-4 px-3 ${i.class} text-center rounded">${i.rec}</td>
                    <td class="py-4 px-3 text-sm text-gray-700 leading-relaxed">${i.comm}</td>
                </tr>`;
            });
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v2 created.")
