r"""
=============================================================================
Script Name: Estate_Orchestrator_v25.py
Purpose: The Zero-Touch Automation Engine for the Family Office.
         v25 UPGRADES & CURES:
         - THE HEADER RESET PROTOCOL: Solved the "Ghost Position" bug. The 
           parser now wipes the POST and CRTT arrays clean every time a new 
           date header is encountered in historical files, guaranteeing only 
           the absolute latest snapshot is saved to the local database.
         - Enforces pristine Self-Healing if a new Master Feed is detected.
         - Points to Generate_Estate_Dashboard_v68.py
=============================================================================
"""
...
# -------------------------------------------------------------------
# EXPLICIT FLEX QUERY PARSER (HEADER RESET PROTOCOL INCLUDED)
# -------------------------------------------------------------------
def parse_ibkr_flex(csv_text):
    ...
    for row in reader:
        ...
        if r_type == 'HEADER':
            ...
            if sec not in data: 
                data[sec] =[]
            elif sec in ['POST', 'CRTT']:
                # THE HEADER RESET PROTOCOL (Fixes the $80M Ghost Position Bloat)
                # If a new date header is encountered for snapshots, wipe the array.
                # This guarantees only the final day's snapshot survives into the database.
                data[sec] =[]
                
        elif r_type == 'DATA':
            if sec in headers:
                # Aggressive Scrubbing
                row_data =[c.strip() for c in row[2:]]
...
def process_ibkr_data(parsed_dfs):
    ...
    # 1. Update/Overwrite Static Snapshots (POST, CRTT)
...
def update_ibkr_api_or_fallback():
    ...
    # Intercept the Master Feed first. If it exists, nuke the corrupted DBs.
    local_csv = os.path.join(TARGET_DIR, 'Estate_Master_Feed.csv')
    if os.path.exists(local_csv):
        print("\n   [!] Found Estate_Master_Feed.csv. Wiping old databases to ensure absolute purity...")
        for db_file in glob.glob(os.path.join(TARGET_DIR, "Local_DB_*.csv")):
            try: os.remove(db_file)
            except: pass
            
        with open(local_csv, 'r', encoding='utf-8') as f: file_text = f.read()
        parsed_data = parse_ibkr_flex(file_text)
        
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        os.rename(local_csv, os.path.join(TARGET_DIR, f"Estate_Master_Feed_Processed_{timestamp}.csv"))
        print(f"   [OK] Seed file ingested, purged corruption, and renamed feed.")
    else:
    ...
    if parsed_data: 
        process_ibkr_data(parsed_data)
    else: 
        print("   [!] No new data to process. Reading existing Local Databases...")
        # Fallback if API fails and no Master Feed exists
        fallback_db = {}
        for sec in['POST', 'CRTT', 'CNAV', 'MTMP', 'TRNT', 'CTRN']:
            db_path = os.path.join(TARGET_DIR, f"Local_DB_{sec}.csv")
            fallback_db[sec] = pd.read_csv(db_path, dtype=str) if os.path.exists(db_path) else pd.DataFrame()
        execute_hierarchy_math(fallback_db)
...
if __name__ == "__main__":
    print("========================================================")
    print("     ESTATE ORCHESTRATOR v25 - INITIALIZING SYNC...      ")
    print("========================================================")
    create_backup()    
    update_ibkr_api_or_fallback()
    update_yfinance_data()
    print("\n3. Generating Dashboard HTML...")
    os.system('python Generate_Estate_Dashboard_v68.py')
    html_path = os.path.join(TARGET_DIR, "Family_Estate_Dashboard_v68.html")
