r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v26.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 26 UPGRADES: 
         - BUG FIX: Implemented "Native JS Array Bridge" (to_js_array) to completely 
           eliminate JSON Syntax Errors. SPY/QQQ cumulative PnL lines now render flawlessly.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v26.html")
...
# NATIVE JS ARRAY INJECTOR: Replaces JSON.parse to completely eliminate syntax crashes
    ...
    for v in vals:
        if pd.isna(v) or (isinstance(v, float) and (np.isinf(v) or np.isnan(v))):
            ...
            if is_date:
                ...
                clean.append(str(round(float(v), 2)))
    return "[" + ",".join(clean) + "]"
...
if os.path.exists(csv_file_path):
    try:
        ...
        # Calculate Daily PnL Histogram Data (Full 1-Year Calendar)
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        # Native Javascript Arrays Injection
...
<head>
    ...
    <title>Family Estate Master Allocator v26</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v26</p>
        </div>
...
# Native JS Replacements (Bypassing JSON.parse entirely)
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v26 created.")
