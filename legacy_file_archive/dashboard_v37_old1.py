...
if not pos_df.empty:
    ...
    for _, opt in open_opts.iterrows():
        ...
        if not open_orders_df.empty:
            has_order = not open_orders_df[(open_orders_df['account'] == acc) & (open_orders_df['symbol'] == opt_sym)].empty
            if not has_order:
                alerts.append(f"🚨 **CRITICAL (Naked Option):** {opt_sym} in {SILO_MAP.get(acc, [acc])[0]} is missing resting OCO brackets! Attach Take-Profit/Stop-Loss immediately.")
        else:
            alerts.append(f"🚨 **CRITICAL (Naked Option):** {opt_sym} in {SILO_MAP.get(acc, [acc])[0]} is missing resting OCO brackets (No resting orders detected globally)!")
...
# Original Options Warnings & Stop Loss
if not journal_raw_df.empty:
    today = datetime.date.today()
    for _, row in journal_raw_df[pd.isnull(journal_raw_df['Close Date'])].iterrows():
        try:
            ...
            if str(dte_rem) != 'Closed':
                try: S_live = float(yf.Ticker('^SPX').history(period='5d')['Close'].iloc[-1])/10.0 if 'XSP' in str(tckr).upper() else float(yf.Ticker('^NDX').history(period='5d')['Close'].iloc[-1])/100.0
                except: S_live = 550.0
                
                s_p = float(row['Short Strike'])*0.01 # Simplified placeholder for alert triggers
                curr_spread_price = s_p * 1.5 
                ...
                if curr_spread_price >= stop_loss_threshold:
                    alerts.append(f"🛑 **STOP LOSS TRIGGERED:** {tckr} ({row['Short Strike']}/{row['Long Strike']}) spread price breached 200% limit (${stop_loss_threshold:.2f}). Close immediately.")
                elif curr_spread_price >= warning_threshold:
                    alerts.append(f"⚠️ **Stop Loss Warning:** {tckr} ({row['Short Strike']}/{row['Long Strike']}) spread price is approaching the 200% limit (${stop_loss_threshold:.2f}).")
        except: pass
...
with c_gb_cash:
    ...
    st.plotly_chart(fig_gauge_cash, width="stretch", use_container_width=True)
...
with c_sa_cash:
    ...
    st.plotly_chart(fig_A_cash, width="stretch", use_container_width=True)
...
with c_sc_cash:
    ...
    st.plotly_chart(fig_C_cash, width="stretch", use_container_width=True)
...
with c_gb_marg:
    ...
    st.plotly_chart(fig_gauge_margin, width="stretch", use_container_width=True)
...
with c_sa_marg:
    ...
    st.plotly_chart(fig_A_margin, width="stretch", use_container_width=True)
...
with c_sc_marg:
    ...
    st.plotly_chart(fig_C_margin, width="stretch", use_container_width=True)
    ...
    with col_ledger:
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "Annualized ROC %": st.column_config.NumberColumn("Ann.\nROC (%)", format="%.2f%%"),
                "Days Remaining": st.column_config.Column("Days\nRemaining"),
