"""
=============================================================================
Script Name: dashboard_v17.py
Purpose: The Streamlit Frontend (Module 17 - 4K UI Optims & Layout Overhaul).
         - Panel 8: 50/25/25 split. Restored Greeks Table & CIO guide.
         - Panel 7: MC Drawdown box moved to side column. Terminal labels appended.
         - Panel 6: Gauges arranged horizontally (Widths: 4/2/2/4/2/2/3).
         - Panel 2 & 1: Font sizes doubled, text colors adjusted to black.
         - Header: Dynamic script discovery added.
=============================================================================
"""
...
def get_active_scripts():
    ...
    return " • ".join(scripts) if scripts else "dashboard_v17.py"
...
for idx, acc in enumerate(SILO_MAP.keys()):
    ...
    with cols[idx]:
        ...
        if not silo_dfs[acc].empty:
            ...
            # Doubled height from 150 to 300
            fig_mini.update_layout(height=300, margin=dict(l=0, r=0, t=0, b=0), plot_bgcolor=color, paper_bgcolor='rgba(0,0,0,0)',
...
if not pos_df.empty:
    ...
    with col_bar:
        ...
        for i, total in enumerate(silo_totals):
            ...
            fig_bar.add_annotation(x=silo_names[i], y=total, text=f"<b>${total/1000:.0f}k</b><br>({pct_total:.1f}%)", showarrow=False, yanchor='bottom', yshift=5, font=dict(size=16))
        ...
        fig_bar.update_layout(
            ...
            legend_title_text="<b>ASSET CLASS</b>", font=dict(size=16) # Increased global font size for chart
        )
    ...
    with col_pie:
        ...
        fig_pie.update_traces(textfont_size=16) # Increased font size
        fig_pie.update_layout(title="Gross Asset Allocation", margin=dict(l=20, r=20, t=40, b=20), legend_title_text="<b>ASSET CLASS</b>", font=dict(size=16))
...
for idx, acc in enumerate(SILO_MAP.keys()):
    ...
    with comp_cols[idx]:
        ...
        st.markdown(f"<div style='font-size: 11px; color: #000000; padding: 10px; border-top: 1px solid #e5e7eb; margin-top: 10px;'>{strats[idx]}</div>", unsafe_allow_html=True)
...
if not attr_df.empty:
        ...
        if not line_df.empty:
            ...
            for col, name, color in zip(['a1_yield', 'a2_beta', 'a3_vrp', 'a4_alpha', 'a5_fees'],['Yield', 'Beta', 'VRP', 'Alpha', 'Fees'], bar_colors):
                ...
                fig_attr_line.add_annotation(
                    x=last_dt, y=val, text=f"${val:,.0f}",
                    showarrow=False, xanchor='left', bgcolor=color,
                    font=dict(color='white', size=11), borderpad=3
                )
                
        fig_attr_line.update_layout(
            title="Cumulative Trajectory", plot_bgcolor='rgba(0,0,0,0)', 
    ...
    <div style="background-color: #f9fafb; padding: 10px; border-radius: 8px; border: 1px solid #e5e7eb; font-size: 11px; color: #4b5563;">
        ...
        <span style="color: #991b1b; font-weight: bold;">Commissions & Fees (a5):</span> Explicit friction pulled directly from IBKR Ledger.
    </div>
    """, unsafe_allow_html=True)
...
# Re-engineered fraction columns:[4, 2, 2, 4, 2, 2, 3] (Total 19 fractions)
...
with c_gb_cash:
    fig_gauge_cash = go.Figure(go.Indicator(
        ...
        gauge = {
            'axis': {'range':[0, 100], 'tickwidth': 1}, 'bar': {'color': "blue"},
    ...
    if pct_cash >= 40: st.success(f"**✓ SAFE:** Optimal.")
    else: st.error(f"**⚠️ WARNING:** Buffer breached 40%.")
...
with c_sa_cash:
    ...
    fig_A_cash = go.Figure(go.Indicator(
        mode="gauge+number", value=pct_cash_A, number={'suffix': "%", 'font': {'size': 20}},
...
with c_sc_cash:
    ...
    fig_C_cash = go.Figure(go.Indicator(
        mode="gauge+number", value=pct_cash_C, number={'suffix': "%", 'font': {'size': 20}},
...
with c_gb_marg:
    ...
    fig_gauge_margin = go.Figure(go.Indicator(
        ...
        gauge = {
            'axis': {'range':[0, 100], 'tickwidth': 1}, 'bar': {'color': "purple"},
            'steps':[{'range':[0, 25], 'color': "rgba(34, 197, 94, 0.3)"}, {'range':[25, 100], 'color': "rgba(239, 68, 68, 0.3)"}],
    ...
    if pct_margin <= 25: st.success(f"**✓ OPTIMAL:** Capacity healthy.")
    else: st.error(f"**⚠️ CAP BREACHED:** Close spreads.")
...
with c_sa_marg:
    ...
    fig_A_margin = go.Figure(go.Indicator(
        mode="gauge+number", value=pct_margin_A, number={'suffix': "%", 'font': {'size': 20}},
        title={'text': "Silo A Margin", 'font': {'size': 12}},
        gauge={'axis': {'range':[0, 100], 'tickwidth': 1}, 'bar': {'color': "purple"},
               'steps':[{'range': [0, 25], 'color': "rgba(34, 197, 94, 0.3)"}, {'range':[25, 100], 'color': "rgba(239, 68, 68, 0.3)"}],
...
with c_sc_marg:
    ...
    fig_C_margin = go.Figure(go.Indicator(
        mode="gauge+number", value=pct_margin_C, number={'suffix': "%", 'font': {'size': 20}},
...
with c_txt:
    ...
    if regime_str == 'Green':
        ...
        st.write("<span style='font-size: 12px;'>**Silos A & C (Options):** Clear to sell new XSP/XND Put Spreads up to 25% margin cap.</span>", unsafe_allow_html=True)
        ...
        st.write("<span style='font-size: 12px;'>**Silos A & C (Options):** Throttle new Put Spreads to strict 45 DTE. Require higher premium floor.</span>", unsafe_allow_html=True)
    else:
        ...
        st.write("<span style='font-size: 12px;'>**Silos A & C (Options):** If VIX spikes violently, buy 90-DTE SPX Puts (Taleb Tail-Hedge) to protect physical assets.</span>", unsafe_allow_html=True)
...
if sim_length > 0:
    ...
    with col_mc_chart:
        ...
        for i in range(200):
            r_c, g_c, b_c = random.randint(50, 200), random.randint(50, 200), random.randint(50, 200)
            mc_fig.add_trace(go.Scatter(y=cum_sim[i], mode='lines', line=dict(color=f'rgba({r_c}, {g_c}, {b_c}, 0.25)', width=1), showlegend=False, hoverinfo='skip'))
        
        # Explicit traces for Best, Worst, Avg, and Original to allow annotation
        mc_fig.add_trace(go.Scatter(y=cum_sim[best_idx], name='Best Case', mode='lines', line=dict(color='rgba(34, 197, 94, 0.8)', width=3)))
        mc_fig.add_trace(go.Scatter(y=cum_sim[worst_idx], name='Worst Case', mode='lines', line=dict(color='rgba(239, 68, 68, 0.8)', width=3)))
        ...
        last_x = sim_length
        mc_fig.add_annotation(x=last_x, y=cum_sim[best_idx][-1], text=f"Best: ${cum_sim[best_idx][-1]:,.0f}", showarrow=False, xanchor='left', bgcolor='rgba(34, 197, 94, 0.8)', font=dict(color='white', size=11))
        mc_fig.add_annotation(x=last_x, y=cum_sim[worst_idx][-1], text=f"Worst: ${cum_sim[worst_idx][-1]:,.0f}", showarrow=False, xanchor='left', bgcolor='rgba(239, 68, 68, 0.8)', font=dict(color='white', size=11))
        ...
        mc_fig.update_layout(
            height=800,
            margin=dict(l=20, r=80, t=30, b=20), plot_bgcolor='rgba(0,0,0,0)', xaxis_title='Trading Days Forward',
    ...
    with col_mc_leg:
        ...
        st.markdown(f"""
        <div style="background-color: rgba(255, 255, 255, 0.9); padding: 15px; border: 1px solid black; border-radius: 5px; font-size: 13px; color: black; margin-top: 50px;">
            {stats_text}
        </div>
        """, unsafe_allow_html=True)
...
if not journal_raw_df.empty:
    ...
    if abs(drift_delta) > 0.01:
        st.error(f"⚠️ **LEDGER DRIFT DETECTED:** Your live TWS margin footprint is **${opt_margin_total:,.0f}**, but your Open Journal positions total **${opt_margin_journal:,.0f}**. You have a **${-drift_delta:,.0f}** discrepancy.")
...
if not journal_raw_df.empty:
    # OPTION 2 Layout: 50% Ledger | 25% 2D Theta | 25% 3D Surface
    ...
    with col_ledger:
        ...
        edited_df = st.data_editor(
            styled_journal, width='stretch', num_rows="dynamic", disabled=disabled_cols, height=750, key="journal_editor",
            column_config={
                "👁️ View 3D": st.column_config.CheckboxColumn("View\n3D", help="Check to visualize topography"),
                ...
                "Total Net Credit (USD)": st.column_config.NumberColumn("Total Net\nCredit\n($)", format="$%.2f"),
                "Target 50% Exit Price (USD)": st.column_config.NumberColumn("Target\n50% Exit\n($)", format="$%.2f"),
                "Closing Price (USD)": st.column_config.NumberColumn("Closing\nPrice\n($)", format="$%.2f"),
                "Total P&L (USD)": st.column_config.NumberColumn("Total\nP&L\n($)", format="$%.2f"),
                "Return on Capital (ROC) %": st.column_config.NumberColumn("ROC\n(%)", format="%.2f%%"),
                "Annualized ROC %": st.column_config.NumberColumn("Annual\nROC\n(%)", format="%.2f%%"),
                "Open Date": st.column_config.DateColumn(), "Close Date": st.column_config.DateColumn()
            }
        )
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            with st.spinner(f"Fetching Live Data for {tckr}..."): S_live, iv_live = fetch_live_data(tckr)
            ...
            with col_3d:
                ...
                for d in y_3d:
                    ...
                    z_row =[]
                    for p in x_vals:
                        if T_3d <= 0.0001: z_row.append((prem - (max(K_s - p, 0) - max(K_l - p, 0))) * qty * 100)
                        else:
                ...
                fig_3d = go.Figure(data=[go.Surface(z=z_3d, x=x_vals, y=y_3d, colorscale=[[0, '#fef2f2'],[0.2, '#fca5a5'], [0.5, 'white'],[0.8, '#86efac'],[1, '#f0fdf4']])])
                fig_3d.add_trace(go.Scatter3d(x=[S_live], y=[curr_dte], z=[unrealized_pnl], mode='markers', name='Current', marker=dict(color='black', size=4)))
                fig_3d.update_layout(title="3D Topography (Time vs Price)", margin=dict(l=0, r=0, b=0, t=40), height=645, scene=dict(xaxis_title='Price', yaxis_title='DTE', zaxis_title='P&L ($)', yaxis=dict(autorange='reversed')))
