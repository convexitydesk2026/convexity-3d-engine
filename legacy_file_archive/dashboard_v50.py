...
3. Launch the UI by running exactly:
   streamlit run dashboard_v50.py

CHANGELOG (v50):
         - FIXED: 3D Topography breakeven plane intersection line thickness.
         - FIXED: Silo container heights and strictly aligned Footer mandates.
         - FIXED: PnL Attribution label color contrast.
         - FIXED: Replaced legacy UCITS with Synthetic Beta in Deployment Matrix.
         - ADDED: Beta-Weighted Delta Portfolio Risk Metric (Enhancement 1).
         - ADDED: Tail Hedge Catastrophe Coverage Ratio (Enhancement 2).
         - ADDED: VRP Capital Velocity Scatter Plot (Enhancement 3).
=============================================================================
"""
...
def get_exact_opt_margin(df_in):
    # v50: Exclude both Tail Hedges and Synthetic Beta (Long Calls) from margin liability math
...
def load_champion_history():
    ...
    conn.commit()
    # v50: Fetching 'rowid' as primary key to allow precise editing later in the ledger
...
# v50: Synthetic Beta Rolling Radar
if not pos_df.empty:
...
if not journal_raw_df.empty:
    ...
    if not active_vrp.empty:
        ...
        fig_scatter.add_trace(go.Scatter(
            ...
            marker=dict(
                size=active_vrp['Collateral Locked (USD)'] / active_vrp['Collateral Locked (USD)'].max() * 40 + 10,
