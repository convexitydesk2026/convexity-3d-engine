...
3. Launch the UI by running exactly:
   streamlit run dashboard_v63.py

CHANGELOG (v63):
         - FEATURE: Added a CSV download button to the Options Performance Ledger to preserve closed trade history.
         - PATCHED: Data-entry crash. Safeguarded Catastrophe Multiplier and Yahoo fetcher against NaN values during live Streamlit editing.
=============================================================================
"""
...
def fetch_live_data(ticker_symbol):
    # v63: Safeguard against empty cells during live data-entry
...
def get_exact_opt_margin(df_in):
    ...
    try:
        ...
        # v63 FIX: Added base_tckr to the groupby!
                ...
                try:
                    ...
                    if len(parts) >= 4:
                        ...
                        if right == 'P' and dte > 60:
                            # v63: Broad Market Whitelist prevents Decapitation Bug for Sector Shorts (like SMH)
...
# v63 FIX: Added Physical US Stocks & International Stocks to Core Assets pool
...
if not pos_df.empty:
    ...
    if not open_opts.empty:
        ...
        for base_tckr, group in open_opts.groupby('base_tckr'):
            ...
            # 3. Tail Hedges
                ...
                strike = longs['strike'].iloc[0]
                matrix_rows += f"<tr><td><b>{base_tckr} Puts (Tail Hedge)</b><br><span style='font-size:11px; color:#15803d; background-color:#dcfce7; padding:2px 4px; border-radius:3px;'><b>TAX: EXCELLENT.</b> Section 1256 Cash-Settled. IRS Safe.</span></td><td style='color:#166534; font-size: 13px;'>{exp_str} {strike} Put<br><br><b>SAFEGUARD.</b> Deep Out-of-the-Money insurance policies silently resting on the ledger.</td><td style='font-size: 13px;'><b>Black Swan Insurance (Grinding Bear).</b><br>• <i>Pro:</i> Mathematically guarantees survival during multi-month systemic meltdowns.<br>• <i>Con:</i> 100% loss of capital expected. Acts as a constant Theta drag on the portfolio.</td><td style='font-size: 13px;'><b>Entry:</b> Financed strictly by 10% of VRP winnings. 120-150 DTE. Delta < -0.05.<br><br><b>Exit:</b> Monetize dynamically during deep market panic to buy physical assets at the bottom.</td><td style='font-size: 13px;'><b>Nominal.</b> Expect this to bleed to $0. Do not track for daily PnL. Let it ride to catch unexpected crashes.</td></tr>"
            ...
            # 5. Cash-Secured Puts (e.g., BE)
            elif not shorts.empty and base_tckr not in ['XSP', 'SPX', 'XND', 'NDX', 'QQQ', 'SMH']:
                strike = shorts['strike'].iloc[0]
                matrix_rows += f"<tr><td><b>{base_tckr} Cash-Secured Puts</b><br><span style='font-size:11px; color:#991b1b; background-color:#fee2e2; padding:2px 4px; border-radius:3px;'><b>TAX: LETHAL TRAP.</b> If assigned, you hold physical US stock. 40% Estate Tax triggers instantly.</span></td><td style='color:#166534; font-size: 13px;'>{exp_str} {strike} Put<br><br><b>WAITING TO BUY.</b> {base_tckr} is at ${spot:.2f}. You are acting as the casino, getting paid while waiting for the stock to drop to your strike.</td><td style='font-size: 13px;'><b>Institutional Discounting.</b><br>• <i>Pro:</i> Generates massive cash yield while waiting to acquire fundamentally sound assets.<br>• <i>Con:</i> Locks up heavy notional margin. Absolute disaster for Non-US persons if assigned.</td><td style='font-size: 13px;'><b>Entry:</b> High IV Rank on targeted infrastructure/AI physical assets.<br><br><b>Exit:</b> 50% Take-Profit to free up margin, or take physical assignment.</td><td style='font-size: 13px;'><b>Nominal.</b> Keep collecting the premium. Beware of assignment risk as expiration approaches.</td></tr>"

    # Add the 3 Future Consideration Instruments
    matrix_rows += f"<tr style='background-color: #f8fafc;'><td><span style='color:#64748b'><b>/MES Put Spreads</b><br><i>(Future Consideration)</i></span><br><span style='font-size:11px; color:#15803d; background-color:#dcfce7; padding:2px 4px; border-radius:3px;'><b>TAX: EXCELLENT.</b> Futures Options. Section 1256.</span></td><td style='color:#64748b; font-size: 13px;'><i>PENDING CAPABILITY</i><br><br>Requires mastery of futures data feeds and SPAN margin accounting.</td><td style='color:#64748b; font-size: 13px;'><b>Hyper-Capital Efficiency (Income).</b><br>• <i>Pro:</i> SPAN margin requires 50% less collateral than XSP. Doubles your ROC instantly.<br>• <i>Con:</i> High leverage requires flawless notional risk monitoring.</td><td style='color:#64748b; font-size: 13px;'><b>Entry:</b> Exact same quantitative rules as XSP (45 DTE, VIX > 15).<br><br><b>Exit:</b> 50% Take-Profit.</td><td style='color:#64748b; font-size: 13px;'><b>Pending Review.</b> Excellent upgrade candidate once the current VRP engine is perfected.</td></tr>"
    matrix_rows += f"<tr style='background-color: #f8fafc;'><td><span style='color:#64748b'><b>VIX Call Options</b><br><i>(Future Consideration)</i></span><br><span style='font-size:11px; color:#15803d; background-color:#dcfce7; padding:2px 4px; border-radius:3px;'><b>TAX: EXCELLENT.</b> Section 1256. Cash-Settled.</span></td><td style='color:#64748b; font-size: 13px;'><i>PENDING CAPABILITY</i><br><br>Requires patching dashboard to map VIX calls as 'Tail Hedges'.</td><td style='color:#64748b; font-size: 13px;'><b>Pure Vega Insurance (Flash Crash).</b><br>• <i>Pro:</i> Explodes violently on systemic fear. Solves the XSP 'path-dependency' flaw.<br>• <i>Con:</i> Vulnerable to contango pricing and rapid volatility crush.</td><td style='color:#64748b; font-size: 13px;'><b>Entry:</b> Complacent, low-VIX markets. 60-90 DTE.<br><br><b>Exit:</b> Sell immediately into the initial volatility spike. Do not hold.</td><td style='color:#64748b; font-size: 13px;'><b>Pending Review.</b> We will deploy $400 of the backlog here once the dashboard is patched.</td></tr>"
    matrix_rows += f"<tr style='background-color: #f8fafc;'><td><span style='color:#64748b'><b>TLT Bond Options</b><br><i>(Future Consideration)</i></span><br><span style='font-size:11px; color:#991b1b; background-color:#fee2e2; padding:2px 4px; border-radius:3px;'><b>TAX: WARNING.</b> Equity Options. Assignment triggers tax trap.</span></td><td style='color:#64748b; font-size: 13px;'><i>PENDING CAPABILITY</i><br><br>Requires macro mandate expansion to include interest rate derivatives.</td><td style='color:#64748b; font-size: 13px;'><b>Macro Diversification.</b><br>• <i>Pro:</i> Allows the Estate to hedge its massive cash position against central bank interest rate shocks.<br>• <i>Con:</i> Highly sensitive to unpredictable FED policy shifts.</td><td style='color:#64748b; font-size: 13px;'><b>Entry:</b> Executed at extremes of the interest rate cycle.<br><br><b>Exit:</b> Standard 50% Take-Profit.</td><td style='color:#64748b; font-size: 13px;'><b>Pending Review.</b> Consider adding to offset equity-heavy Barbell risk in 2027.</td></tr>"
...
if not journal_raw_df.empty:
    ...
    if not active_vrp.empty:
        ...
        # v63: Dynamically map LIVE TWS PnL to calculate Live Annualized ROC
        live_roc_list = []
        live_ann_roc_list = []
        ...
        for idx, r in active_vrp.iterrows():
            try:
                ...
                if acc and not pos_df.empty:
                    mask = (pos_df['account'] == acc) & pos_df['symbol'].str.contains(tckr)
                    k_s = r.get('Short Strike')
                    k_l = r.get('Long Strike')
                    
                    if pd.notna(k_s) and k_s > 0:
                        mask_s = mask & (pos_df['symbol'].str.contains(f"_{float(k_s)}_") | pos_df['symbol'].str.contains(f"_{int(k_s)}_"))
                        mask_l = mask & (pos_df['symbol'].str.contains(f"_{float(k_l)}_") | pos_df['symbol'].str.contains(f"_{int(k_l)}_"))
                        live_pnl = pos_df[mask_s | mask_l]['unrealized_pnl'].sum()
                    else:
                        mask_l = mask & (pos_df['symbol'].str.contains(f"_{float(k_l)}_") | pos_df['symbol'].str.contains(f"_{int(k_l)}_"))
                        live_pnl = pos_df[mask_l]['unrealized_pnl'].sum()
        ...
        with tab1:
            ...
            if not vrp_df.empty:
                ...
                fig1 = go.Figure(go.Scatter(
                    ...
                    marker=dict(size=sizes, color=vrp_df['Live ROC %'], colorscale='RdYlGn', showscale=True, line=dict(width=1, color='black')),
                    hovertemplate="<b>%{text}</b><br>Days Rem: %{x}<br>Live Ann ROC: %{y:.1f}%<extra></extra>"
                ))
            ...
            else: st.info("No active VRP income trades.")

        with tab2:
            csp_df = active_vrp[active_vrp['Radar Class'] == 'CSP'].copy()
            if not csp_df.empty:
                dist_list, yield_list = [], []
                for _, r in csp_df.iterrows():
                    try:
                        S, _ = fetch_live_data(r['Ticker'])
                        K = float(r['Short Strike'])
                        prem = float(r['Premium Collected (USD)'])
                        dist_list.append(((S - K) / S) * 100)
                        yield_list.append((prem / K) * 100)
                    except:
                        dist_list.append(0)
                        yield_list.append(0)
                ...
                fig2 = go.Figure(go.Scatter(
                    ...
                    text=csp_df['Ticker'] + ' $' + csp_df['Short Strike'].astype(str), textposition="top center",
                    marker=dict(size=25, color=csp_df['Distance %'], colorscale='Blues', line=dict(width=1, color='black')),
                    hovertemplate="<b>%{text}</b><br>Distance to Assign: %{x:.1f}%<br>Yield: %{y:.1f}%<extra></extra>"
                ))
                fig2.add_vline(x=0, line_dash="dash", line_color="red", annotation_text="Assignment Zone")
                fig2.update_layout(title="Assignment Discount Radar", xaxis_title="Distance to Strike (%)", yaxis_title="Premium Yield (%)", xaxis=dict(autorange="reversed"), height=350, margin=dict(l=0, r=0, t=30, b=0))
                st.plotly_chart(fig2, width="stretch")
            else: st.info("No active Cash-Secured Puts.")

        with tab3:
            sb_df = active_vrp[active_vrp['Radar Class'] == 'Synthetic Beta'].copy()
            if not sb_df.empty:
                fig3 = go.Figure()
                fig3.add_vrect(x0=0, x1=45, fillcolor="red", opacity=0.1, layer="below", annotation_text="Ejection Zone", annotation_position="top left")
                fig3.add_trace(go.Scatter(
                    x=sb_df['Days Remaining'], y=[1]*len(sb_df), mode='markers+text',
                    text=sb_df['Ticker'] + ' ' + sb_df['Long Strike'].astype(str), textposition="top center",
                    marker=dict(size=30, color='#3b82f6', symbol='diamond', line=dict(width=2, color='black')),
                    hovertemplate="<b>%{text}</b><br>Days Rem: %{x}<extra></extra>"
                ))
                fig3.update_layout(title="Synthetic Beta Rolling Conveyor", xaxis_title="Days Remaining (DTE) →", yaxis=dict(visible=False), xaxis=dict(range=[180, 0]), height=250, margin=dict(l=0, r=0, t=30, b=0))
                st.plotly_chart(fig3, width="stretch")
            else: st.info("No active Synthetic Beta trades.")
        ...
        with tab4:
            ...
            if not cat_df.empty:
                ...
                for _, r in cat_df.iterrows():
                    # v63: Safe extraction preventing NaN crashes during live UI editing
                ...
                fig4 = go.Figure()
                fig4.add_trace(go.Bar(name='Sunk Cost', x=names, y=costs, marker_color='#dc2626'))
                fig4.add_trace(go.Bar(name='Est. Payout (-30% Crash)', x=names, y=payouts, marker_color='#16a34a'))
                fig4.update_layout(title="Catastrophe Multiplier (-30% Shock)", barmode='relative', height=350, margin=dict(l=0, r=0, t=30, b=0))
                st.plotly_chart(fig4, width="stretch")
            else: st.info("No active Catastrophe Hedges.")
...
if not journal_raw_df.empty:
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            # v63: 3D Engine Native Debit Spread & Call Detection
