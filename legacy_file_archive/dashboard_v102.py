...
streamlit run dashboard_v102.py
CHANGELOG (v102):
...
- STAGE 8 (v100): Integrated standalone Market Flow Engine (market_flow_report.html) as a collapsible hybrid component.
=============================================================================
"""
                ...
                if calc_mode == "Scale-In (Pyramid)" and is_valid_pyramid:
                    if calc_dir == "Long" and calc_entry <= existing_avg_local:
                        ...
                        if available_risk_usd <= 0:
                            ...
                            shares = int(available_risk_usd // new_risk_per_share_usd)
                            if shares > 0:
                                capital_req_usd = shares * entry_usd
                                ...
                                <div style="background-color: #f8fafc; padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1; margin-top: 10px;">
                                    <div style="font-size: 11px; color: #64748b; font-weight: bold; text-transform: uppercase;">Max New Shares Authorized</div>
                                    <div style="font-size: 22px; font-weight: 900; color: #1d4ed8;">{shares:,}</div>
                    ...
                    spread_usd = abs(entry_usd - stop_usd)
                    shares = int(risk_budget_usd // spread_usd)
                    capital_req_usd = shares * entry_usd
                    cash_remaining = calc_uninvested_cash - capital_req_usd
                    
                    st.markdown(f"""
                    <div style="background-color: #f8fafc; padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1; margin-top: 10px;">
                        <div style="font-size: 11px; color: #64748b; font-weight: bold; text-transform: uppercase;">Max Authorized Shares</div>
                        <div style="font-size: 22px; font-weight: 900; color: #1d4ed8;">{shares:,}</div>
                        <hr style="margin: 8px 0;">
                        <div style="font-size: 12px; color: #475569;">Risk per Share (USD): <b>${spread_usd:,.2f}</b></div>
