"""
=============================================================================
Script Name: dashboard_v9.py
Purpose: The Streamlit Frontend (Module 9 - The Masterpiece).
         - Integrated "Sync Live" button in sidebar.
         - Adds Risk Alerts, Matrix, and Gauge Chart Capacity Trackers.
         - Extensive aesthetic overrides (Line widths, Annotations, Legends).
=============================================================================
"""
...
for idx, acc in enumerate(SILO_MAP.keys()):
    ...
    with cols[idx]:
        ...
        if not silo_dfs[acc].empty:
            ...
            fig_mini = go.Figure()
            # Doubled thickness for mini charts
            ...
            fig_mini.update_layout(height=150, margin=dict(l=0, r=0, t=0, b=0), plot_bgcolor=color, paper_bgcolor='rgba(0,0,0,0)',
                                   yaxis=dict(zeroline=True, zerolinecolor='rgba(0,0,0,0.5)', zerolinewidth=2))
...
if not pos_df.empty:
    ...
    pie_df['market_value'] = pie_df['market_value'].abs() # Margins are absolute
    ...
    with col_bar:
        ...
        for asset in bar_df.columns:
            # Match the value in pie_df to get the rich label
        ...
        # Add Margin Lock Diamond Marker!
        opt_margin_A = pos_df[(pos_df['account'] == 'U23144948') & (pos_df['sec_type'] == 'OPT')]['position'].abs().sum() * 2500
        opt_margin_C = pos_df[(pos_df['account'] == 'U23154199') & (pos_df['sec_type'] == 'OPT')]['position'].abs().sum() * 2500
        
        # Determine actual label for Margin Lock to display in Legend
...
# EXACT REGIME SQUARES WITH DYNAMIC TOR TEXT
...
fig_pnl.add_trace(go.Scatter(
    ...
    hoverinfo='text', hovertext=[f"Regime: {r} | TOR: {t}" for r, t in zip(chart_df['regime'], chart_df['tor'])],
    name='Market Regime', showlegend=False, yaxis='y3'
))

# Box Annotations at the end of the lines!
...
if not attr_df.empty:
    ...
    # Notice the darker green for VRP: '#166534'
    ...
    with col_bar:
        ...
        # Doubled zeroline thickness
        fig_attr_bar.update_layout(title="Absolute PnL by Strategy", plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=20, r=20, t=40, b=20), yaxis=dict(zeroline=True, zerolinecolor='black', zerolinewidth=4, gridcolor='LightGray'))
        st.plotly_chart(fig_attr_bar, width='stretch')
        
    with col_line:
        fig_attr_line = go.Figure()
        # Doubled line thicknesses
        ...
        fig_attr_line.update_layout(title="Cumulative Trajectory", plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=20, r=20, t=40, b=20), yaxis=dict(zeroline=True, zerolinecolor='black', zerolinewidth=4, gridcolor='LightGray'), legend=dict(orientation="h", yanchor="bottom", y=-0.2, xanchor="center", x=0.5))
        st.plotly_chart(fig_attr_line, width='stretch')
        
    with col_vel:
        st.markdown("##### Options Engine Velocity")
        st.caption("(Silos A & C)")
        st.metric("Win Rate", "100.0%", help="Measures % of perfectly closed option spreads in Silos A/C.")
        st.metric("Avg Days in Trade", "6.2 d", help="Average calendar days capital is locked in margin.")
...
# Calculate Current Global Allocations
alloc_map = {}
for i, r in pos_df.iterrows():
    ...
    if ac == 'Crypto': alloc_map['BTC/ETH ETPs'] = alloc_map.get('BTC/ETH ETPs', 0) + r['market_value']
        ...
        if r['currency'] != 'USD': alloc_map['International Stocks'] = alloc_map.get('International Stocks', 0) + r['market_value']
        else: alloc_map['US Tech CFDs'] = alloc_map.get('US Tech CFDs', 0) + r['market_value']
...
# Matrix Checksums
gross_phys = df_matrix['Current Global Alloc. %'].sum()
opt_liab = (pos_df[pos_df['sec_type'] == 'OPT']['market_value'].sum() / global_metrics['nav']) * 100 if global_metrics['nav'] > 0 else 0
...
with c2:
    opt_margin_total = pos_df[pos_df['sec_type'] == 'OPT']['position'].abs().sum() * 2500
...
if sim_length > 0:
    ...
    # 200 MULTICOLOR SPAGHETTI LINES (Rendered FIRST so they sit in the background)
    for i in range(200):
    ...
    # AVERAGE PATH (Rendered SECOND so it sits above spaghetti but below Original)
    mc_fig.add_trace(go.Scatter(y=mc_avg_path, name='Avg Path (Stable)', mode='lines', line=dict(color='blue', width=4)))
    
    # MASSIVE ORIGINAL HISTORY LINE (Rendered LAST so it dominates the absolute forefront)
