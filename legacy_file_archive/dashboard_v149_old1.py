...
def load_benchmarks(start_date, end_date):
    ...
    # New RSP SMAs for the Macro Regime Filter
    ...
    def get_regime_and_tor(row):
        vix = row['^VIX']
        rsp = row['RSP']
        rsp_50 = row['rsp_sma_50']
        rsp_200 = row['rsp_sma_200']
        
        if pd.isna(rsp_50) or pd.isna(rsp_200): 
            return 'Yellow', 2
            
        # DOWNTREND (Red): RSP breaks and closes below the 200-day SMA.
        if rsp < rsp_200:
            return 'Red', 0
            
        # UPTREND (Green): RSP is trending above the 50-day SMA and 200-day SMA.
        elif rsp > rsp_50 and rsp > rsp_200:
            # VIX throttles the Total Open Risk (TOR) allowance within the Green regime
            return 'Green', 5 if vix < 15 else (4 if vix <= 20 else 3)
            
        # RANGEBOUND (Yellow): RSP is whipsawing below 50 SMA but remains above 200 SMA.
        else: 
            return 'Yellow', 2 if vix < 20 else 1
...
with calc_placeholder.container():
    ...
    # Only count losses that occurred on or after the v7.0 inception date
    ...
    is_locked = recent_losses >= 3 or live_risk_budget <= 0
    
    budget_color = "#16a34a" if live_risk_budget > 0 else "#dc2626"
    
    # Fixed Python 3.12+ SyntaxWarning by double-escaping the dollar signs
    st.markdown(f"""
    ...
    with st.expander("Position Sizing Engine", expanded=not is_locked):
        ...
        c1, c2 = st.columns(2)
        calc_risk_pct = c1.number_input("Risk %", min_value=0.01, max_value=5.0, value=0.10, step=0.05, format="%.2f", disabled=is_locked)
