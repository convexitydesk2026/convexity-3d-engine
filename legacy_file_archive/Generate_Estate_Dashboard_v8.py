r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v8.py
Purpose: Generates the Interactive Family Estate Dashboard.
         VERSION 8: 
         - Clarifies Market Value (Net Liquidity) inputs for Silo Balances.
         - Explicitly renames Margin to "Overlay" to prevent accounting confusion.
         - Updates tooltips to reinforce the "Thin Air" derivative logic.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v8.html")
...
<head>
    ...
    <title>Family Estate Master Allocator v8</title>
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[1800px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v8</p>
        </div>
        ...
        <div class="mb-8 bg-gray-50 p-4 rounded-xl border shadow-sm">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div><label class="block text-sm font-bold text-gray-700">Silo A ($) - Vault</label><input type="number" id="siloA" value="599511" class="mt-1 w-full rounded-md p-2 border" oninput="updateUI()"></div>
        ...
        <div class="mb-8 bg-white p-4 rounded-xl border shadow-sm">
            ...
            <p class="text-sm text-gray-500 mb-4 border-b pb-2">Physical assets must total 100%. Margin/Options are phantom overlays locking your physical collateral.</p>
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-blue-50 p-3 rounded-lg border border-blue-200 shadow-inner">
                    ...
                    <p class="text-xs font-bold text-gray-500 mb-1 uppercase tracking-wide">Physical Assets (Must = 100%)</p>
                    ...
                    <label class="text-xs font-semibold text-green-700" id="a_xsp_label">Options Margin: </label><input type="range" id="a_xsp" min="0" max="100" step="0.1" value="20.8" class="w-full mb-1" oninput="updateUI()">
                </div>
                ...
                <div class="bg-green-50 p-3 rounded-lg border border-green-200 shadow-inner">
                    ...
                    <label class="text-xs font-semibold text-green-700" id="c_xsp_label">Options Margin: </label><input type="range" id="c_xsp" min="0" max="100" step="0.1" value="24.2" class="w-full mb-1" oninput="updateUI()">
                </div>

                <!-- Silo D -->
                <div class="bg-yellow-50 p-3 rounded-lg border border-yellow-300 shadow-inner">
                    <h4 class="font-bold text-yellow-900 mb-2 text-sm border-b border-yellow-300 pb-1">Silo D</h4>
                    <p class="text-xs font-bold text-gray-500 mb-1 uppercase tracking-wide">Physical Assets (Must = 100%)</p>
    ...
    <script>
        ...
        const instruments =[
            ...
            { inst: "Intl Stocks (Direct)", type: "Stock", risk: "Aggressive", alpha: "High", sharpe: "Negative", strat: "Swing Trading", jur: "Europe/Asia", min: "0%", max: "3%", rec: "Neutral", class: "good", comm: "<strong>Direct ownership of non-US physical equities.</strong><br>Safe from IRS. Wider bid/ask spreads and liquidity constraints compared to US Tech." },
        ...
        function updateUI() {
            ...
            let a_xsp  = parseFloat(document.getElementById('a_xsp').value);  let a_cash = parseFloat(document.getElementById('a_cash').value);
            let a_tot = a_ib01 + a_cspx + a_xuse + a_eimi + a_cash;
            document.getElementById('a_warning').innerText = Math.abs(a_tot - 100) > 0.1 ? `Asset Total: ${a_tot.toFixed(1)}%. Must = 100%!` : "";
            
            const amt_A_ib01 = sA * (a_ib01/100); const amt_A_cspx = sA * (a_cspx/100); const amt_A_xuse = sA * (a_xuse/100); 
            const amt_A_eimi = sA * (a_eimi/100); const amt_A_xsp  = sA * (a_xsp/100);  const amt_A_cash = sA * (a_cash/100);
            ...
            document.getElementById('a_xsp_label').innerText  = `Options Margin (Overlay): ${fmtCur(amt_A_xsp)} (${a_xsp.toFixed(1)}%)`;
            ...
            document.getElementById('b_warning').innerText = Math.abs(b_tot - 100) > 0.1 ? `Asset Total: ${b_tot.toFixed(1)}%. Must = 100%!` : "";
            ...
            let c_xsp  = parseFloat(document.getElementById('c_xsp').value);  let c_cash = parseFloat(document.getElementById('c_cash').value);
            let c_tot = c_ib01 + c_cspx + c_xuse + c_eimi + c_cash;
            document.getElementById('c_warning').innerText = Math.abs(c_tot - 100) > 0.1 ? `Asset Total: ${c_tot.toFixed(1)}%. Must = 100%!` : "";
            
            const amt_C_ib01 = sC * (c_ib01/100); const amt_C_cspx = sC * (c_cspx/100); const amt_C_xuse = sC * (c_xuse/100); 
            const amt_C_eimi = sC * (c_eimi/100); const amt_C_xsp  = sC * (c_xsp/100);  const amt_C_cash = sC * (c_cash/100);
            ...
            document.getElementById('c_xsp_label').innerText  = `Options Margin (Overlay): ${fmtCur(amt_C_xsp)} (${c_xsp.toFixed(1)}%)`;
            ...
            document.getElementById('d_warning').innerText = Math.abs(d_tot - 100) > 0.1 ? `Asset Total: ${d_tot.toFixed(1)}%. Must = 100%!` : "";
            ...
            let barData =[
                ...
                // Margin Overlay: Diamond Markers
                { x: ['Silo A', 'Silo C'], y:[amt_A_xsp, amt_C_xsp], name: 'Options Margin (Overlay)', type: 'scatter', mode: 'markers', 
                  marker: {symbol: 'diamond', size: 14, color: '#ef4444', line: {color: 'white', width: 1.5}}, 
                  hovertemplate: 'Collateral Locked (Not a physical asset): $%{y:,.0f}<extra></extra>' }
            ];

            let barLayout = { 
                barmode: 'stack', 
                title: 'Physical Assets per Silo ($ / %)', 
                margin: {b: 40, t: 80}, 
            ...
            // Chart 2: Pie Chart (ONLY PHYSICAL ASSETS)
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v8 created with Accounting Fixes.")
