"""
=============================================================================
Script Name: attribution_engine_v22.py
Purpose: Parses the official IBKR Flex Query (MTM and CNAV).
         ...
         - FIXED: Double-Counting Bug. Excludes IBKR "Grand Total" summary rows.
...
def run_attribution():
    print("========================================================")
    print("   ESTATE P&L ATTRIBUTION ENGINE (v22 FULL AUTOMATION)")
        ...
        for row in reader:
            ...
            # -- EXTRACT DATA --
                ...
                if sec_type == 'CNAV' and cnav_idx:
                    ...
                    # v22 FIX: Ignore Summary Rows to prevent parsing "Total" as an account
                ...
                # --- MARK-TO-MARKET PERFORMANCE SECTION (MTMP) ---
                    ...
                    # V22 FIX: Only drop explicit "Total" rows. Do NOT drop blank symbols (Restores Cash/FX PnL)
                    ac_str = str(asset_class).strip().lower()
                    sym_str = str(symbol).strip().lower()
                    if 'total' in ac_str or 'total' in sym_str:
                        continue
    ...
    # -------------------------------------------------------------------------
    # WRITE CASH FLOWS & ATTRIBUTION (v22: DYNAMIC STITCHING)
    # -------------------------------------------------------------------------
    ...
    if min_date:
        # v22: Only delete official flows that are equal to or newer than the CSV's oldest date.
    ...
    print("[+] SUCCESS: v22 Engine Complete. Infinite Data Stitching Deployed.\n")
