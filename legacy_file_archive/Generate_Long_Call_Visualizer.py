r"""
=============================================================================
Script Name: Generate_Long_Call_Visualizer.py
Purpose: Generates a unified, interactive HTML dashboard to visualize 
         LONG CALL Options (Buying Options). Features real-time Black-Scholes 
         Call calculations, showing the brutal impact of Time Decay (Theta) 
         on an option buyer.
...
file_name = "Long_Call_Visualizer.html"
...
<head>
    ...
    <title>Long Call Option Visualizer</title>
...
<body id="main-body" class="neutral-bg text-gray-800 font-sans p-4 md:p-8">
    ...
    <script>
        ...
        function updateDashboard() {
            ...
            let maxLoss = prem_paid * qty * 100; // Buyer's max loss is premium paid
            ...
            document.getElementById('out_theta').innerText = "-$" + Math.abs(netTheta).toFixed(2); // Emphasize it's a bleed
            ...
            let min_plot = K * 0.7; // Scale appropriately for stocks vs indexes
            let max_plot = K * 1.5; 
            
            for(let p = min_plot; p <= max_plot; p += 0.5) {
                x_vals.push(p);
                // Expiration: Max(0, S-K) - Premium
