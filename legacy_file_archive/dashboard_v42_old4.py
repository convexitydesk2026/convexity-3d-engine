...
3. Launch the UI by running exactly:
   streamlit run dashboard_v42.py

CHANGELOG (v42):
         - FIX: Dynamic FX (Currency) Converter utilizing v22 DB schema.
         - FIX: Hardened Epoch Base NAV to prevent 1R fluctuation.
         - FIX: Physical US Stock parser utilizing native currency tags.
         - NEW: Champion Closed Trades Ledger & Expectancy Matrix.
=============================================================================
"""
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v22.py")
...
COLOR_PALETTE = {
    ...
    'Physical US Stocks': '#4f46e5', 'International Stocks': '#db2777'
}
...
def fetch_fx_rates():
    ...
    inverted = {'KRW': 'KRW=X', 'SEK': 'SEK=X', 'CAD': 'CAD=X', 'CHF': 'CHF=X', 'JPY': 'JPY=X'}
...
def load_champion_history():
    ...
    conn.commit()
    df = pd.read_sql_query("SELECT * FROM champion_closed_trades ORDER BY close_date DESC", conn)
...
active_global_tor = champion_df['Open Risk (R)'].sum() if not champion_df.empty else 0
...
<div style="background-color: #f3f4f6; padding: 20px; border-radius: 12px; border: 1px solid #e5e7eb; margin-bottom: 20px;">
    ...
    <div style="display: grid; grid-template-columns: repeat(7, 1fr); gap: 15px; text-align: center; font-family: monospace; font-size: 20px;">
        ...
        <div style="margin-top:20px;">Global DD Days<br><span style="color: #1f2937; font-weight: 900; font-size: 28px;">{global_metrics['dd_days']} d</span></div>
        <div style="margin-top:20px;">Calmar<br><span style="color: #1f2937; font-weight: 900; font-size: 28px;">{calmar:.2f}</span></div>
        <div style="margin-top:20px;">Global ROC<br><span style="{col_html(global_metrics['roc'])} font-weight: 900; font-size: 28px;">{global_metrics['roc']:.2f}%</span></div>
...
if not journal_raw_df.empty:
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            is_call = K_s < K_l
            is_long_put = (K_s == 0 or pd.isna(K_s)) and K_l > 0
            ...
            with col_3d:
                ...
                # Theta Glide Path (Works for both Spreads and Long Puts)
                ...
                for d in y_glide:
                    ...
                    if is_long_put:
                        ...
                        if is_call:
                            t_s_glide, _, _, _, _ = get_call_greeks(S_live, K_s, T_glide, r_rate, iv_dec)
                            t_l_glide, _, _, _, _ = get_call_greeks(S_live, K_l, T_glide, r_rate, iv_dec)
                        else:
                            t_s_glide, _, _, _, _ = get_put_greeks(S_live, K_s, T_glide, r_rate, iv_dec)
                            t_l_glide, _, _, _, _ = get_put_greeks(S_live, K_l, T_glide, r_rate, iv_dec)
...
if not champion_df.empty:
    ...
    global_tor = champion_df['Open Risk (R)'].sum()
...
with c_log:
    st.markdown("##### 📝 Log Alpha Exit (Full or Partial)")
    st.caption("When a trade closes in TWS, log the realized P&L here to update your R-Metrics.")
    with st.form("champion_log_form", clear_on_submit=True):
        state_symbols = all_states_df['symbol'].unique().tolist() if not all_states_df.empty else []
        active_symbols = champion_df['Symbol'].unique().tolist() if not champion_df.empty else []
        unified_symbols = sorted(list(set(active_symbols + state_symbols)))
        
        log_sym = st.selectbox("Select Asset to Log", unified_symbols + ["Other (Manual Entry)"])
        ...
        log_notes = st.text_input("Tags / Notes (e.g., 'Scaled out 50% at target')")
        
        if st.form_submit_button("Record Trade in Ledger"):
            if log_sym != "Other (Manual Entry)" and not all_states_df[all_states_df['symbol'] == log_sym].empty:
                match = all_states_df[all_states_df['symbol'] == log_sym].iloc[0]
                base_nav = match['base_nav']
                r_base = base_nav * 0.0025
                acc = match['account']
                silo = SILO_MAP.get(acc, [acc])[0]
            else:
                r_base = 500.0 # Fallback 1R base if manually entered
                acc = "Manual"
                silo = "Manual"
            ...
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("INSERT INTO champion_closed_trades VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
                      (log_date.isoformat(), log_sym, acc, silo, r_base, log_pnl, r_multiple, log_notes))
            conn.commit()
            conn.close()
            st.success(f"Successfully logged {log_sym} Exit: {r_multiple:+.2f} R")
            st.cache_data.clear()
            st.rerun()

with c_calc:
    st.markdown("##### 🏆 Champion Profitability Calculator")
    st.caption("Aggregated Expectancy, Win Rate, and R-Multiples across all closed Alpha trades.")
    
    if not champ_hist_df.empty:
        tot_trades = len(champ_hist_df)
        winners = champ_hist_df[champ_hist_df['r_multiple'] > 0]
        losers = champ_hist_df[champ_hist_df['r_multiple'] <= 0]
        ...
        net_r_gained = champ_hist_df['r_multiple'].sum()
        ...
        <table style="width:100%; border-collapse: collapse; background-color: white; border: 1px solid #cbd5e1; font-size: 14px;">
            <tbody>
                <tr style="background-color: #f8fafc; border-bottom: 1px solid #e5e7eb;">
                    <td style="padding: 10px; font-weight: bold; color: #334155;">Total Exits Logged</td>
        ...
        st.info("No closed trades logged yet. As you scale out of positions, log the exits on the left and the R-Metrics matrix will populate here.")