...
streamlit run dashboard_v113.py
CHANGELOG (v113):
...
- STAGE 20 (v112): Converted Accountability Journal 'Tags' column to a free-form comma-separated text field to bypass Streamlit's single-select dropdown limitation. Added 'ATR Ext' and injected a visual Tag Dictionary cheat-sheet.
=============================================================================
"""
    ...
    with st.expander("Position Sizing Engine", expanded=False):
        ...
        if calc_mode == "Scale-In (Pyramid)":
            ...
            if active_alpha.empty:
                ...
                st.markdown(f"<div style='font-size: 11px; background-color: #eff6ff; padding: 6px; border-radius: 4px; color: #1e3a8a; margin-bottom: 10px; border: 1px solid #bfdbfe;'>Existing Position: <b>{abs(existing_shares):,.0f}</b> shares @ <b>{existing_avg_local:,.2f} {calc_currency}</b></div>", unsafe_allow_html=True)
                
                calc_entry = st.number_input(f"New Entry Price ({calc_currency})", min_value=0.0, value=0.0, step=1.0)
                calc_stop = st.number_input(f"New Blended Stop Loss ({calc_currency})", min_value=0.0, value=0.0, step=1.0)
        ...
        if calc_entry > 0 and calc_stop > 0 and calc_nav > 0:
            ...
            if valid_dir:
                ...
                if calc_mode == "Scale-In (Pyramid)" and is_valid_pyramid:
                    if calc_dir == "Long" and calc_entry <= existing_avg_local:
                        st.error("🚫 **CFO Reject:** New entry must be HIGHER than existing average cost (Do not add to losers).")
                        valid_dir = False
                    elif calc_dir == "Short" and calc_entry >= existing_avg_local:
                        st.error("🚫 **CFO Reject:** New entry must be LOWER than existing average cost (Do not add to losers).")
                        valid_dir = False
                    else:
                        if calc_dir == "Long":
                            old_risk_usd = (existing_avg_usd - stop_usd) * existing_shares
                        else:
                        ...
                        if available_risk_usd <= 0:
                            st.error(f"⚠️ **Risk Budget Exhausted.** The existing position already consumes ${old_risk_usd:,.0f} of risk (Silo Limit: ${risk_budget_usd:,.0f}). You must raise the Stop Loss tighter to mathematically subsidize a new entry.")
                            valid_dir = False
                        else:
                            proposed_shares = int(available_risk_usd // new_risk_per_share_usd)
                else:
                    proposed_shares = int(risk_budget_usd // new_risk_per_share_usd)

                if valid_dir and proposed_shares > 0:
            ...
            for w_idx, week in enumerate(month_matrix):
                ...
                for day in week:
                    if day == 0:
                        ...
                        week_html += f"<div class='cal-cell'><div class='cal-cell-header'><span>{day}</span></div>{pnl_str}{ret_str}</div>"
...
# Dynamic Days Active Updater
cj.execute("SELECT id, open_date FROM alpha_campaigns WHERE status='Open'")
    ...
    if st.button("🔍 Scan for Undocumented Campaigns", width="stretch"):
        ...
        for _, r in open_positions.iterrows():
            ...
            if not cj.fetchone():
                ...
                # FIX: Commit immediately after every insertion to release the DB write-lock.
                conn_journal.commit()
                new_campaigns += 1
...
with col_close:
    if st.button("🏁 Check for Closed Campaigns", width="stretch"):
        cj.execute("SELECT id, symbol, open_date, entry_price, initial_stop FROM alpha_campaigns WHERE status='Open'")
        open_camps = cj.fetchall()
        closed_count = 0
        
        for camp in open_camps:
            c_id, sym, o_date, entry, stop = camp
            pos_row = pos_df[pos_df['symbol'] == sym]
            
            if pos_row.empty or pos_row['position'].iloc[0] == 0:
                ...
                if total_pnl == 0.0:
                    cj.execute("SELECT SUM(realized_pnl) FROM daily_positions WHERE symbol=? AND date >= ?", (sym, o_date))
                    pnl_fb = cj.fetchone()[0]
                    total_pnl = pnl_fb if pnl_fb else 0.0
                    
                # 2. Max Shares Check for Risk Math
                cj.execute("SELECT MAX(ABS(position)) FROM daily_positions WHERE symbol=? AND date >= ?", (sym, o_date))
        ...
        conn_journal.commit()
        if closed_count > 0:
            st.success(f"Successfully closed, graded, and locked {closed_count} campaigns!")
            st.rerun()
        else:
            st.info("No open campaigns were detected as closed today.")

st.markdown("##### 🟢 Active Campaigns (Action Required: Set Stops & Tags)")
df_open = pd.read_sql_query("SELECT id, open_date, symbol, type, regime_in, sector, industry, sma_20, sma_50, sma_200, entry_price, initial_stop, tags, thesis, days_active FROM alpha_campaigns WHERE status='Open'", conn_journal)
...
if not df_open.empty:
    ...
    edited_open = st.data_editor(
        ...
        disabled=["id", "open_date", "symbol", "type", "regime_in", "sector", "industry", "sma_20", "sma_50", "sma_200", "entry_price", "days_active"],
        hide_index=True,
