"""
=============================================================================
Script Name: sync_engine_v16.py
Purpose: Phase 2 of the Local-First Estate Architecture.
         - Fixes the PnL Math Glitch (from v15).
         - NEW: Audits and captures 'AccruedCash' to track the "invisible bleed" 
           of CFD overnight financing holding costs. Injects it as a CASH 
           liability position to perfectly balance Net Liquidation.
=============================================================================
"""
...
def sync_tws():
    print("========================================================")
    print("   ESTATE SYNC ENGINE v16 - CFD OVERNIGHT AUDIT")
    print("========================================================")
    ...
    for acc in accounts:
        ...
        # Insert 0.0 for daily deposits to preserve historical math
        ...
        # Save the Uninvested Cash directly as a portfolio position
        ...
        # NEW: Audit and Save Accrued Interest (CFD Overnight Bleed) as a liability position
        if accrued_cash != 0:
            cursor.execute('''INSERT INTO daily_positions
                              (date, account, symbol, sec_type, position, market_price, market_value, avg_cost, unrealized_pnl, realized_pnl)
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
    ...
    conn.commit()
    conn.close()
    print(f"[+] Sync Complete. Accrued CFD Fees captured. DB Patched.")
