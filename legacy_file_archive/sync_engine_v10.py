"""
=============================================================================
Script Name: sync_engine_v10.py
Purpose: Phase 1 of the Local-First Estate Architecture.
         Uses deterministic verification via the Master Position List to
         guarantee 100% data extraction and completely bypass TWS pacing.
=============================================================================
"""
...
def sync_tws():
    print("========================================================")
    print("   ESTATE SYNC ENGINE v10 - DETERMINISTIC VERIFICATION")
    print("========================================================")
    ...
    print("[*] Fetching Master Position List to bypass TWS stream delays...")
    master_positions = ib.reqPositions()
    expected_counts = {}
    ...
    for acc in accounts:
        ...
        print(f"[*] Master List indicates {expected} active contracts.")
        
        # 2. Open the stream
        ib.client.reqAccountUpdates(True, acc)
        ib.sleep(2.0) # Initial packet arrival buffer
        
        # 3. DETERMINISTIC WAIT
        # We hold the line until the stream delivers exactly what the Master List promised,
        # OR until we at least get the account balances for empty silos.
        timer = 0
        while (len(ib.portfolio(acc)) < expected or len(ib.accountValues(acc)) == 0) and timer < 15:
            ib.sleep(1.0)
            timer += 1
            
        if timer >= 15:
            print("[!] Warning: TWS stream timed out before all contracts arrived.")
        else:
            print(f"[+] Verified: {len(ib.portfolio(acc))}/{expected} streaming positions received.")
            
        # 4. Snap the data
        values = ib.accountValues(acc)
        portfolio_items = ib.portfolio(acc)
        
        # 5. Shut off stream & enforce Anti-Spam Cooldown
