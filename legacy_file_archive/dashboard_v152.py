...
streamlit run dashboard_v152.py
CHANGELOG (v152):
- STAGE 59 (v152): Implemented v7.1 "6-Gear Progressive Braking System" and "Sector Veto" macro governors.
...
def load_benchmarks(start_date, end_date):
    ...
    bench_df['qqq_sma_20'] = bench_df['QQQ'].rolling(window=20).mean()
    ...
    def get_regime_and_tor(row):
        ...
        rsp_20, rsp_50 = row['rsp_sma_20'], row['rsp_sma_50']
        spy_20, spy_50 = row['sma_20'], row['sma_50']
        qqq_20, qqq_50 = row['qqq_sma_20'], row['qqq_sma_50']
        
        if pd.isna(rsp_50) or pd.isna(spy_50) or pd.isna(qqq_50): 
            return 'Yellow', 2
            
        indices_below_50 = sum([spy < spy_50, qqq < qqq_50, rsp < rsp_50])
        
        # Gear 0 (Red): Multiple indices < 50 SMA OR VIX > 22
        if indices_below_50 >= 2 or vix > 22:
            return 'Red', 0
            
        # Gear 1 (Orange): ONE index breaks < 50 SMA, others hold. VIX < 22
        elif indices_below_50 == 1 and vix < 22:
            return 'Yellow', 1
            
        # Gear 5 (Deep Green): All indices > 20 SMA AND VIX < 15
        elif spy > spy_20 and qqq > qqq_20 and rsp > rsp_20 and vix < 15:
            return 'Green', 5
            
        # Gear 4 (Light Green): All indices > 20 SMA AND VIX 15-18
        elif spy > spy_20 and qqq > qqq_20 and rsp > rsp_20 and 15 <= vix <= 18:
            return 'Green', 4
            
        # Gear 3 (Yellow-Green): Indices mixed but ALL > 50 SMA AND VIX < 18
        elif indices_below_50 == 0 and vix < 18:
            return 'Green', 3
            
        # Gear 2 (True Yellow): All indices > 50 SMA BUT VIX 18-22
        elif indices_below_50 == 0 and 18 <= vix <= 22:
            return 'Yellow', 2
...
with calc_placeholder.container():
    ...
    with st.expander("Position Sizing Engine", expanded=not is_locked):
        ...
        base_gear = temp_chart_ui['tor'].iloc[-1] if not temp_chart_ui.empty else 2
        is_vetoed = False
        if target_sym_atr:
            is_vetoed = evaluate_sector_veto(target_sym_atr)
            
        active_gear = max(0, base_gear - 1) if is_vetoed else base_gear
        gear_risk_map = {5: 0.25, 4: 0.25, 3: 0.125, 2: 0.125, 1: 0.05, 0: 0.00}
        max_r = gear_risk_map.get(active_gear, 0.0)
        
        c1, c2 = st.columns(2)
        
        if active_gear == 0:
            calc_risk_pct = 0.0
            c1.number_input("Risk %", value=0.0, disabled=True, help="Gear 0 (Red): Alpha Engine Halted.")
        else:
            veto_text = " (Sector Veto: -1 Gear)" if is_vetoed else ""
            calc_risk_pct = c1.number_input("Risk %", min_value=0.01, max_value=max_r, value=max_r, step=0.025, format="%.3f", disabled=is_locked, help=f"Gear {active_gear}{veto_text}: Max Risk capped at {max_r}%.")
        ...
        if calc_mode == "Scale-In (Pyramid)":
            ...
            # calc_sym_init is already captured above for the Sector Veto
        ...
        if calc_entry > 0 and calc_stop > 0 and calc_nav > 0 and not is_locked:
            ...
            if valid_dir:
                # Use the active_gear calculated earlier
                daily_tor_pct = active_gear * 0.5  
...
chart_df['regime_bg'] = chart_df['tor'].map(gear_color_map)
chart_df['regime_txt'] = chart_df['tor'].map(gear_txt_map)
