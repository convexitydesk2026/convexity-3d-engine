"""
=============================================================================
Script Name: dashboard_v11.py
Purpose: The Streamlit Frontend (Module 11).
         - Monte Carlo simulated paths correctly grounded at $0 origin.
         - Option Spread Margin logic corrected (divided by 2 legs).
         - Universal Zero-Axis standardization (1px solid black).
         - Matrix reconciles exactly to 100.00% via Accruals/FX dynamic row.
         - XSP/XND options explicitly mapped to their Matrix rows.
=============================================================================
"""
...
if not pos_df.empty:
    ...
    with col_bar:
        ...
        # Add Margin Lock Diamond Marker (Math logic fixed to divide legs by 2)
...
# Calculate Current Global Allocations (Options Logic Fixed)
alloc_map = {}
...
def get_pct(inst):
    if inst == 'Accruals, Unsettled & FX': return 0.0 # Handled below
...
# Apply Dynamic Matrix Reconciliation
...
with c2:
    # Math Fixed: Divided legs by 2 to accurately count the real spread margin capacity
    opt_margin_total = (pos_df[pos_df['sec_type'] == 'OPT']['position'].abs().sum() / 2) * 2500
...
if sim_length > 0:
    ...
    # Ground paths dynamically to $0 on Day 0
    ...
    # Ground history path to $0 on Day 0
