...
streamlit run dashboard_v104.py
CHANGELOG (v104):
- STAGE 12 (v104): Implemented Granular Daily PnL Calendar matrix inside the Institutional Return Heatmap section.
- STAGE 11 (v103): Integrated Macro Risk Governor into the pre-trade calculator. Enforces Global TOR limits across all silos and pinpoints risk culprits with actionable advice.
- STAGE 10 (v102): Upgraded Alpha Risk Calculator with Pyramiding (Scale-In) Logic. Auto-fetches existing cost basis and enforces "No Adding to Losers" institutional protocol.
=============================================================================
"""
...
with st.sidebar:
    ...
    if st.button("🌐 Generate Market Flow Report", width="stretch"):
        with st.spinner("Calculating Institutional Flow & Generating PDF... Please wait (~15s)"):
            try:
                subprocess.run(["python", "market_flow_engine_v2.py"], cwd=TARGET_DIR, check=True)
        ...
        if calc_entry > 0 and calc_stop > 0 and calc_nav > 0:
            ...
            if valid_dir:
                ...
                if valid_dir and proposed_shares > 0:
                    ...
                    daily_tor_pct = temp_chart['tor'].iloc[-1] if not temp_chart.empty else 0
                    ...
                    if projected_global_risk > max_global_risk_usd:
                        ...
                        <div style="background-color: #fef2f2; padding: 15px; border-radius: 6px; border: 1px solid #ef4444; margin-bottom: 15px;">
                            ...
                            <div style="font-size: 12px; color: #7f1d1d;">
                                Your regime allows <b>{daily_tor_pct}%</b> Global NAV risk (<b>${max_global_risk_usd:,.0f}</b>).<br>
                                Current Open Risk is <b>${current_global_risk_abs:,.0f}</b>.<br>
                                This trade pushes projected risk to <b>${projected_global_risk:,.0f}</b> (Excess: ${projected_global_risk - max_global_risk_usd:,.0f}).<br><br>
                                {culprit_text}
                                {action_text}
                            </div>
                        </div>
...
if not global_df.empty:
    with st.expander("📅 View Granular Daily PnL Calendar", expanded=False):
        ...
        if available_months:
            ...
            cal_html = f"""
            <style>
                .cal-wrapper {{ background: #1e293b; color: #f8fafc; border-radius: 8px; padding: 20px; font-family: sans-serif; margin-top: 10px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }}
                .cal-header {{ display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; border-bottom: 1px solid #334155; padding-bottom: 10px; }}
                .cal-title {{ font-size: 22px; font-weight: bold; color: #e2e8f0; }}
                .cal-total {{ font-size: 16px; font-weight: normal; color: #94a3b8; }}
                .cal-total span {{ font-weight: bold; }}
                .cal-grid {{ display: grid; grid-template-columns: repeat(8, 1fr); gap: 1px; background: #334155; border: 1px solid #334155; border-radius: 6px; overflow: hidden; }}
                .cal-day-label {{ background: #0f172a; color: #94a3b8; font-size: 13px; text-align: center; padding: 10px 0; font-weight: bold; text-transform: uppercase; }}
                .cal-cell {{ background: #1e293b; padding: 12px; display: flex; flex-direction: column; min-height: 90px; transition: background 0.2s; }}
                .cal-cell:hover {{ background: #283548; }}
                .cal-cell-header {{ font-size: 14px; color: #cbd5e1; margin-bottom: 8px; font-weight: bold; display: flex; justify-content: space-between; }}
                .pnl-green {{ color: #34d399; font-weight: bold; font-size: 16px; margin-top: auto; }}
                .pnl-red {{ color: #f87171; font-weight: bold; font-size: 16px; margin-top: auto; }}
                .pnl-zero {{ color: #64748b; font-size: 16px; margin-top: auto; }}
                .ret-text {{ font-size: 12px; color: #94a3b8; margin-top: 4px; }}
                .week-total-cell {{ background: #0f172a; border-left: 2px solid #334155; }}
            </style>
            <div class="cal-wrapper">
                <div class="cal-header">
                    <div class="cal-title">{sel_month_name}, {sel_year}</div>
                    <div class="cal-total">Monthly P&L: <span style="color: {month_color};">{month_sign}${month_pnl_total:,.2f}</span></div>
                </div>
                <div class="cal-grid">
            """
            ...
            for w_idx, week in enumerate(month_matrix):
                ...
                for day in week:
                    if day == 0:
                        ...
                        # Uses Return % instead of trade count (since DB natively tracks performance, not execution quantities)
                        ret_str = f"<div class='ret-text'>{day_ret:+.2f}%</div>" if day_pnl != 0 else "<div class='ret-text'>0.00%</div>"
                            
                        week_html += f"""
                        <div class='cal-cell'>
                            <div class='cal-cell-header'><span>{day}</span></div>
                            {pnl_str}
                            {ret_str}
                        </div>
                        """
                        
                # Compile Weekly Totals Column
                if week_pnl > 0:
                ...
                week_html += f"""
                <div class='cal-cell week-total-cell'>
                    <div class='cal-cell-header' style='color:#f8fafc;'>Week {w_idx+1}</div>
                    {w_pnl_str}
                </div>
                """
                cal_html += week_html
