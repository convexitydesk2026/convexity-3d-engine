r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v62.py
Purpose: Generates the Interactive Family Estate Dashboard.
         ...
         - Hardcoded filename output to v62.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v62.html") # v62 CHANGE
...
def process_metrics(df_acc):
    ...
    # --- v62 FIX: Eliminate the "Dead Air" penalty for Volatility Metrics ---
...
print("\n========================================================")
print("             CIO DIAGNOSTICS & LOGGING (v62)            ") # v62 CHANGE
print("========================================================")
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        # --- v62 ALIGNMENT LOGIC: Find the exact day actual trading started ---
...
<head>
    ...
    <title>Estate Master Dashboard v62</title> <!-- v62 CHANGE -->
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8 relative">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v62</p>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <div class="xl:col-span-12 mt-2 grid grid-cols-1 xl:grid-cols-12 gap-4">
                ...
                <!-- MONTE CARLO (Left - 8 cols) -->
                <div class="xl:col-span-8 bg-white border rounded-xl p-4 shadow-sm overflow-hidden flex flex-col">
                ...
                <!-- PNL ATTRIBUTION & VELOCITY (Right - 4 cols) -->
                <div class="xl:col-span-4 bg-white border rounded-xl p-4 shadow-sm flex flex-col">
                    <h3 class="text-xl font-bold mb-4 text-gray-800 border-b pb-2">PnL Attribution & Capital Velocity</h3>
                    
                    <!-- Donut Chart -->
                    <div id="pnl-attribution-chart" class="w-full h-[320px] mb-4 border-b border-gray-100 pb-2"></div>
                    
                    <!-- Capital Velocity Widget -->
                    <div class="flex-grow flex flex-col justify-end">
                        <h4 class="text-xs font-black text-gray-500 uppercase tracking-widest mb-3">Options Engine (Silo C) Velocity</h4>
            ...
            <!-- ========================================================= -->
            <!-- PANEL 6: DEPLOYMENT & CAPACITY TRACKER (v62 UPGRADE)      -->
            <!-- ========================================================= -->
            <div class="xl:col-span-12 mt-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 relative overflow-hidden flex flex-col justify-between">
                            <div>
                                ...
                                <!-- v62 DYNAMIC DEPLOYMENT SCHEDULE -->
    ...
    <script>
        ...
        function updateUI() {
            ...
            // =========================================================
            // PANEL 6 LOGIC (Deployment Tracker - v62 UPGRADE)
            // =========================================================
            ...
            // v62: Dynamic Tranche Calculator & Date Generator
            let baseTranche = excessCash > 0 ? (excessCash / 10) : 0; // Deploys excess over 5 months (10 bi-weekly tranches)
            let dcaAmount = Math.max(baseTranche, 10000); // Ensures a minimum $10k tranche if excess is small but > 0
            ...
            // v62 PnL Attribution Donut Plot (Zero-Sum Fix)
            let pnlAttrData = safeParse('VAR_PNL_ATTRIBUTION');
            if(Object.keys(pnlAttrData).length > 0) {
                let attrColors = { "Risk-Free Yield": "#3b82f6", "VRP Engine": "#86efac", "Active Alpha": "#a855f7", "Core Beta": "#f97316" };
                let attrLabels = Object.keys(pnlAttrData);
                let attrValues = Object.values(pnlAttrData).map(v => Math.max(v, 0)); 
                
                // Fallback if all data is exactly zero (prevents invisible chart)
                let sumValues = attrValues.reduce((a, b) => a + b, 0);
                if (sumValues === 0) {
                    attrLabels = ["Awaiting Trading Data"];
                    attrValues = [1];
                    attrColors = { "Awaiting Trading Data": "#e5e7eb" };
                }
                ...
                let donutData =[{
                    ...
                    marker: { colors: attrLabels.map(l => attrColors[l] || "#9ca3af") }
                }];
                ...
                let donutLayout = {
                    ...
                    annotations:[{ font: {size: 13, weight: 'bold'}, showarrow: false, text: 'PnL<br>Sources', x: 0.5, y: 0.5 }]
                };
                Plotly.react('pnl-attribution-chart', donutData, donutLayout, {displayModeBar: false, responsive: true});
            }
        }
        ...
        if (datesPnl && datesPnl.length > 0) {
            ...
            // Apply 12 days of padding to the left, 5 to the right
        ...
        if (Object.keys(liveAlloc).length > 0) {
            ...
            if (liveAlloc['A']) {
                ...
                document.getElementById('a_xsp').value = liveAlloc['A']['OPT_MARGIN'] || 0; // v62 CHANGE
            }
            ...
            if (liveAlloc['C']) {
                ...
                document.getElementById('c_xsp').value = liveAlloc['C']['OPT_MARGIN'] || 0; // v62 CHANGE
            }
        }
...
silo_d_holdings_js = str([]) # Bypassing the parsing glitch
...
# --- v62 PNL ATTRIBUTION INJECTION ---
pnl_json_path = os.path.join(target_directory, 'PnL_Attribution.json')
pnl_attr_js = str({})
if os.path.exists(pnl_json_path):
    with open(pnl_json_path, 'r') as f:
        pnl_attr_js = f.read()
html_content = html_content.replace("'VAR_PNL_ATTRIBUTION'", f"'{pnl_attr_js}'")

# --- v62 CAPITAL VELOCITY INJECTION ---
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v62 created.")
