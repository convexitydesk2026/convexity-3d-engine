...
3. Launch the UI by running exactly:
   streamlit run dashboard_v66.py

CHANGELOG (v66):
         - UPDATED: Master Options Matrix to include Conviction CSPs & 120-DTE Black Swan nomenclature.
         - REMOVED: Future Consideration instruments (/MES, TLT, VIX) to maintain structural simplicity.
=============================================================================
"""
...
def fetch_live_data(ticker_symbol):
    # v66: Safeguard against empty cells during live data-entry
...
def get_exact_opt_margin(df_in):
    ...
    try:
        ...
        # v66 FIX: Added base_tckr to the groupby!
...
# v66 FIX: Added Physical US Stocks & International Stocks to Core Assets pool
...
st.subheader("6C. The S.W.A.N. Stress Test (Net-Shock Visualizer)")

swan_shock_pct = 0.30
swan_vix_spike = 0.80

# 1. Equity & Synthetic Beta Losses
swan_equity_loss = bw_usd_exposure * swan_shock_pct

# 2. VRP Stop-Loss Assumptions
swan_vrp_loss = 0.0
if not journal_raw_df.empty:
...
# 3. Tail Hedge Payout Simulation
swan_th_payout = 0.0
for _, r in pos_df[pos_df['asset_class'] == 'Tail Hedge'].iterrows():
    try:
        parts = r['symbol'].split('_')
...
with col_swan_chart:
    fig_waterfall = go.Figure(go.Waterfall(
        ...
        x=["Starting NAV", "Equities (-30%)", "VRP Stops", "Tail Payout", "Ending NAV"],
    ...
    fig_waterfall.update_layout(
        title="Portfolio Impact Waterfall (-30% S&P 500 Crash)",
        margin=dict(l=20, r=20, t=40, b=20),
        plot_bgcolor='rgba(0,0,0,0)',
...
with col_swan_text:
    ...
    <div style="background-color: #f8fafc; padding: 25px; border-radius: 8px; border: 1px solid #cbd5e1; height: 100%;">
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 15px;">
            ...
            <span style="font-size: 18px; font-weight: bold; color: #dc2626;">-30.0%</span>
        </div>
        ...
        <p style="font-size: 14px; color: #334155; font-style: italic; line-height: 1.6;">
            "If the S&P 500 crashes 30% tomorrow, the Estate will only suffer an estimated <b>{abs(swan_estate_impact_pct):.1f}%</b> drawdown. 
            The <b>${(swan_equity_loss + swan_vrp_loss):,.0f}</b> losses from our core equity exposure and VRP stops are overwhelmingly absorbed 
            by a projected <b>${swan_th_payout:,.0f}</b> payout from our deep OTM Black Swan insurance policies. Generational wealth is preserved."
        </p>
    </div>
    ''', unsafe_allow_html=True)
...
if not journal_raw_df.empty:
    ...
    if not active_vrp.empty:
        ...
        # v66: Dynamically map LIVE TWS PnL to calculate Live Annualized ROC
        live_roc_list = []
        live_ann_roc_list = []
...
if not journal_raw_df.empty:
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            # v66: 3D Engine Native Debit Spread & Call Detection
