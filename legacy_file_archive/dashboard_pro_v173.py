"""
=============================================================================
Script Name: dashboard_pro_v173.py
...
streamlit run dashboard_pro_v173.py
CHANGELOG (v173 PRO):
- STAGE 81 (v173 PRO): Added Section 100 Project Management Module (Gantt + 5-Week Calendar).
- STAGE 80 (v172 PRO): Dynamic Silo Generation. Eradicated hardcoded IBKR accounts. Implemented Hybrid Auto-Discovery via 
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v37.py")
...
df_tasks = pd.read_sql_query("SELECT * FROM project_tasks", conn_pm)
...
if len(df_tasks) <= 1:
    ...
    c_pm.executemany(
        "INSERT INTO project_tasks (task_name, start_date, end_date, status, category) VALUES (?, ?, ?, ?, ?)",
        sprint_tasks
    )
    conn_pm.commit()
    df_tasks = pd.read_sql_query("SELECT * FROM project_tasks", conn_pm)
...
def commit_task_edits():
    ...
    for row in adds:
        ...
        c_pm_edit.execute(
            "INSERT INTO project_tasks (task_name, start_date, end_date, status, category) VALUES (?, ?, ?, ?, ?)",
            (row.get('task_name', 'New Task'), start_val, end_val, row.get('status', 'Pending'), row.get('category', 'General'))
        )
...
if not edited_tasks.empty:
    ...
    st.markdown("### 📊 Sprint Waterfall (Gantt)")
    color_discrete_map = {'Pending': '#94a3b8', 'In Progress': '#3b82f6', 'Done': '#16a34a'}
    ...
    color_map_html = {'Pending': '#94a3b8', 'In Progress': '#3b82f6', 'Done': '#16a34a'}
    ...
    for i in range(35): # 5 weeks
        ...
        for _, task in day_tasks.iterrows():
            bg_color = color_map_html.get(task['status'], '#94a3b8')
