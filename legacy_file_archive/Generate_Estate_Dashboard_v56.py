r"""
=============================================================================
Script Name: Generate_Estate_Dashboard_v56.py
Purpose: Generates the Interactive Family Estate Dashboard.
         ...
         - Hardcoded filename output to v56.
...
full_file_path = os.path.join(target_directory, "Family_Estate_Dashboard_v56.html") # v56 CHANGE
...
def process_metrics(df_acc):
    ...
    # --- v56 FIX: Eliminate the "Dead Air" penalty for Volatility Metrics ---
...
print("\n========================================================")
print("             CIO DIAGNOSTICS & LOGGING (v56)            ") # v56 CHANGE
print("========================================================")
...
if not bench_df.empty and not global_df.empty:
    ...
    if first_date:
        ...
        # --- v56 ALIGNMENT LOGIC: Find the exact day actual trading started ---
...
<head>
    ...
    <title>Estate Master Dashboard v56</title> <!-- v56 CHANGE -->
...
<body class="bg-slate-50 text-gray-800 font-sans p-4 md:p-6">
    ...
    <div class="max-w-[98%] 2xl:max-w-[2400px] mx-auto bg-white rounded-xl shadow-xl p-6 border border-gray-200">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-500 mt-2 font-medium">Silo Allocation & Institutional Data Engine v56</p> <!-- v56 CHANGE -->
        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 relative overflow-hidden">
                            ...
                            <div class="w-full bg-gray-200 rounded-full h-2.5 mb-2">
                                <div class="bg-blue-600 h-2.5 rounded-full" id="trk_cash_bar" style="width: 0%"></div>
                            </div>
                            <p class="text-[10px] text-gray-600 font-bold" id="trk_cash_status">Status: Calculating...</p>
                        </div>
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 relative overflow-hidden">
                            ...
                            <p class="text-xs text-gray-500 mb-3 border-b pb-2">Estate Safety Cap: 25% of Net Liq</p>
                            <div class="flex justify-between items-end mb-1">
                            ...
                            <div class="w-full bg-gray-200 rounded-full h-2.5 mb-2 relative">
                                <div class="absolute top-0 left-[25%] w-0.5 h-full bg-red-600 z-10" title="25% Cap"></div>
                                <div class="bg-green-500 h-2.5 rounded-full" id="trk_margin_bar" style="width: 0%"></div>
                            </div>
                            <p class="text-[10px] text-gray-600 font-bold" id="trk_margin_status">Status: Calculating...</p>
                        </div>
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4 relative overflow-hidden">
                            ...
                            <div class="flex flex-col justify-center h-[60px]">
                                <span class="text-sm font-bold text-gray-800" id="trk_dca_action">Awaiting Regime Data...</span>
                            </div>
                        </div>
    ...
    <script>
        ...
        function updateUI() {
            ...
            document.getElementById('trk_cash_bar').style.width = Math.min(cashBufferPct, 100) + "%";
            
            if (cashBufferPct >= 40) {
                document.getElementById('trk_cash_status').innerHTML = `<span class="text-green-600">✓ Safe. Excess Cash: ${fmtCur(totalCashBuffer - (totalEstate * 0.40))} ready for deployment.</span>`;
            } else {
                document.getElementById('trk_cash_status').innerHTML = `<span class="text-red-600">⚠️ WARNING: Buffer under 40%. Halt Equity DCA.</span>`;
            }
            ...
            document.getElementById('trk_margin_pct').innerText = marginPct.toFixed(1) + "%";
            document.getElementById('trk_margin_bar').style.width = Math.min(marginPct, 100) + "%";
            
            if (marginPct < 25) {
                document.getElementById('trk_margin_status').innerHTML = `<span class="text-green-600">✓ Optimal. Remaining Capacity: ${fmtCur(maxMarginAllowed - totalMarginLocked)}</span>`;
                document.getElementById('trk_margin_bar').className = "bg-green-500 h-2.5 rounded-full";
            } else {
                document.getElementById('trk_margin_status').innerHTML = `<span class="text-red-600">⚠️ CAP BREACHED. Close spreads immediately.</span>`;
                document.getElementById('trk_margin_bar').className = "bg-red-600 h-2.5 rounded-full";
            }

            try {
                let lastRegime = regimeStatus && regimeStatus.length > 0 ? regimeStatus[regimeStatus.length - 1].toLowerCase() : 'yellow';
                let actionText = "";
                if (lastRegime === 'green') { actionText = "🟢 GREEN REGIME:<br>Proceed with scheduled systematic $10k DCA into Physical UCITS. Options engine clear for deployment."; }
                else if (lastRegime === 'yellow') { actionText = "🟡 YELLOW REGIME:<br>Hold current equity DCA. Funnel all new cash flow into IB01. Throttle options DTE to 45 strict."; }
                else if (lastRegime === 'red') { actionText = "🔴 RED REGIME:<br>Halt all active trading. Defend 40% Cash Buffer. Evaluate VIX for Taleb Tail-Hedge deployment."; }
        ...
        if (Object.keys(liveAlloc).length > 0) {
            ...
            if (liveAlloc['A']) {
                ...
                document.getElementById('a_xsp').value = liveAlloc['A']['OPT_MARGIN'] || 0; // v56 CHANGE
            }
            ...
            if (liveAlloc['C']) {
                ...
                document.getElementById('c_xsp').value = liveAlloc['C']['OPT_MARGIN'] || 0; // v56 CHANGE
            }
        }
...
try:
    ...
    print("====================================================================")
    print(f"SUCCESS! Estate Dashboard v56 created.")
