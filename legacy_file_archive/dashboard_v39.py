...
3. Launch the UI by running exactly:
   streamlit run dashboard_v39.py

CHANGELOG (v39):
         - NEW: Dedicated Cash Flow Ledger architecture.
         - FIX: Decoupled IBKR 'Cash Balance' from True Cash Flows.
         - FIX: Mathematical overhaul of process_metrics (IRR/PnL fixes).
         - UI: Added 'Log Cash Flow' input form to Sidebar.
         - (Includes all Naked Option/PDT alerts and v37 UI/UX polishes).
=============================================================================
"""
...
SYNC_SCRIPT = os.path.join(TARGET_DIR, "sync_engine_v19.py")
    ...
    if selected_rows.empty:
        ...
        if str(raw_dte) == 'Closed':
            ...
            with col_3d:
                ...
                # ---------------------------------------------------------
                # ENHANCEMENT S1: Solid Perimeters for the 4 Structural Planes
                # ---------------------------------------------------------
                # Green Plane Perimeter (Short Strike)
                fig_3d.add_trace(go.Scatter3d(
                    x=[K_s, K_s, K_s, K_s, K_s], 
                    y=[0, init_dte, init_dte, 0, 0], 
                    z=[z_min, z_min, z_max, z_max, z_min], 
                    mode='lines', line=dict(color='green', width=3), showlegend=False, hoverinfo='skip'
                ))
                
                # Red Plane Perimeter (Long Strike)
                fig_3d.add_trace(go.Scatter3d(
                    x=[K_l, K_l, K_l, K_l, K_l], 
                    y=[0, init_dte, init_dte, 0, 0], 
                    z=[z_min, z_min, z_max, z_max, z_min], 
                    mode='lines', line=dict(color='red', width=3), showlegend=False, hoverinfo='skip'
                ))
                
                # Yellow Plane Perimeter (50% Time Stop)
                fig_3d.add_trace(go.Scatter3d(
                    x=[x_vals[0], x_vals[-1], x_vals[-1], x_vals[0], x_vals[0]], 
                    y=[half_dte, half_dte, half_dte, half_dte, half_dte], 
                    z=[z_min, z_min, z_max, z_max, z_min], 
                    mode='lines', line=dict(color='yellow', width=3), showlegend=False, hoverinfo='skip'
                ))
                
                # Gray Plane Perimeter (Zero PnL Breakeven Floor)
                fig_3d.add_trace(go.Scatter3d(
                    x=[x_vals[0], x_vals[-1], x_vals[-1], x_vals[0], x_vals[0]], 
                    y=[0, 0, init_dte, init_dte, 0], 
                    z=[0, 0, 0, 0, 0], 
                    mode='lines', line=dict(color='gray', width=3), showlegend=False, hoverinfo='skip'
                ))

                # =========================================================
                # ⬇️ PASTE THIS NEW BLOCK EXACTLY HERE ⬇️
                # =========================================================
                
                # ENHANCEMENT A: Magenta Scaffolding (Plane Intersections)
                ...
                # ENHANCEMENT B: The Theta Glide Path (Flat Market Trajectory)
                ...
                fig_3d.add_trace(go.Scatter3d(x=[S_live] * len(y_glide), y=y_glide, z=z_glide, mode='lines', name='Theta Glide Path', line=dict(color='cyan', width=10, dash='dashdot')))
                ...
                # ENHANCEMENT S2: The 200% Stop-Loss Target (Red Cross)
                # A 200% stop-loss means taking a loss equal to 2x the premium collected.
                ...
                fig_3d.add_trace(go.Scatter3d(
                    ...
                    mode='markers', 
                    name='200% Stop Loss', 
                    marker=dict(color='#dc2626', size=15, symbol='cross')
                ))
